...
- READMEs for every published package: a full guide on the `concile` npm page,
...
- `@concile/cli` exposes a `./bin` subpath export so the `concile` umbrella
  package can ship the CLI as its own `concile` command.

## [0.1.0] — 2026-07-21

**Initial public release.** The whole platform, published to npm:

- **`concile`** — the one-package install: client SDK (`concile`), React hooks
  (`concile/react`), server function authoring (`concile/server`), schema +
  validators (`concile/values`), component composition (`concile/config`), and
  the `concile` CLI (`dev`, `serve`, `deploy`, `build`, `migrate`).
- **Components** (opt-in): `@concile/auth` (OAuth, passkeys, TOTP MFA, email
  flows), `@concile/scheduler` (scheduled functions + crons),
  `@concile/workflow` (durable workflows with saga compensation),
  `@concile/triggers`, `@concile/notifications`, `@concile/authz`.
- **Enterprise** (source-available, commercial license, free in this phase):
  `@concile/fleet` (multi-node scale-out), `@concile/objectstore-substrate`,
  `@concile/runtime-cloudflare-shard`.
- Engine internals published as `@concile/*` (storage adapters for SQLite,
...
**Cloudflare deployment tier — validated on real Cloudflare.** Concile now runs
...
- **Turnkey `cloudflare` deploy target** (`concile deploy --target cloudflare`). Reconciles `wrangler.jsonc` bindings (DO + `new_sqlite_classes` migration + `nodejs_compat`, optional R2) and shells `wrangler deploy`; never bundles a provider SDK.
...
Authentication hardening and notification delivery. `@concile/auth` gains TOTP
two-factor and passkeys/WebAuthn; `@concile/notifications` gains multi-provider
...
- **`@concile/docstore-sqlite`** (zero-config default) and **`@concile/docstore-postgres`** (`pg` driver, `pg_advisory_lock` single-writer, no app-schema migrations), selected via `--database-url`/`CONCILE_DATABASE_URL`. Behavioral parity via a shared conformance suite.
- **File storage** (`@concile/storage`): `_storage` system table + `ctx.storage`, two-phase proxied (FS) / presigned (S3) uploads behind a `BlobStore` seam (`@concile/blobstore-fs`, `@concile/blobstore-s3`), private-by-default HMAC capability URLs, background orphan reaper, Range requests.
...
- **`@concile/scheduler`** — `runAfter`/`runAt`/`cancel`, `cronJobs()` with catch-up policies, retries/backoff, cascading cancel, on a recurring **driver** seam.
- **`@concile/workflow`** — durable multi-step workflows via deterministic replay over a `workflows`/`steps`/`events` journal; `step.run*`/`sleep`/`waitForEvent`, `Promise.all` fan-out, a live `workflow:status` query, and **saga/compensation** (reverse-order unwind, halt-on-failed-compensation, cancel-compensates).
- **`@concile/triggers`** — durable cursor over the MVCC log (missed changes impossible by construction), at-least-once in-order per-document delivery, self-pause + circuit breaker.
...
- **`concile dev`** — watch + codegen + hot reload + serve sync/HTTP/dashboard.
- **`concile serve`** — production entrypoint (required admin key, `0.0.0.0`, graceful shutdown), working **Docker `docker compose up`** self-host, key-less dashboard.
- **`concile deploy`** — opt-in push-based live hot-swap of functions + additive-only schema onto a running `serve`, atomic swap.
- **`concile build`** — single self-contained executable via `bun build --compile`, cross-compile targets, `{"ready":…}` startup line.
- **`concile migrate`** — Convex-first on-ramp (import codemod + divergence report).
...
### Tiered scale-out (Tier 2, `ee/@concile/fleet`)
...
- **`@concile/test`** — Layer-1 `createTestConcile` over the real engine + a conformance suite.
- **`@concile/bench`** — reactive benchmark harness (`bun run bench:reactive`/`bench:compare`).
