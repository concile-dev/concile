...
First-party authentication for Concile: email + password accounts (argon2id hashing, per-call
...
3. **No httpOnly-cookie / CSRF mode** — Concile is WebSocket-first; identity flows over `SetAuth`,
...
7. **External identity is not a general SSO/IdP surface** — Concile is an OAuth/OIDC *client* and a
   third-party-JWT *verifier*, never an identity *provider*: no SAML, and Concile never issues
   tokens another service could verify.
8. **No provider access-token storage/refresh** — `signInWithIdToken`/the OAuth flow trade the
   external identity for a Concile session and then discard the provider's own
