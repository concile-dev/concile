 ...
 * mechanism `@concile/test`'s own `_test:_run` uses; this slice's tests follow the raw
 * `composeComponents` + `EmbeddedRuntime.create` idiom (not `createTestConcile`), so it's
