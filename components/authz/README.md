...
**Reactive, typed authorization for Concile.** One model that scales from a two-line ownership rule to multi-tenant SaaS with sharing and hierarchies — enforced *inside the engine* so you can't forget a check, and reactive *by construction* so revoking access empties live subscriptions instantly.
...
Most authorization is bolted on: a library you call (and forget to call), a database feature that only your DB enforces, or a separate service you sync to. Concile authz is **none of those**. It is:

- **Reactive by construction.** A permission check *reads* authorization data, and Concile tracks every query's read-set. So when you revoke a role or unshare a document, every affected live query **re-runs and updates in the same instant** — no cache invalidation, no polling, no refetch. No other authorization model gives you this, because none of them runs *inside* a reactive engine.
...
`@concile/authz` is a Concile component. Add it to your project's `concile.config.ts`:
...
```ts
// concile/documents.ts — no manual authz call needed; the policy is enforced by the engine
...
**Bootstrapping the first admin.** There is deliberately no ungated path to the first role. Seed it out-of-band through the privileged admin surface (the `concile` CLI / admin API / dashboard), which writes the first `admin` assignment directly — e.g. `runSystem("_system:insertDocument", { table: "authz/role_assignments", fields: { userId, role: "admin", scopeType: "", scopeId: "" } })`. From then on, that admin manages everyone else through the gated mutations.
...
```ts
import { expectPolicy } from "@concile/authz/testing";
...
| | Concile authz | OpenFGA / SpiceDB (ReBAC) | SQL row-level security | Plain function checks |
|---|---|---|---|---|
