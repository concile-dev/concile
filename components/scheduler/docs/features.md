...
|---|---|---|
| **`@concile/scheduler`** (this catalog's ①–④, ⑥–⑨) | one-off scheduling · crons · durable job runner · flow control | **one function invocation** per job, durable + retried |
| **`@concile/workflow`** (§5, later slice) | multi-step step-journaling durable execution | **many steps**, resumes mid-run; built on `requires: ["scheduler"]` |
...
## §5. Durable multi-step workflows  *(`@concile/workflow`, later slice — cataloged for completeness)*
...
- **Runs in `concile dev`** — local, no external broker; the heartbeat/driver is in-process at Tier 0.
