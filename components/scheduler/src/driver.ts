...
import { isConcileError } from "@concile/errors";
...
/**
 * `@concile/scheduler`'s driver — the event-driven loop that actually RUNS due jobs.
 *
...
export function schedulerDriver(): SchedulerDriver {
  ...
  // re-schedule work either. (Mirrors the same guard in `@concile/storage`'s reaper driver.)
  //
    ...
    do {
      ...
      for (const job of peeked.due) {
        ...
        try {
          ...
          // instead of burning every retry. A plain (non-Concile) error defaults to retryable, so
          // the scheduler's existing "retry unknown failures up to maxFailures" behavior is unchanged.
          result = { kind: "failed", error: String(e), retryable: isConcileError(e) ? e.retryable : true };
        }
