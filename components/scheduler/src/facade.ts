import type { ComponentContext, ActionApi } from "@concile/executor";
import { GuestDatabaseWriter } from "@concile/executor";
import type { JSONValue } from "@concile/values";

/**
 * A function reference as produced by codegen's `api`/`internal` proxy (see
 * `@concile/client`'s `FunctionReference`/`getFunctionPath`). Replicated here (rather than
 * depending on `@concile/client`, a client-facing SDK package) since a server component
...
/** Resolve a `fnRef` (string path or codegen ref) to its string path. Mirrors `@concile/client`'s `getFunctionPath`. */
...
export async function enqueueInternal(
  ...
  const jobId = await db.insert(
    ...
    compact({
      ...
      // (e.g. `@concile/workflow` threading a step's declared `maxAttempts` through here) and is
 ...
 * design (see `ActionCtx`'s doc comment in `@concile/executor`), and the scheduler never
