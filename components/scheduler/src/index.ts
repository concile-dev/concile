import { defineComponent, type ComponentDefinition, type BootContext } from "@concile/component";
...
/**
 * `defineScheduler()` — the `@concile/scheduler` component: the `jobs`/`job_args`/`crons`
 ...
 * and the `ContextProvider.write` opt-in on `@concile/executor`.
 *
