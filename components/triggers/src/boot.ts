...
/**
 * Boot-time work for `@concile/triggers`: handler-path validation (fail-fast) and cursor
...
export function validateHandlers(ctx: DriverContext, opts: TriggersOpts): void {
  if (!ctx.functionKind) {
    throw new Error(
      "@concile/triggers: the runtime's DriverContext does not provide functionKind — handler validation " +
        "cannot run. This means the composed runtime is older than @concile/component's triggers support; " +
        "upgrade @concile/runtime-embedded.",
    );
  }
  for (const [table, cfg] of Object.entries(opts)) {
    ...
    if (kind === undefined) {
      throw new Error(
        `@concile/triggers: trigger "${table}" references handler "${cfg.handler}", which is not a registered ` +
          `function. Check the path matches an exported mutation or action (e.g. "notifications:_onMessage").`,
      );
    }
    if (!isInternalPath(cfg.handler)) {
      throw new Error(
        `@concile/triggers: trigger "${table}"'s handler "${cfg.handler}" must be an internal function — a ` +
    ...
    if (kind !== "mutation" && kind !== "action") {
      throw new Error(
        `@concile/triggers: trigger "${table}"'s handler "${cfg.handler}" is a ${kind}, not a mutation or ` +
 ...
 * idiom — `DriverContext.readLog`'s doc comment, `@concile/component`).
 *
