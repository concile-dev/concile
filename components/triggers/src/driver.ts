import type { Driver, DriverContext, LogChange } from "@concile/component";
 ...
 * also a general defense-in-depth re-check, mirroring `@concile/scheduler`'s `SWEEP_MS`. A
...
/**
 * `@concile/triggers`' driver — one independent event loop PER configured trigger (design spec
 * D2), not a single shared loop: each trigger name gets its own `running`/`pendingWake`/`inFlight`
 * coalescing state (mirroring `@concile/scheduler`'s `schedulerDriver` shape, but keyed by
 ...
 *    `computeBackoff` (`@concile/scheduler`) — see `_recordFailure`'s doc comment (`./modules.ts`)
...
export function triggersDriver(opts: TriggersOpts): TriggersDriver {
  ...
  function iterate(name: string): Promise<void> {
    if (running.get(name)) {
      // Mirrors `@concile/scheduler`'s coalescing (see its `driver.ts` for the full rationale):
    ...
    const pass = runPass(name).finally(() => {
      ...
      // Closes a residual micro-window (mirrors `@concile/scheduler`'s `iterate()`): a wake can
   ...
   * returning with a stale view. Mirrors `@concile/scheduler`'s `runPass` shape exactly — this
  ...
  async function runOnePass(name: string): Promise<void> {
    ...
    while (cursorTs < targetBound) {
      ...
      try {
        ...
        // `@concile/scheduler`'s driver uses for a job's arbitrary `result.value` (see its
        // `driver.ts`).
  ...
  // Re-arms itself after every fire, mirroring `@concile/scheduler`'s `armSweep`/`sweepOnce`.
  ...
  return {
    ...
    start(c) {
      ...
      unsubscribeCommit = ctx.onCommit((inv) => {
        ...
        // `@concile/scheduler`'s onCommit filter, which DOES react to its own control-table
