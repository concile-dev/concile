...
/**
 * `defineTriggers(opts)` — the `@concile/triggers` component: react to committed data changes
 ...
 * receiving `{ changes: LogChange[] }` (`@concile/component`'s `LogChange` — see its doc
 * comment for `op`/`oldDoc`/`changeId` semantics). No `context`/`contextType` — unlike
 * `@concile/scheduler`, nothing calls a `ctx.triggers.*` facade from a mutation; the whole
