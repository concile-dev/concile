...
import type { QueryCtx, MutationCtx } from "@concile/executor";
import { computeBackoff } from "@concile/scheduler";

/**
 * Internal modules for `@concile/triggers` — registered on `defineTriggers()`'s `modules` map
 ...
 * name `"triggers/cursors"` (mirrors `@concile/scheduler`'s `./modules.ts` module doc comment).
 *
...
/** Drop `undefined`-valued keys before a `db.replace` (the wire codec rejects `undefined`). Mirrors `@concile/scheduler`'s `compact` (duplicated per that package's own established convention — see its `modules.ts`/`facade.ts`). */
 ...
 * determinism-for-OCC-replay property `@concile/scheduler`'s `_complete` relies on (see
 * `computeBackoff`'s doc comment, `@concile/scheduler`): a replay of this exact mutation call
