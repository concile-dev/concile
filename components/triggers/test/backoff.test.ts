// components/triggers/test/backoff.test.ts — retries back off via `@concile/scheduler`'s
...
import { computeBackoff, DEFAULT_BACKOFF_OPTIONS } from "@concile/scheduler";
import { makeRuntimeWithTriggers } from "./helpers";

describe("@concile/triggers — backoff", () => {
  it("computeBackoff's own delay bounds hold for the failureCount values the pause threshold walks through", () => {
    ...
    // calls (`../src/modules.ts`) is the REAL `@concile/scheduler` export, not a local
    ...
    // (0.5 + 0.5*rng()) = 0.75 at rng()=0.5 (`computeBackoff`'s doc comment, `@concile/scheduler`) = 750.
