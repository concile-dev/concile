...
import type { FnRef, SchedulerContext } from "@concile/scheduler";
import { getFunctionPath } from "@concile/scheduler";
import { sendEventImpl } from "./events";

export type { FnRef } from "@concile/scheduler";
 ...
 * `@concile/scheduler`'s `schedulerContext`) — without it, `cctx.db` is read-only and `start`'s
...
export function workflowContext(cctx: ComponentContext): WorkflowContext {
  ...
  return {
    ...
    async cancel(runId, opts) {
      ...
      // scheduler's own `cancel()` (`@concile/scheduler`'s `schedulerContext`) further cascades to
 ...
 * `api.runMutation`, exactly like `@concile/scheduler`'s `schedulerActionContext` delegates to
