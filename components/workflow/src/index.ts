...
/**
 * `defineWorkflow({ workflows })` — the `@concile/workflow` component: the `workflows`/`steps`/
 ...
 *
 * `contextWrite: true` is load-bearing the same way it is for `@concile/scheduler`: it's what
...
export function defineWorkflow(opts: { workflows: WorkflowRegistry; maxParallelism?: number }): ComponentDefinition {
  return defineComponent({
    ...
    contextType: { import: "@concile/workflow", type: "WorkflowContext" },
    contextWrite: true,
    ...
    // mirroring `@concile/scheduler`'s `schedulerActionContext` — see `workflowActionContext`'s
