...
import type { QueryCtx, RegisteredFunction } from "@concile/executor";
import type { JSONValue } from "@concile/values";
import type { OnCompleteResult, SchedulerContext } from "@concile/scheduler";
 ...
 * (`@concile/scheduler`'s `facade.ts`/`modules.ts`) already uses before a `db.insert`/`replace`;
 ...
 * like `@concile/scheduler`'s own `_cronTick`. Privileged mode bypasses namespace prefixing
...
/** Drop `undefined`-valued keys — the wire codec (`convexToJson`) rejects `undefined`; omit rather than null it out. Mirrors `@concile/scheduler`'s `compact` (not exported, so replicated here). */
 ...
 * analog of `@concile/scheduler`'s `fireOnComplete` (`components/scheduler/src/facade.ts`).
 ...
 * see `fireOnComplete` in `@concile/scheduler`'s `facade.ts` for that shape's origin.
 *
 ...
 * Mirrors `@concile/scheduler`'s `_enqueue`/`_cancel` (`components/scheduler/src/modules.ts`)
