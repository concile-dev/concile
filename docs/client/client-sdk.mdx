...
frames are dropped, not held: the transport announces the close, and recovery moves up a level. When
a socket finally does open, the client runs the same reopen sequence (`SetAuth` replay, resubscribe,
flush unsent mutations) as a mid-session reconnect, rebuilding everything from its own state rather
than from replayed raw frames.
