...
Think of `@concile/client` as the framework-agnostic engine that powers the reactive sync protocol behind your app. It includes a transport layer, a `ConcileClient` built right on top of it, and a set of handy hooks (via `@concile/client/react`) that subscribe your components to queries and automatically re-render them whenever new data arrives.

In this guide, we will walk you through everything you need to know about the client. We cover how to build it, how transports work, and every single `ConcileClient` method. We also dive into React hooks, the wire lifecycle, authentication, typed codegen, and how to handle errors. If you are curious about the two biggest features we built on this client, check out our deep dives on [Optimistic updates](/docs/client/optimistic-updates) and [Offline sync](/docs/client/offline-sync).
...
`useConcileClient()` reads it back out. It throws if called outside a `ConcileProvider` (every
...
`ConcileClient` (with a fresh transport) rather than reusing a closed instance.
