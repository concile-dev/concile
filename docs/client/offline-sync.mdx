...
While [Optimistic updates](/docs/client/optimistic-updates) and the transport's [automatic reconnect](/docs/client/client-sdk#reconnection-in-detail) do a great job handling connections that drop for a few seconds, they don't help much if you reload the page.

If you close your laptop mid flight or crash your browser tab, any mutations that were still `unsent` (queued while offline) or `inflight` (sent, but waiting on a response) are just gone by default. There is no backup, and you never really find out what happened to them.

That is where the Receipted Outbox steps in to save the day. It is an **opt-in** layer you can add to the reactive client. With it, any mutations you make offline are saved securely. They can survive a full reload or crash and will automatically drain in the right order once you are back online. Plus, the server keeps a personalized receipt for every single one, so you never have to worry about accidentally applying the same change twice.
...
It keeps everything in one IndexedDB database (`concile-outbox`), on purpose: the shared
...
instead backs off (the same jittered-exponential shape `@concile/scheduler` uses for its own
...
Concile has **no merge and no CRDT layer**, offline or online. A queued mutation, when it
...
- [Client SDK](/docs/client/client-sdk): constructing `ConcileClient`, transports, reconnection,
