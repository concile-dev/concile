...
**one durable queue**, though, and a [Web
Locks](https://developer.mozilla.org/en-US/docs/Web/API/Web_Locks_API)-elected leader tab drains the
**entire shared queue**: every tab's entries under their own recorded identity, not just the
leader's own writes.
...
([`mintId`](#client-supplied-ids-create-then-reference-chains)): mint the id before the mutation is
sent, use it as the placeholder row's `_id`, and have the updater check whether a row with that id
is already present before inserting:
...
re-executes a committed mutation, this means either an astronomical collision or an app bug, never a
silent merge).
