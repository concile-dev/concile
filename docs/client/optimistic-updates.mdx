...
When you run a mutation, it takes a quick round trip. Your function executes on the server, saves the changes, and sends the result back through the WebSocket. Even though this only takes a few milliseconds, it can feel a bit sluggish if you're waiting for a chat message to send or a task to be checked off.

That is where `withOptimisticUpdate` comes in to save the day. It lets you skip the waiting game. You can show the expected result the very instant your user clicks. Then, it seamlessly swaps in the actual saved data without any annoying flickering. The moment your temporary guess goes away, the real data takes its place on the exact same frame.

In this guide, we will walk you through the entire v1 setup. We cover `useMutation(...).withOptimisticUpdate(...)`, the `OptimisticLocalStore` that your updater works with, and the behind the scenes magic that guarantees a smooth swap. Just keep in mind that we don't use a `defineLocalMutators` registry, pending or failed queue accessors, or optimistic support for actions since actions don't have a commit timestamp to help manage the layers. You can learn more about that in [Actions](/docs/core-concepts/actions).
