...
When you run a mutation, it takes a quick round trip. Your function executes on the server, saves
the changes, and sends the result back through the WebSocket. Even though this only takes a few
milliseconds, it can feel a bit sluggish if you're waiting for a chat message to send or a task to
be checked off.

That is where `withOptimisticUpdate` comes in to save the day. It lets you skip the waiting game.
You can show the expected result the very instant your user clicks. Then, it seamlessly swaps in the
actual saved data without any annoying flickering. The moment your temporary guess goes away, the
real data takes its place on the exact same frame.

In this guide, we will walk you through the entire v1 setup. We cover
`useMutation(...).withOptimisticUpdate(...)`, the `OptimisticLocalStore` that your updater works
with, and the behind the scenes magic that guarantees a smooth swap. Just keep in mind that we don't
use a `defineLocalMutators` registry, pending or failed queue accessors, or optimistic support for
actions since actions don't have a commit timestamp to help manage the layers. You can learn more
about that in [Actions](/docs/core-concepts/actions).
...
import type { OptimisticLocalStore } from "@concile/client";
...
optimistic updates. It's also what gives the server itself return-value validation (see [Argument
validation](/docs/core-concepts/mutations#argument-validation)).
...
- **`placeholderId(table)`** returns the same id every time this mutation's updater replays. Call it
  ...
  the hood it's built as `` `${entryEntropy}:${table}:${callOrdinal}` ``. Treat the exact shape as
  an opaque, unspecified string and never parse it.
...
Use `store.placeholderId(table)` and `store.now()` instead, always. Either impure call mints a fresh
value on every replay. React, keying off it, sees a brand-new row instead of the same optimistic row
re-rendering: remounts, list-key churn, and lost input focus, on every unrelated server push while
your mutation is pending.
...
codec, and it isn't a real row. It exists only so your optimistically-inserted object has some
stable `_id` to key a React list on, until the real row arrives and replaces it in the same atomic
swap the reconciliation contract below describes.
...
app's generated `_generated/ids.ts`. See [Client-supplied
ids](/docs/client/offline-sync#client-supplied-ids) in the offline guide.
...
exactly what the freeze itself is trying to remind you to do. It's gated on `process.env.NODE_ENV
!== "production"`, so it costs nothing at runtime in a production build.
...
snaps back to exactly what it was before, never an inverse `-1` applied on top of whatever it
happens to be now.
...
below. If you're porting code that relies on await-then-local-cache-read semantics, the [migration
guide](/docs/reference/migrate-from-convex) covers the difference.
...
- **Echo-snap on a wrong guess.** If your updater's predicted value differs from what the server
  ...
  This is inherent to optimism over an arbitrary server-side mutation: the more of the server's
  logic your updater predicts, the smaller the snap. The demo app has a dedicated wrong-guess toggle
  that guesses `+2` while the server commits `+1`, so you can watch this snap happen on purpose.
...
was published. The client, seeing its observed frontier advance to that commit's timestamp,
correctly dropped the now-`completed` optimistic layer, but the base it adopted didn't yet contain
the write. The result: a genuine flicker, the optimistic row vanishing for one frame before
reappearing on a later push (and, with no further writes to trigger that later push, in the worst
case never reappearing at all).
...
point, so by the time a cross-shard subscription's re-run is triggered, every oracle involved
...
For durability across a reload or crash, not just a brief reconnect, see [Offline
sync](/docs/client/offline-sync), which layers a durable outbox and cross-tab live rendering on top
