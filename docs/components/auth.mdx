...
`@concile/auth` is our very own first-party authentication component, ready to go right out of the box. It sets you up with email and password accounts, anonymous sign-in options that you can easily upgrade later, and a seriously tough session model. From there, you can mix and match whatever else you need, like email verification, password resets, magic links, one-time codes, social OAuth logins, third-party JWT or OIDC, two-factor TOTP, and even passkeys.

No matter how a user logs in, the journey always wraps up the same way by creating a real session row right in your database. That exact row is what `ctx.auth.getUserId()` checks inside the transaction, just like any normal read. Because of this, it joins the calling function's [read set](/docs/get-started/how-it-works#read-sets-what-a-query-actually-records). If you ever need to revoke a session, it's as simple as making a write to that row. Any subscribed query relying on that identity will rerun and update instantly, meaning no polling or messing with separate invalidation channels.

Everything beyond basic password sign-in is totally optional, tucked away in its own config block. `defineAuth()` with
...
- **Cross-tab broadcast.** After a winning refresh, the new pair is posted over a
  `BroadcastChannel` (`concile:auth:pair`) so every other tab adopts it immediately rather than
...
password sign-in uses. Concile's own sessions always stay DB rows: an external identity is
...
mints a Concile session through the exact same core the OAuth callback uses.

This is an exchange model, not per-request verification. Concile verifies the token once and
...
When `@concile/notifications` is composed alongside `auth`, every A2 email send (OTP,
...
Concile is WebSocket-first: identity flows over the `SetAuth` message, not request headers. So
