...
Let's say your `documents` table needs a few different levels of access. Maybe some folks can read absolutely everything, while others can only see their team's documents, and a few can only look at things that were shared with them directly. Instead of painfully writing permission checks by hand for every single query and mutation, `@concile/authz` lets you handle all of this smoothly.

It hooks you up with three great tools: role-based access control (RBAC) complete with inheritance and scoped grants, relationship-based access control (ReBAC) for those sharing-style permissions, and declarative row policies that automatically filter what a query shows and tightly control what a mutation can write.
