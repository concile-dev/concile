...
Think of a notification as just another row in a table. `@concile/notifications` is a handy opt-in component that fires off messages across four different channels: **email**, **SMS/WhatsApp**, an in-app **inbox**, and mobile or web **push**. Each channel works with a swappable provider adapter, so you are never locked into relying on just one vendor.

Its real standout feature is the **reactive in-app inbox**. Because a notification is just a row in a live-queried table, a user's inbox and unread count update instantly. That means you don't need a dedicated realtime service, you don't have to deal with polling, and there's no separate push infrastructure to maintain.

Once you get how `email` works, you pretty much understand the rest. Every channel shares the exact same send path, preference gate, retry and backoff systems, dead-letter machine, and topic fan-out mechanism. There is no separate push or SMS system to figure out since the differences between channels just come down to a few small branches inside the shared code.
...
ambient caller token (the fallback identity `callerId` uses when `@concile/auth` isn't composed).
...
With `@concile/auth` composed (or an upstream token-verifying proxy), the resolved id is
...
you write, or a composed component like `@concile/auth`, see
...
Point your provider's delivery webhook (Resend, Twilio) at your deployment and Concile verifies
...
`https://…` URL configured in its console, but Concile serves plain HTTP behind your
...
Compose **both** `@concile/auth` and `@concile/notifications` in the same
`concile.config.ts`, and auth's own transactional emails (verification, password reset, magic
...
- **Graceful fallback.** `@concile/auth` does not depend on `@concile/notifications` as a
