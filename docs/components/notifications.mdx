...
Think of a notification as just another row in a table. `@concile/notifications` is a handy opt-in
component that fires off messages across four different channels: **email**, **SMS/WhatsApp**, an
in-app **inbox**, and mobile or web **push**. Each channel works with a swappable provider adapter,
so you are never locked into relying on just one vendor.

Its real standout feature is the **reactive in-app inbox**. Because a notification is just a row in
a live-queried table, a user's inbox and unread count update instantly. That means you don't need a
dedicated realtime service, you don't have to deal with polling, and there's no separate push
infrastructure to maintain.

Once you get how `email` works, you pretty much understand the rest. Every channel shares the exact
same send path, preference gate, retry and backoff systems, dead-letter machine, and topic fan-out
mechanism. There is no separate push or SMS system to figure out since the differences between
channels just come down to a few small branches inside the shared code.
...
- [Email digest](#email-digest), [Auth unification](#auth-unification), [the
  driver](#how-delivery-actually-runs-the-driver), [schema reference](#schema-reference)
...
component never hard-codes a vendor, so you can write your own adapter against a documented contract
instead of waiting on a built-in one.

## Enabling it

Compose the component in `concile.config.ts`, like any other opt-in component:
...
different, send-focused surface (`send`, `sendNow`, `sendToTopic`, and nothing else). See [Mutation
vs. action surface](#mutation-vs-action-surface) below.

## Sending

`ctx.notifications.send(args)` is available on every mutation context. It writes directly through
the **calling mutation's own transaction**: an enqueue rolls back if the mutation does, and it fans
out reactively to any live subscription (like the in-app inbox) the instant the mutation commits:

```ts title="concile/users.ts"
...
- **`suppressed`**: channels skipped by the recipient's own preferences (see
  [Preferences](#preferences-and-critical-categories)).
- **`deferred`**: channels buffered into an email digest instead of sent immediately (see [Email
  digest](#email-digest)).
...
- **`to`** is addressed per channel and resolved per channel at send time: `email` requires
  ...
  a *recipient identity*, not a contact method). Sending `["email","push"]` to a `to` missing either
...
- **`template`** is either a **registered key** looked up in the channel's `templates` map (as in
  ...
  registration: `{ email: { subject, text, html? }, sms: "…", in_app: { title, body }, push: {
  title, body, data? } }`. Only the fields you actually use need to be present. Omit an optional
  field rather than setting it to `undefined`: a value that's explicitly `undefined` is rejected
  when the row is written.
- **`data`** is the payload handed to a registered template function (`(data) => Content`). Template
  rendering is pure and runs *inside* the transaction. There's no I/O, so it's safe to run before
...
replayed (an OCC conflict retry, a client retrying a timed-out call). This is the pattern that makes
an OTP or a receipt email safe:
...
`idempotencyKey`, written atomically with the message rows). No second send, no second row. When the
underlying provider supports a native idempotency header (Resend, and any custom adapter that honors
`EmailMessage.idempotencyKey`), the key passes through to it too, as defense in depth.
...
`unregisterPushToken`, `identity()`) exists only on the mutation facade. Calling one of them from an
action is a runtime TypeError, not a silent no-op. Do that work in a mutation the action invokes via
`ctx.runMutation` instead. `ctx.notifications.identity()` (mutation facade) returns the raw ambient
caller token (the fallback identity `callerId` uses when `@concile/auth` isn't composed). Most app
code never needs it directly.

#### `sendNow`: synchronous delivery from an action

```ts title="concile/blast.ts"
...
```ts
import type { EmailProvider, EmailMessage, SendResult } from "@concile/notifications";
import { NotificationSendError } from "@concile/notifications";
...
(`to`/`from`/`body`/`kind?`); `PushProvider` is covered in [Writing your own push
provider](#writing-your-own-push-provider).

## Multi-provider fallback

An email or SMS channel can list additional providers tried, in order, after the primary fails. All
of this happens within the **same** delivery attempt, with no extra retry/backoff round-trip:
...
**Webhooks with multiple providers.** The delivery-webhook route is unchanged and still keyed by *channel*,
not provider (`POST /api/notifications/webhooks/email`). No vendor-dashboard callback URL needs to
change when you add a fallback.
...
receives the channel-level `webhookSecret` from config. A fallback provider is expected to carry its
own signing material inside its own factory args (the same way `twilioSms({ accountSid, authToken
})` already does), not share the primary's secret.
...
```ts title="concile.config.ts"
import { defineConfig } from "@concile/component";
import { defineNotifications, expoPush, fcmPush, apnsPush } from "@concile/notifications";
...
```ts
import { registerForPush, unregisterForPush } from "@concile/client/react";
...
Possession of a device token is authority over its routing. Treat a device token as a device-local
secret, not something safe to log or expose.
...
user's iPhone (APNs), Android phone (FCM), and an Expo-managed dev client all at once,
transparently.
...
Same small `send`-only shape, fanned out per **provider group** (not per token). The component calls
`send` once per configured provider with every token routed to it in that call's `to` array:

```ts
import type { PushProvider, PushMessage, PushSendResult } from "@concile/notifications";
...
```tsx
import { useNotifications } from "@concile/client/react";
...
Both `notifications` and `unreadCount` are reactive. A new send or a `markRead` from any tab updates
every subscribed component with no manual refetch. `undefined` first-frame results coalesce to
`[]`/`0`, so you never need to branch on a loading sentinel.
...
With `@concile/auth` composed (or an upstream token-verifying proxy), the resolved id is trustworthy
and isolation is enforced. Without either, the identity falls back to the raw `setAuth(...)` bearer
token, which an unauthenticated client can set to any value it likes. Compose
[Auth](/docs/components/auth) (or verify the token upstream) before relying on inbox isolation in
production.
...
configured otherwise). A user can opt a `(category, channel)` pair out. A suppressed send is skipped **before**
it ever writes a queued row or reaches a provider.

**Default-allow.** With no preference row at all, every category/channel is enabled. Preferences are
opt-*out*, not opt-*in*. Resolution is most-specific-wins: a channel-specific row for a category
overrides a category-wide row (channel omitted) for the same category, which overrides the
allow-by-default fallback. This all happens at a single chokepoint (`recordSend`), so there's no
second gate anywhere else to keep in sync.
...
you write, or a composed component like `@concile/auth`, see [Auth unification](#auth-unification)).
Never forward it straight from client input: it's exactly the same trust boundary as `to`.
...
```tsx
import { useNotificationPreferences } from "@concile/client/react";
...
are **strictly self-only**: there's no way for a client to pass a `userId` and subscribe a different
user (that would be an IDOR). Both are idempotent. Subscribing twice, or unsubscribing when not
...
`ctx.notifications.sendToTopic(args)` fans a send out to every current subscriber of a topic. It's **action-only**
(paging through an arbitrarily large subscriber list is bulk work, same reasoning as any other bulk
read from an action):
...
- **Paginated internally.** An arbitrarily large subscriber list is processed in bounded pages under
  ...
  transactions, a subscribe/unsubscribe landing mid-broadcast may include or skip that one
  subscriber. A keyed broadcast stays safe against a double either way.

## Delivery reliability

Email/SMS/push sends are automatically retried on a transient failure, and a crashed in-flight send
is recovered. Once you wire a provider's delivery webhook, the message row also picks up a second,
independent status axis as the provider reports what happened after the send. **This section applies
only to `email`/`sms`/`push`.** `in_app` has no queue or send step (it's written `sent`
synchronously in your transaction), so there's nothing to retry, reclaim, or hear back about.
...
- A **plain `Error`** (or any throw that isn't a `NotificationSendError`) is treated as **retryable**:
  a transient 5xx/network blip. The row goes back to `queued` with a backed-off `nextAttemptAt` and
  its `attempts` count incremented.
...
`sending` row older than `reclaimLeaseMs` (default `60000`) back to `queued`, counting an attempt as
it does, so a row that keeps crashing still eventually dead-letters instead of looping forever:
...
Point your provider's delivery webhook (Resend, Twilio) at your deployment and Concile verifies its
signature, then reactively updates the message row with a normalized `deliveryStatus` as the
...
verified, well-formed batch is applied event-by-event via an internal mutation, and the route
...
`https://…` URL configured in its console, but Concile serves plain HTTP behind your TLS-terminating
reverse proxy. The route reconstructs the public URL from `X-Forwarded-Proto`/ `X-Forwarded-Host`
(the common proxy default) before verifying, falling back to the raw request URL if neither header
is set (direct exposure or local dev). Forging these headers can never make verification *pass* (the
HMAC still needs the real secret). It can only make an attacker's own forged callback fail.
Resend/Svix signs the request body, not the URL, so it's unaffected either way.
...
`deliveryStatus: "delivered"`, then `"opened"`, then `"clicked"` as webhooks arrive. It's written **monotonically**
by lifecycle rank (`dropped` < `bounced`/`complained`/`failed_permanent` < `delivered` < `opened` <
`clicked`), so a redelivered or out-of-order webhook event is a no-op rather than regressing a later
status back to an earlier one. Because it's an ordinary field on the `messages` row, subscribing to
a query over it sees the update reactively, with no polling.
...
- **Per-group transaction isolation.** Each `(recipient, category)` group's flush runs in its **own**
  ...
  every *other* queued notification (including critical auth OTPs) on the node. A failed flush's
  items stay un-flushed and retry on the next pass, never claimed-but-dropped.
...
Compose **both** `@concile/auth` and `@concile/notifications` in the same `concile.config.ts`, and
auth's own transactional emails (verification, password reset, magic link, OTP) automatically route
through the notifications delivery path instead of auth's standalone email provider. There's no
extra wiring:
...
- **Graceful fallback.** `@concile/auth` does not depend on `@concile/notifications` as a package.
  It duck-types a minimal `ctx.notifications` shape at runtime. If notifications isn't composed in
  your project, auth silently falls back to its own standalone `EmailProvider`, byte-identical to
  how it behaved before this feature existed. Composing (or later removing) notifications is purely
  additive.
...
- **Digest scope**: SMS/in_app digest (email-only today), per-user digest frequency (a fixed
  ...
  than purely time-windowed), and a crash-orphan digest reaper (a
...
- **A markup/visual template registry** (Liquid/MJML-style). Inline typed template functions are the
  v1 authoring model.
- **Push rich payloads, engagement receipts, web push, and per-device results**: see [Push
  non-goals](#push-non-goals).
