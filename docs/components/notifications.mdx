...
*channel*, not provider (`POST /api/notifications/webhooks/email`). No vendor-dashboard callback URL
needs to change when you add a fallback.
...
configured otherwise). A user can opt a `(category, channel)` pair out. A suppressed send is skipped
...
rather than regressing a later status back to an earlier one. Because it's an ordinary field on the
`messages` row, subscribing to a query over it sees the update reactively, with no polling.
...
- **Per-group transaction isolation.** Each `(recipient, category)` group's flush runs in its
  **own** transaction. If your `digestTemplate` throws for one recipient's item shape, only that
  ...
  delivery of every *other* queued notification (including critical auth OTPs) on the node. A failed
  flush's items stay un-flushed and retry on the next pass, never claimed-but-dropped.
