...
The core engine (schema, queries, mutations, actions, reactivity) doesn't really know about auth, notifications, or workflows, and honestly, it doesn't need to. Think of these capabilities as components, which are basically self-contained packages that plug right into the engine using a single seam called `ComponentDefinition`. Each one chips in with its own mix of tables, a handy `ctx.<name>` facade, background behavior, and whatever HTTP routes it might need.

When you're setting up a project, you get to handpick exactly the components you want in your `concile.config.ts` file. We don't install anything for you behind your back, and absolutely nothing runs unless you specifically ask for it.
...
(Adapted from `examples/auth-demo/concile.config.ts`, the reference pattern real projects copy
from. `examples/chat/concile.config.ts` shows the smallest possible case: a single
...
project. `packages/cli/src/load-config.ts` just imports `concile.config.ts` (or
`concile.config.js`, whichever exists) and reads `.components` off the default export. If neither
...
true`, as `@concile/scheduler`, `@concile/workflow`, and `@concile/notifications` do. `boot`
...
whichever subset it needs: `@concile/authz` composes tables, a facade, policies, and a boot step;
...
from `concile.config.ts`, and baked into the running deployment's table numbers, module map,
context providers, and drivers.

Adding `defineNotifications(...)` to your config, or removing `defineTriggers(...)`, requires
restarting the process (`concile serve` again, or a redeploy of a `concile build` binary).
...
- **`functionKind?(path)`** *(optional)*: resolves a registered path's real kind
  ...
  at start time (`@concile/triggers`' `handler` option) and fail with an instructive "unknown path"
...
opt-in. Each is composed by name in `concile.config.ts`, absent from a deployment that doesn't list
