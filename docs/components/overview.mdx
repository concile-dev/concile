---
title: Components
description: Opt-in capabilities composed in concile.config.ts.
---

{/* diataxis: explanation */}

The core engine (schema, queries, mutations, actions, reactivity) doesn't really know about auth,
notifications, or workflows, and honestly, it doesn't need to. Think of these capabilities as
components, which are basically self-contained packages that plug right into the engine using a
single seam called `ComponentDefinition`. Each one chips in with its own mix of tables, a handy
`ctx.<name>` facade, background behavior, and whatever HTTP routes it might need.

When you're setting up a project, you get to handpick exactly the components you want in your
`concile.config.ts` file. We don't install anything for you behind your back, and absolutely nothing
runs unless you specifically ask for it.
...
```mermaid
flowchart LR
  Config["concile.config.ts"] --> Compose["composeComponents\n(topological sort by requires)"]
...
You declare which components are active in a single `concile.config.ts` at your project root,
...
(Adapted from `examples/auth-demo/concile.config.ts`, the reference pattern real projects copy from.
`examples/chat/concile.config.ts` shows the smallest possible case: a single `defineTriggers({
messages: { handler: "audit:_onChange" } })` for a table audit log.)

That's the entire surface. There is no separate CLI command that "installs" a component into a
project. `packages/cli/src/load-config.ts` just imports `concile.config.ts` (or `concile.config.js`,
whichever exists) and reads `.components` off the default export. If neither file is present, the
project composes zero components (`{ components: [] }`) and behaves as pure core engine.
...
configuration `concile deploy` reads (see [Deploy & build](/docs/deploy/deploy-and-build)).
`@concile/component` also exports an `env(name, fallback?)` helper for referencing environment
...
|---|---|---|---|---|---|
| Auth | `@concile/auth` | (none) | `ctx.auth` | (none) | `/api/auth/oauth/` (if `oauth` configured) |
| Authorization | `@concile/authz` | `auth` | `ctx.authz` | (none) | (none) |
| Scheduling | `@concile/scheduler` | (none) | `ctx.scheduler` | dispatch loop | (none) |
| Workflows | `@concile/workflow` | `scheduler` | `ctx.workflow` | (via scheduler) | (none) |
| Triggers | `@concile/triggers` | (none) | (none) | log-cursor loop | (none) |
| Notifications | `@concile/notifications` | (none) | `ctx.notifications` | delivery sweep | `/api/notifications/webhooks/` (if a webhook provider is configured) |
...
- **[Auth](/docs/components/auth)** (`defineAuth`): password and hardened session authentication.
  ...
  (Google, GitHub, Microsoft, Discord, Facebook, Apple built in). `ctx.auth.getUserId()` resolves
  the caller's session inside the transaction, participating in the read set like any other read, so
  a revoked session reactively invalidates whatever it was gating.
...
- **[Notifications](/docs/components/notifications)** (`defineNotifications`): a `Channel ×
  ...
  (`useNotifications`/`<Inbox>`) on the client. `ctx.notifications.send(...)` is the single
  chokepoint every send routes through, including auth's own verification/reset/magic-link emails
  when both are composed.
...
true`, as `@concile/scheduler`, `@concile/workflow`, and `@concile/notifications` do. `boot` runs
one-time startup work, `driver` runs a recurring background loop, `httpRoutes` mounts reserved-path
endpoints, and `policies` attaches row-level authorization rules. A component uses whichever subset
it needs: `@concile/authz` composes tables, a facade, policies, and a boot step;
`@concile/scheduler` adds a driver, an action-mode facade, and a codegen re-export on top.

For the full field-by-field walkthrough of every seam, with a worked example, see [Build a custom
component](/docs/contributing/extending/custom-component).
...
- Not `"app"` and not starting with `_`. Both are reserved: `"app"` is the implicit name for your
  own bare-namespace code, and `_`-prefixed is the convention for internal, non-user-callable paths.
...
`concile deploy` (see [Deploy & build](/docs/deploy/deploy-and-build)) can push a new version of
your app's own functions and an additive schema change onto an already-running `concile serve`,
atomically, with no restart. But the *set* of composed components is decided once, at process boot,
from `concile.config.ts`, and baked into the running deployment's table numbers, module map, context
providers, and drivers.

Adding `defineNotifications(...)` to your config, or removing `defineTriggers(...)`, requires
restarting the process (`concile serve` again, or a redeploy of a `concile build` binary). There's
no live "activate this component" admin call.

</Callout>

This mirrors the same boundary `concile deploy`'s additive-schema gate draws for your own
...
request: the scheduler's job-dispatch loop, `@concile/triggers`' cursor over the MVCC log,
`@concile/notifications`' delivery sweep and retry/backoff loop.
...
- **`setTimer(atMs, cb)` / `clearTimer(handle)`**: arms a wake at an *absolute* wall-clock instant,
  ...
  can disagree (a process that just resumed from hibernation), and it would silently restart the
...
- **`readLog({ afterTs, tables?, limit? })`**: reads committed changes from the MVCC log after a
  timestamp, in ascending order. This is the durable change-feed seam `@concile/triggers` is built
  on. `limit` bounds scanned revisions, not matched ones, so a cursor makes progress even over a
  quiet watched table on a busy log. The returned `maxScannedTs` (not the last change's own `ts`) is
  what a cursor should advance to, so scanned-but-unmatched ranges are never rescanned. `limit: 0`
  peeks the current stable bound at O(1) cost with no scan at all: how a brand-new trigger seeds its
...
- **`functionKind?(path)`** *(optional)*: resolves a registered path's real kind
  ...
  path at start time (`@concile/triggers`' `handler` option) and fail with an instructive "unknown
  path" or "wrong kind" error before ever calling it, rather than crashing on the first commit.
...
1. Must live under a reserved namespace. `pathPrefix` must start with `/api/` or `/_`. Your app's
   own `http.ts` mounts everywhere else, so a component route can never collide with an app route.
...
`@concile/auth` mounts `GET`/`POST /api/auth/oauth/` (GET for query-mode OAuth callbacks, POST for
Apple's `form_post` callback: same handler, two methods, no overlap since they're keyed per method).
`@concile/notifications` mounts `POST /api/notifications/webhooks/` when a webhook-capable provider
is configured.
...
`concile.config.ts` composes. Every deployment gets
`ctx.storage.generateUploadUrl`/`getUrl`/`getMetadata`/`delete`/`store`/`get`, with no entry in
`concile.config.ts` and no way to opt out.
...
There's no `defineStorage()` to add or remove. See [File storage](/docs/core-concepts/file-storage)
for its API.

</Callout>

Everything else described on this page (auth, authz, scheduler, workflow, triggers, notifications)
is opt-in. Each is composed by name in `concile.config.ts`, absent from a deployment that doesn't
list it, with zero tables, zero facades, and zero background drivers running for a capability you
never composed.
