...
Think of `@concile/scheduler` as cron that is built right into your database, rather than something awkwardly bolted on the side. A scheduled job is literally just a row in a `jobs` table. Whether you want to run it later, just once, at a very specific time, or on a regular schedule, it works exactly like any other write in concile: it is durable, and there is absolutely nothing polling in the background.

A background driver takes care of dispatching the work. It wakes up instantly on every commit and keeps a wall-clock timer set for the very next pending job. This means you will never lose a job to a crash right after claiming it but before finishing it.

It also serves as the rock-solid foundation for two other components. [`@concile/workflow`](/docs/components/workflows) routes every single step through the scheduler's job queue, while both `@concile/triggers` and `@concile/notifications` lean on the same recurring driver system for their own background loops.
...
path (which is how `@concile/workflow` threads a step's declared `maxAttempts` through) is your
...
`@concile/workflow` cancels its step jobs directly via `scheduler.cancel` rather than chaining
