...
Think of `@concile/scheduler` as cron built into your database, not bolted on beside it. A
scheduled job is a row in a `jobs` table. Running it later, once, at an exact time, or on a
recurring cadence, works the same as any other write in concile: durably, with nothing polling in
the background.
...
It's the foundation two other components build on. [`@concile/workflow`](/docs/components/workflows)
dispatches every step through the scheduler's own job queue, and both `@concile/triggers` and
`@concile/notifications` reuse the same recurring-driver seam for their own background loops.
