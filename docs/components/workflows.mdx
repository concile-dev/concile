...
Sometimes jobs just take more than one step. You might need to reserve stock, charge a card, and then mark the order as fulfilled, maybe even taking a pause or waiting for an external signal in the middle. `@concile/workflow` handles that entire sequence for you, dependably and durably.

A workflow easily survives a process restart because it is not just floating in memory. The result of every step gets logged into a durable table, and concile actually **replays the whole handler from the very beginning** every time the run moves forward. If a step already succeeded, it resolves instantly by checking the journal, meaning only a genuinely new step ends up doing any real work.

It relies on [`@concile/scheduler`](/docs/components/scheduling) (`requires: ["scheduler"]`) rather than replacing it. Because every step goes through the scheduler's own job queue, it gets to inherit all of the scheduler's great features like retries, backoffs, dead lettering, and cascading cancels.
...
`@concile/workflow` requires `@concile/scheduler` to be composed first (or at least alongside
...
`ctx.runMutation`, mirroring exactly how `@concile/scheduler`'s own action-mode facade delegates
