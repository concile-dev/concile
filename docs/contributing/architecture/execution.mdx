...
1. **Checking arguments on the way in.** If you declare a function with an `args` validator (like
   `query({ args: { ... }, handler })`), we check the incoming arguments against that validator
   *before* your handler even gets to run. If things don't match up, we throw an
   `ArgumentValidationError` that points out exactly which field messed up and why. The only
   exception here is `httpAction`s, which skip this step because they take a raw `Request` instead
