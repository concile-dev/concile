...
Everything in Concile, queries, mutations, reactivity, the dashboard's data browser, ultimately
...
up. No prior Concile knowledge assumed.
...
Concile's storage layer does something different: it **never overwrites**. Every write, insert,
...
This one rule is the whole of **snapshot isolation** in Concile: the same scan, run again later
...
product. That narrowness is deliberate. It's what lets Concile claim **deploy anywhere** without
...
Concile ships two database backends today, plus a Cloudflare-native adapter, and the two
...
`MAX(ts) + 1` computed inside its own transaction. Because Concile enforces a single writer at a
...
`nextval('concile_ts')` inside the same transaction as the row inserts, so the timestamp becomes
...
`CONCILE_DATABASE_URL`, with no code changes and no migrations, since the physical schema (next
...
This is also why Concile never needs a migration step as your app's `schema.ts` evolves: there's
...
Table numbers are partitioned by range: **1 to 9999 are reserved for Concile's own internal
