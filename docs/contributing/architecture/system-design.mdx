...
This page is the map. It explains the one idea that everything else in Concile's engine is built around, and how that one idea lets a tiny single-binary app and a large distributed one share the exact same code.
...
Concile's answer is not to pick a lane. It makes both **the same system at two different operating points**, connected by a handful of stable internal interfaces. Growing from one to the other is a deployment and configuration change, never a rewrite of your functions.
...
Concile functions come in three flavors, and the difference between them is entirely about determinism:
