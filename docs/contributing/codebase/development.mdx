...
This page is for people working on concile's own codebase, not people building an app with
...
- **Bun >= 1.2.** Bun is both the package manager and the runtime for the day-to-day workflow
  (`bun install`, `bun run build`, `concile dev`). Install it from [bun.sh](https://bun.sh) if
  you don't have it.
- **Node >= 22** is a fully-supported *target* for the published packages. End users can run
  concile on Node, but as a contributor you develop and run the tooling on Bun.

<Callout title="Why Bun for development but Node as a target?">
  Concile's engine is written to be runtime-agnostic behind small seams (like the storage
...
When package A depends on package B (say, `@concile/cli` depends on `@concile/values`), A's
...
`bun run --filter @concile/cli test`, your edit is invisible. Nothing breaks, nothing updates.
The test just silently keeps running against whatever was in `dist/` before your edit. You have
to `bun run build` (or `bun run --filter @concile/values build`) first to refresh `dist/`.
...
2. It watches the `concile/` folder (`--dir concile`) for function and schema changes.
...
If you only changed a function in `concile/` you don't need to do anything else. The watch loop
...
a **real** `concile dev` (or `concile serve`) server, the actual CLI entrypoint an end user
...
<Callout title="Any cross-package feature must be proven through the real CLI server">
  ...
  and never wired through `concile dev`/`serve` the way a real user's request would travel, has
...
inference, and how fast `concile dev` starts up are treated as core product surface. If a
...
documentation (this site): how someone *uses* concile. `docs/dev/` and `docs/superpowers/` are
internal engineering docs (specs, architecture decisions, plans): how concile itself is
