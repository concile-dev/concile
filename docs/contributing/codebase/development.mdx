...
This means if you make an edit to `packages/values/src/index.ts` and then immediately run `bun run
--filter @concile/cli test`, you won't see your changes. The test will silently keep running against
the old compiled code in `dist/`. To see your updates, you need to run `bun run build` or `bun run
--filter @concile/values build` to refresh the `dist/` folder first.
