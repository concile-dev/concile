...
This is a how-to for building your own component: a package shaped like `@concile/scheduler` or
`@concile/auth` that a project opts into from its `concile.config.ts`. It assumes you already
...
as a couple of tables and a background loop (`@concile/triggers`), or as full-featured as tables, a
facade, an action-mode facade, HTTP routes, and a boot step (`@concile/auth`). The six components
...
`concile.config.ts`. Nothing is auto-installed, and nothing about the engine itself needs to change
...
or a client is meant to invoke directly. `@concile/scheduler`'s own dispatch loop, for example,
...
`@concile/authz`'s permission checks) only ever need to read. If your facade needs to write (insert
...
- `contextType: { import, type }` tells `concile codegen` which package and type name to import so
...
- `serverExports: string[]` lets you re-export a helper alongside `query`/`mutation`/`action` from
  `_generated/server.ts`, sourced from the same `contextType.import` path. `@concile/scheduler` sets
...
This is exactly the pattern `@concile/scheduler`'s `schedulerActionContext` and
`@concile/workflow`'s `workflowActionContext` both use: an action-mode facade delegates to an
...
- **`readLog({ afterTs, tables?, limit? })`**: reads committed changes from the underlying change log
  ...
  component (like `@concile/triggers`) is built on, described further below. Each returned change in
...
### A simpler driver: `@concile/triggers`

Not every driver needs a writable facade. `@concile/triggers` has no `context` at all: you configure
it declaratively (`{ table: { handler } }` in `concile.config.ts`), and its driver is just a cursor
...
`@concile/scheduler` uses it to read an app's declared `cronJobs()` schedule and write matching rows
...
## Worked example: `@concile/scheduler`
...
listing it in `concile.config.ts`:
...
(This mirrors `examples/auth-demo/concile.config.ts`, which composes `auth`, `scheduler`, and
...
This composed set is fixed at boot. Adding or removing a component from `concile.config.ts` needs a
restart (`concile serve` again, or a fresh `concile build`); there's no live "activate this
component" call. That's a deliberate, narrower boundary than `concile deploy`'s live hot-swap of
