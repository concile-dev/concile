...
[Reactivity](/docs/core-concepts/reactivity)). This is the shape every webhook in concile takes:
...
A conventional `concile/http.ts` default-exports an `httpRouter()` populated with `route()` calls. This
