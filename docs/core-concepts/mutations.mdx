...
A mutation is a function that writes data. It's the only place in concile allowed to write:
queries can only read, and actions don't get a `ctx.db` at all.

Every write in a concile app happens inside a mutation, no matter what triggered it: a client call,
...
generated locally) via the typed `mintId` that `concile codegen` (and `concile dev`'s
...
When a project composes a component in `concile.config.ts`, its facade is attached to `ctx`
