...
A field wrapped in `v.optional` is considered present only when the property exists on the object **and**
is not `undefined`. This means both `{ }` and `{ assigneeId: undefined }` count as "absent" for
validation purposes. But when it is present, the value gets checked against the inner validator just
like normal.
...
you are using the SQLite or Postgres adapters, your app's tables and fields are treated as raw *data*,
not as strict DDL.
...
This clever mechanism is what makes the additive-only rule discussed earlier a matter of pure *validation*,
rather than forcing a migration against physical storage. Nothing ever has to catch up with a schema
change since the storage layer never possessed app-shaped columns to begin with.
