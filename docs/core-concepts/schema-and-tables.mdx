...
A concile schema is a single TypeScript file. It lists every table in your database and the
shape of the documents each one holds. Your editor and the compiler understand it, and concile
...
Your `concile/schema.ts` file default-exports a `defineSchema` call. It maps table names to
...
concile assigns both on insert. Every table also gets a built-in `by_creation` index (creation
...
`concile deploy` reason about schema compatibility purely from JSON (see
...
At Tier 0/1 (the default: a single `concile dev`/`serve` process), `.shardKey` is metadata only.
...
Some tables are built into concile itself, rather than declared in your `schema.ts`. The current
...
In practice you won't hand-decode ids yourself. `concile codegen` emits a typed `mintId` in
...
Your schema changes as your app does. concile's live deploy (`concile deploy`) enforces an
...
Underneath `schema.ts`, concile's storage is **schemaless**. On both the SQLite and Postgres
...
- [Configuration reference](/docs/reference/configuration): the same `v.*` catalog in quick
  lookup form, alongside `concile.config.ts` and environment variables.
