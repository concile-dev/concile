...
concile has two Cloudflare deployment paths. Most projects want the first one; the Containers
...
`concile deploy --target cloudflare` provisions `@concile/runtime-cloudflare`'s Durable Object
...
Your worker source needs only a few lines: `export class ConcileDO extends
ConcileDurableObject { … }` plus `export default createWorkerHandler("CONCILE_DO")`.
`concile deploy` doesn't scaffold this file for you. It reconciles bindings into an existing
`wrangler.jsonc`, additively, never dropping a field you hand-wrote.

You also need a `concile/` directory with committed `_generated/`, the same requirement as
`concile serve`. `concile deploy` refreshes codegen before packaging.
...
`concile deploy --target cloudflare`:
...
<Callout type="info">
  ...
  fails if your committed `concile/_generated/` has drifted from a fresh codegen run.
</Callout>
...
`CONCILE_ADMIN_KEY` is required. The DO fails fast on an empty admin key. `concile deploy`
...
`concile deploy --target cloudflare` reconciles an `r2_buckets` binding into `wrangler.jsonc`.
...
first reaches it. Set `CONCILE_DO_LOCATION_HINT` (one of the 11 Cloudflare region codes, for
...
`@concile/docstore-d1`). The D1 tables and indexes for every `.global()` table are created
...
If you specifically want the same portable `concile serve` image (SQLite/Postgres storage
options) you'd run anywhere else, running inside a Cloudflare Container instead of a Durable
Object, that's possible too. It's hand-wired though (no `concile deploy` integration), and it has
...
It uses Workers (a stateless router), Containers (running the shipped `concile serve` image),
...
- [Deploy and build](/docs/deploy/deploy-and-build): `concile deploy`'s `serve` target and
  `concile build`, the other ways to ship a change.
