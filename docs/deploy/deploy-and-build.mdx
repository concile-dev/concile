...
sure the runtime fully starts before it begins serving. Unfortunately, `bun build --compile
--bytecode` will flat out reject top-level await. Since cold-start speed really doesn't matter much
...
In practice, these options aren't mutually exclusive at all. You could easily run `concile serve
--allow-deploy` inside a container and use `concile deploy` against it for all your day-to-day
