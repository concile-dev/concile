...
Reactivity in Concile is not magic, and it definitely isn't polling. It all comes down to one simple idea: **the engine tracks exactly what each query reads, tracks exactly what each mutation writes, and only refreshes a query when those two things overlap.**

Everything else you will read on this page follows from getting that single idea right. That includes our data model, how we handle commits, how we find affected subscriptions without scanning every single one, and why we strictly enforce determinism.
...
understand this loop, you understand concile.
