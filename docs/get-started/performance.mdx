...
We believe in measuring performance, not just making bold claims. Every number you see on this page comes from a fully reproducible benchmark that you can run yourself (using `bun run bench:*`). We also clearly state the exact conditions under which each measurement was taken.

The short version: Concile is a fresh TypeScript engine that competes in the exact same league as the Rust reference implementation it was modeled after (Convex). It delivers extremely fast reactive pushes and has honest, well-understood performance limits whether you run it on a single node or across a cluster.

## How to read these numbers

Benchmarks are notoriously easy to get wrong. Usually, it happens by accident when you compare two systems under slightly different conditions. The classic trap is running your own system natively while running your competitor inside a Docker container (which adds noticeable overhead on a Mac). You might look dramatically faster, but you're really just measuring the container penalty, not the software.

<Callout type="warn" title="Ballparks, not guarantees">

An early concile-vs-Convex run fell into exactly that trap and showed a 60x win. The win evaporated the moment both systems were put in identical containers. So, on this page: comparisons are same-substrate (both systems in identical containers) or they're not reported. Numbers come from a single machine; Docker Desktop on macOS taxes absolute latency, loopback has no WAN jitter, and several cells are co-located rather than genuinely distributed. The *shapes* (single-writer flatness, sub-linear scale-out, log-scaling fan-out) hold on other hardware. The absolute milliseconds will differ, so run the benchmarks on your own substrate before relying on any figure.
...
Throughput stays roughly flat as writers pile up. That's the single-writer signature, and it's true for both systems. The difference: Convex enforces a hard cap of 16 concurrent writers, and throughput drops past it, while concile has no such wall and degrades gently instead.

**The overhead is lean.** concile adds history, reactivity, and safety around every write. Measured as three rungs of the same write on one machine:
...
**Streaming reads on Postgres** (added in 0.0.8): paginated and limited reads (`.paginate()`, `.take(n)`) stream from a server-side cursor that stops fetching the moment the query engine has enough, instead of materializing the whole index range. Measured at 100k rows on the paginated shape: **−92% p50 wall-clock** and roughly 1000x fewer rows fetched. On by default, with a `CONCILE_PG_STREAM=0` kill switch.
