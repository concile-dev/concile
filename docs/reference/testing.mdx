...
Most test harnesses mock the database and hope the mock behaves like the real thing. `@concile/test`
...
This page is the complete reference for the harness. It covers every option `createTestConcile`
...
`createTestConcile` is async. Always `await` it, since it boots a real runtime: schema setup,
...
Keys from `import.meta.glob` come back with leading path segments, like `./concile/messages.ts`. The
harness normalizes them: it strips `./`/`../` prefixes, a leading `concile/` root, and the file
...
closing it in `t.close()`. This is how Concile's own suite proves read-your-own-writes parity on
...
Routes a `Request` through your app's `http.ts` router exactly as the real `concile dev`/`serve`
...
`http.ts`'s default export (an `HttpRouter`) is resolved once at `createTestConcile` time; a route
...
The identity reaches function code only through a context provider (for example `@concile/auth`'s
...
One `ConcileClient` is shared across every `t.subscribe(...)` call on a backend and all of its
...
Deterministic time control for `@concile/scheduler` jobs and crons, with no real timers or sleeps
involved. Both are driven by the harness's own virtual clock (starting at a fixed epoch millisecond,
mutable only through these two calls). Both are also clean no-ops if no `@concile/scheduler`
...
- **Both throw if `opts.now` was supplied** to `createTestConcile`. With a custom clock, the caller
...
Every `createTestConcile()` call is a fully independent backend: its own SQLite `:memory:` database
...
Any component your app opts into via `concile.config.ts` (the scheduler, workflow, triggers,
...
would under `concile dev`/`serve`. File storage (`ctx.storage`, the `_storage` table, the orphan
reaper driver) is composed unconditionally, without needing to be listed. It's always-on in
production too.

## Conformance suite

Concile's own repository ships a conformance suite: a large body of tests, all written against
`@concile/test`, whose entire purpose is to pin down and regression-guard the shipped engine's
...
`@concile/auth`'s `ctx.auth`, or a component reading `cctx.identity`). There is no bare
...
`concile migrate` rewrites your Convex import specifiers automatically, but it does **not**
translate `convex-test` calls to `@concile/test`. See
...
`@concile/test`'s in-process harness (everything above) is the right tool for function and
...
protocol, Docker boot, `concile deploy`'s live hot-swap, cross-runtime Bun vs. Node behavior), start
a real `concile dev`/`serve` as a child process and drive it with `@concile/client` over a real
WebSocket, or `POST /api/run` over HTTP. This is exactly the pattern Concile's own end-to-end tests
...
There is no `concile run` command and no separate `createConcile` testing API. Use
`@concile/test` for in-process tests, and `POST /api/run`, the dashboard's function runner, or the
...
- [Schema & tables](/docs/core-concepts/schema-and-tables): `defineSchema`, indexes, and
  `schemaValidation`.
- [Migrate from Convex](/docs/reference/migrate-from-convex): what `concile migrate` rewrites
