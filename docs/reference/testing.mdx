...
- **`advanceTimers(ms)`** pushes the virtual clock forward by exactly `ms` milliseconds and triggers
  ...
  `finishScheduledFunctions`. Similar to how fake timers like `advanceTimersByTime` work, it will
  **not** process jobs that are scheduled beyond that `ms` window. It will always move the clock
  forward (or throw an error if you provided a custom `now` function), even if you haven't composed
  a scheduler, because controlling time is a fundamental feature of the harness, not just a
  scheduler trick.
