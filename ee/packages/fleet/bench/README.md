...
relocated out of `test/` during the Phase-0 harness slice. They exercise the commercial `@concile/fleet`
package and so stay under the EE license here rather than moving into the MIT `benchmarks/` tree.

They will be converted into `@concile/bench` scenarios (write / scaled-write axes) when those axes are
activated. Until then, run one directly with vitest if needed:

    bun run --filter @concile/fleet exec vitest run bench/overhead-pg.bench.ts
