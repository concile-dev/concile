...
import { SimpleIndexCatalog, mutation, type RegisteredFunction } from "@concile/executor";
import { createEmbeddedRuntime } from "@concile/runtime-embedded";
import { shardIdList, encodeStorageIndexId } from "@concile/id-codec";
...
function benchModules(): Record<string, RegisteredFunction> {
  return {
    ...
    // Concile has no `ctx.db.patch` — read-merge-replace, same pattern as the fixture app's
...
export interface CommitBenchOpts {
   ...
   *  commits through the two-buffer stage-then-flush committer (`CONCILE_GROUP_COMMIT=1`'s runtime
...
// Opt-in on top of the embedded-postgres gate (CONCILE_BENCH=1): the matrix is ~4.5 minutes of
...
// (T5-gate runs, perf work), mirroring bench-fanout-pg's CONCILE_BENCH_FANOUT_PG pattern.
const RUN_BENCH = HAS_EMBEDDED_PG && process.env["CONCILE_BENCH"] === "1";
...
function buildRealPgStore(databaseUrl: string, numShards: number): { store: PostgresDocStore; client: NodePgClient } {
  const client = new NodePgClient({
    ...
    applicationName: `concile-b4-bench-${numShards}`,
