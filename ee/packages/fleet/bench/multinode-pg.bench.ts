 ...
 * (CONCILE_FLEET_MULTI_WRITER) against a SHARED Postgres, waits for the balancer to partition the
...
function spawnFleetServe(databaseUrl: string, port: number, numShards: number): ServeProcess {
  ...
  const proc = spawn(
    ...
    {
      env: { ...process.env, CONCILE_ADMIN_KEY: ADMIN_KEY, CONCILE_FLEET_MULTI_WRITER: "1", CONCILE_FLEET_SHARDS: String(numShards) },
...
const RUN = embeddedPgAvailable() && process.env["CONCILE_BENCH_MULTINODE"] === "1";
const maybe = RUN ? describe : describe.skip;

maybe("bench-multinode — distributed write throughput (opt-in: CONCILE_BENCH_MULTINODE=1 + embedded-postgres)", () => {
  afterAll(async () => {
