...
/**
 * Overhead ladder — an apples-to-apples answer to "how much does Concile's transaction layer cost
 ...
 *
 * embedded-postgres-gated + opt-in (CONCILE_BENCH_OVERHEAD=1). Single-client sequential
...
import { newDocumentId, shardIdList } from "@concile/id-codec";
...
const RUN = embeddedPgAvailable() && process.env["CONCILE_BENCH_OVERHEAD"] === "1";
const maybe = RUN ? describe : describe.skip;

maybe("bench-overhead — raw PG vs store commit vs full mutation (opt-in: CONCILE_BENCH_OVERHEAD=1)", () => {
  afterAll(async () => {
    await pgServer?.stop();
    pgServer = undefined;
  });
  ...
  it("overhead ladder — prints raw INSERT / store.commitWrite / runtime.run, same box", async () => {
    ...
    const client = new NodePgClient({
      ...
      applicationName: "concile-overhead-bench",
