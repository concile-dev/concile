 ...
 * typed `ConcileError` (an OCC conflict, a validation failure, …) means a LIVE owner answered
 ...
 */
import type { WriteRouter, ClientReplay } from "@concile/runtime-embedded";
import { DEFAULT_SHARD, type ShardId } from "@concile/id-codec";
import type { JSONValue } from "@concile/values";
import { isConcileError, concileErrorFromJSON, NOT_SHARD_OWNER_CODE, type ConcileErrorJSON } from "@concile/errors";
...
export class WriteForwarder implements WriteRouter {
  ...
  async forward(
    ...
    try {
      ...
      // A ConcileError here means a LIVE node answered DEFINITIVELY. Two cases:
      ...
      // A non-ConcileError (fetch rejected: the owner may have failed over or the connection
      // blipped) is the original TRANSPORT-failure retry case, unchanged from before.
      const shouldRetry = !isConcileError(firstErr) || firstErr.code === NOT_SHARD_OWNER_CODE;
  ...
  private async post(
    ...
    if (!res.ok || parsed.error !== undefined) {
      ...
      // message.
      if (parsed.errorJson) throw concileErrorFromJSON(parsed.errorJson);
