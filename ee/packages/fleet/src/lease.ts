...
import type { PgClient, PgRow, PgValue } from "@concile/docstore-postgres";
 ...
 *  deployment can shorten it via `CONCILE_FLEET_LEASE_TTL_MS` (ops/test tuning) — see
...
export interface LeaseManagerOptions {
   ...
   *  `CONCILE_FLEET_LEASE_TTL_MS` for the wedged-writer E2E (and available to operators as tuning);
...
export class LeaseManager {
  ...
  async selfFence(shardId: ShardId): Promise<void> {
    ...
    await this.client.query(
      `UPDATE shard_leases SET
         ...
         frontier_ts = GREATEST(frontier_ts, (SELECT nextval('concile_ts')))
  ...
  async evictExpired(shardId: ShardId = SHARD_ID): Promise<{ fenced: boolean; oldAppName: string | null }> {
    try {
      return await this.client.transaction(async (tx) => {
        ...
        // `frontier_ts = GREATEST(frontier_ts, (SELECT nextval('concile_ts')))`, NEVER `nextval`
        // alone. `frontier_ts` is the true high-water mark (the commit guard maintains it >= every
        // committed ts), while the `concile_ts` sequence can LAG reality in a mixed
   ...
   * `concile_ts` sequence per beat, then for EACH held shard advance its frontier up to `N` —
  ...
  async closeIdleFrontiers(runExclusiveOnShard: TryRunExclusiveOnShard): Promise<bigint> {
    ...
    const tsRows = await this.client.query(`SELECT nextval('concile_ts') AS ts`);
