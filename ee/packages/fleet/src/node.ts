...
/**
 * Fleet node composition (Task 6). Two entry points `concile serve --fleet` calls around the
...
import { NodePgClient, PostgresDocStore } from "@concile/docstore-postgres";
import { SqliteDocStore, NodeSqliteAdapter, BunSqliteAdapter } from "@concile/docstore-sqlite";
import type { DatabaseAdapter } from "@concile/docstore-sqlite";
...
import { DEFAULT_SHARD, shardIdList, type ShardId } from "@concile/id-codec";
import {
...
/** Default lease TTL (ms) when `CONCILE_FLEET_LEASE_TTL_MS` is unset — mirrors `lease.ts`'s own
 *  default so the two never drift. */
export const DEFAULT_LEASE_TTL_MS = 15_000;

/** Default number of shards a fleet node runs (B2a). T5 owns the persist-once/env/`CONCILE_FLEET_
 ...
 * fixed ratio to the TTL is computed here so one env var (`CONCILE_FLEET_LEASE_TTL_MS`) scales the
...
export function fleetApplicationName(advertiseUrl: string): string {
  ...
  return `concile-fleet-${discriminator}`;
}
 ...
 *  `NodeSqliteAdapter` would crash a Bun-hosted `concile serve --fleet` with "no such built-in
...
/**
 * Whether this deployment runs in MULTI-WRITER mode (`CONCILE_FLEET_MULTI_WRITER`), read in ONE
...
export function fleetMultiWriterEnabled(): boolean {
  return /^(1|true|yes)$/i.test(process.env.CONCILE_FLEET_MULTI_WRITER ?? "");
}

/**
 * Whether group commit (Fleet B4) is enabled (`CONCILE_GROUP_COMMIT`), read the SAME way as
...
export function groupCommitEnabled(): boolean {
  return /^(1|true|yes)$/i.test(process.env.CONCILE_GROUP_COMMIT ?? "");
}
...
export interface FleetHandles {
   ...
   * prior sample, so it reports 0 for that one call. Structurally all-zero when `CONCILE_GROUP_
...
export async function prepareFleetNode(deps: {
   ...
   *  `CONCILE_FLEET_LEASE_TTL_MS` (ops/test tuning; the wedged-writer E2E uses 4000). Default 15000
   ...
   *  T5 owns the persist-once/`CONCILE_FLEET_SHARDS`/mismatch-fail-fast story. Drives the commit
...
export async function startFleetNode(deps: StartFleetNodeDeps): Promise<FleetHandles> {
  ...
  // Multi-writer scale-out is OPT-IN (`CONCILE_FLEET_MULTI_WRITER`), off by default — see the
  ...
  const invalidationSink = async (inv: AppliedInvalidation): Promise<void> => {
    ...
    try {
      ...
      // `@concile/triggers`) never wakes on a co-writer's commit and instead sleeps up to its own
