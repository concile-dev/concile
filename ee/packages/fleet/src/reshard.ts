 ...
 *  ee `@concile/fleet` package has no dependency on core `packages/cli`, mirroring the same
  ...
  constructor() {
    super(
      "this database is not a fleet store (no shard_leases/fleet_nodes tables) — a fleet is " +
        "initialized by `concile serve --fleet`; run the fleet at least once before resharding",
    );
