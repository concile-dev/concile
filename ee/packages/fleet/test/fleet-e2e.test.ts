...
/**
 * Fleet slice ship gate: proves the EXACT production Tier-2 path — multiple REAL `concile serve
...
import { startEmbeddedPg, embeddedPgAvailable, type EmbeddedPg } from "@concile/docstore-postgres/test-support/embedded-pg";
...
// for mapping a shard-key value to its shard id. Imported from `@concile/id-codec`'s built `dist`
 ...
 *  `CONCILE_FLEET_LEASE_TTL_MS` to shrink the failover clock; unset (the existing scenarios) is
...
/** NUM_SHARDS this suite's sharded scenario pins (via `CONCILE_FLEET_SHARDS`) so the in-test
...
/* -------------------------------------------------------------------------- */
/* Multi-writer scenario helpers (B2b — CONCILE_FLEET_MULTI_WRITER=1)          */
/* -------------------------------------------------------------------------- */
...
maybeDescribe("concile serve --fleet — Tier-2 ship gate (real embedded postgres, real processes, failover)", () => {
  afterAll(async () => {
  ...
  it(
    ...
    async () => {
      ...
      // timescale: lease TTL 4s (⇒ probe ~1.33s, acquire-retry ~0.53s inside @concile/fleet). A
      ...
      const FLEET_TTL = { CONCILE_FLEET_LEASE_TTL_MS: "4000" };
  ...
  it(
    ...
    async () => {
      ...
      const SHARDS_ENV = { CONCILE_FLEET_SHARDS: String(NUM_SHARDS) };
  ...
  it(
    ...
    async () => {
      ...
      // Multi-writer scale-out is OPT-IN behind CONCILE_FLEET_MULTI_WRITER (default OFF keeps the
      ...
      const MW_ENV = {
        CONCILE_FLEET_MULTI_WRITER: "1",
        CONCILE_FLEET_SHARDS: String(NUM_SHARDS),
        CONCILE_FLEET_LEASE_TTL_MS: "6000",
      };
  ...
  it(
    ...
    async () => {
      ...
      // Multi-writer at the DEFAULT 15s lease TTL (NO CONCILE_FLEET_LEASE_TTL_MS). This is
      ...
      const MW_ENV = { CONCILE_FLEET_MULTI_WRITER: "1", CONCILE_FLEET_SHARDS: String(NUM_SHARDS) };
  ...
  it(
    ...
    async () => {
      ...
      // Single-writer fleet (A owns every shard). `CONCILE_FLEET_SHARDS=1` pins every write to the
      ...
      let nodeA: ServeProcess | undefined;
      try {
        nodeA = spawnFleetServe(databaseUrl, portA, undefined, { CONCILE_FLEET_SHARDS: "1" });
        const bootA = await waitForReadyOrExit(nodeA);
        if (!bootA.ready) throw new Error(`node A failed to boot: exit=${bootA.exitCode} stderr=${bootA.stderr}`);
        expect(bootA.ready.role).toBe("writer");
  ...
  it(
    ...
    async () => {
      ...
      const GC_ENV = { CONCILE_GROUP_COMMIT: "1", CONCILE_FLEET_SHARDS: String(NUM_SHARDS) };
