 ...
 * live `NodePgClient` connection string so it's proven end to end only in the `concile serve
...
import type { NodePgClient } from "@concile/docstore-postgres";
import { SqliteDocStore, NodeSqliteAdapter } from "@concile/docstore-sqlite";
import { newDocumentId, encodeStorageIndexId, shardIdList, shardIdForKeyValue, DEFAULT_SHARD, type ShardId } from "@concile/id-codec";
...
describe("Fleet B3 Task 2 — hybrid nodes", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CONCILE_FLEET_MULTI_WRITER = "1";
  ...
  afterEach(() => {
    ...
    delete process.env.CONCILE_FLEET_MULTI_WRITER;
  ...
  // driver here (e.g. `@concile/triggers`) would sleep on its wall-clock beat instead of waking
  ...
  it("single-writer WRITER boot (multi-writer OFF) is byte-identical: NO replica tailer is stood up", async () => {
    delete process.env.CONCILE_FLEET_MULTI_WRITER; // single-writer topology
    const client = new PgliteClient();
    const primary = new PostgresDocStore(client);
    await primary.setupSchema();
...
describe("Fleet B3 hazard — re-acquired shard re-floors the WRITE oracle (no stale RMW snapshot)", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CONCILE_FLEET_MULTI_WRITER = "1"; // hybrid regime — observeTimestamp feeds the QUERY oracle
  });
  afterEach(() => {
    delete process.env.CONCILE_FLEET_MULTI_WRITER;
    rmSync(tmp, { recursive: true, force: true });
