 ...
 * proven only through the real `concile serve --fleet` E2E; here we exercise their extracted,
...
import { newDocumentId, encodeStorageIndexId } from "@concile/id-codec";
import { encodeIndexKey } from "@concile/index-key-codec";
import type { DocStore, DocumentLogEntry, IndexWrite, InternalDocumentId } from "@concile/docstore";
...
describe("fleet node lifecycle", () => {
  ...
  describe("(e) reconcileReplicaIdentity — C7 foreign-replica deployment-id stamp", () => {
    ...
    it("the old prepareFleetNode call site is gone: sync boot's prep step never touches pgStore at all", async () => {
      ...
      // the full integration is the `concile serve --fleet` E2E. What IS unit-testable here is the
...
describe("F1 fix (Fenced Frontier B1 whole-branch review, BLOCKER): writer-boot frontier seed closes the pre-loaded-database bootstrap hole", () => {
  ...
  // exactly what a pre-fleet `concile serve` looks like. The operator then enables `--fleet` for
