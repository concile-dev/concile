# @concile/runtime-cloudflare-shard (Enterprise)

> Licensed under the **Concile Commercial License** — see [`ee/LICENSE`](../../LICENSE). This is a
> paid-tier (scale-out) package. Single-node Cloudflare self-host stays free forever via the
> [`@concile/runtime-cloudflare`](../../../packages/runtime-cloudflare) package.
...
and forwards to it. Each shard-DO is an **unmodified** `ConcileDurableObject` from the free
`@concile/runtime-cloudflare` package (M1 reuses Slice 3 verbatim — no engine change). The DO **name
...
1. **Explicit envelope** — `X-Concile-Shard: <value>` header or `?shard=<value>` query param. The
...
1. **Explicit** — `?region=<hint>` param or `X-Concile-Region: <hint>` header. App-controlled and
...
`X-Concile-Fanout: true`) fans the SAME request out to every shard-DO, concatenates each shard's
