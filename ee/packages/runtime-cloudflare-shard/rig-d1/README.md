...
The first-ever composition of the multi-shard router (`@concile/runtime-cloudflare-shard`) with the
...
`d1: bindingD1Client(env.DB)` into every shard-DO's `appConfig` (from `@concile/docstore-d1`, added
...
npx wrangler d1 create concile-shard-d1

# 3. deploy (pass the admin key as a --var override so no secret/var conflict with the placeholder)
npx wrangler deploy --var CONCILE_ADMIN_KEY:<a-strong-key>
#    → https://concile-do-shard-d1-fixture.<subdomain>.workers.dev

# 4. run the E2E against the live deployment
node e2e.mjs --url https://concile-do-shard-d1-fixture.<subdomain>.workers.dev --admin-key <same-key>

# 5. tear down
npx wrangler delete
npx wrangler d1 delete concile-shard-d1
```
