 ...
 *
 *   (a) EXPLICIT — a `?region=<hint>` param or an `X-Concile-Region: <hint>` header. App-controlled
 ...
 */
import { isValidLocationHint } from "@concile/runtime-cloudflare";

/** The explicit-region envelope sources, mirroring the shard-key `?shard=` / `X-Concile-Shard` pair. */
const REGION_HEADER = "x-concile-region";
...
/** Read the explicit `?region=` / `X-Concile-Region` hint, if any (header wins, mirroring the
