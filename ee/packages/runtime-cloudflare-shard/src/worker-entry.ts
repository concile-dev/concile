 ...
 * and it emits the SAME concrete DO class — a plain `ConcileDurableObject` subclass from the FREE
 ...
 * shard-DO is that SAME unmodified `ConcileDurableObject`, its `appConfig()` already knows how to use
...
export interface ShardWorkerEntryInputs {
  ...
  /** The shard-DO `durable_objects` binding name (e.g. `"CONCILE_DO"`). */
  bindingName: string;
  /** The exported DO class name the `wrangler.jsonc` migration references (e.g. `"ConcileDO"`). */
  doClassName: string;
  ...
  /** The env var the admin key is read from at DO boot (default `CONCILE_ADMIN_KEY`). */
   ...
   * since each shard-DO is that SAME unmodified `ConcileDurableObject` class. Absent → no R2 import,
...
export function generateShardWorkerEntrySource(inp: ShardWorkerEntryInputs): string {
  const host = inp.hostPackage ?? "@concile/runtime-cloudflare";
  const shard = inp.shardPackage ?? "@concile/runtime-cloudflare-shard";
  const adminEnv = inp.adminKeyEnvVar ?? "CONCILE_ADMIN_KEY";
  const mode = inp.mode ?? "key";
  const r2 = inp.r2BindingName;
  const L: string[] = ["// AUTO-GENERATED by `@concile/runtime-cloudflare-shard` worker-entry — do not edit."];
  ...
  L.push(`import { ConcileDurableObject } from ${JSON.stringify(host)};`);
  ...
  // no `@concile/blobstore-r2` at all (mirrors the free single-shard generator verbatim).
  if (r2) L.push(`import { R2BlobStore } from "@concile/blobstore-r2";`);
  L.push("");
  ...
  L.push("");
  L.push(`export class ${inp.doClassName} extends ConcileDurableObject {`);
