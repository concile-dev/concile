...
describe("createShardWorkerHandler — fanOut", () => {
  it("concatenates two shards' arrays, no partial", async () => {
    ...
    const handler = createShardWorkerHandler("CONCILE_DO", { mode: "hash", numShards: 2, loaded });
    const res = await handler.fetch(fanoutReq(), { CONCILE_DO: ns });
  ...
  it("does not exceed the concurrency cap in-flight (bounded pool over many shards)", async () => {
    ...
    const handler = createShardWorkerHandler("CONCILE_DO", { mode: "hash", numShards: N, loaded });
    const res = await handler.fetch(fanoutReq(), { CONCILE_DO: ns });
    expect(res.status).toBe(200);
