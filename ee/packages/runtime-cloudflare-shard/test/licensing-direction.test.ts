 ...
 * ONE-WAY: this paid (ee/) package depends on the FREE `@concile/runtime-cloudflare` and reuses its
...
describe("licensing direction — free never imports the paid shard package", () => {
  it("packages/runtime-cloudflare/src references no @concile/runtime-cloudflare-shard", () => {
    const offenders: string[] = [];
    scanTs(freePkgSrc, (file, text) => {
      if (text.includes("@concile/runtime-cloudflare-shard")) offenders.push(file);
    });
  ...
  it("the free host package.json does not depend on the shard package", () => {
    ...
    expect(Object.keys(allDeps)).not.toContain("@concile/runtime-cloudflare-shard");
  });

  it("this package does NOT depend on @concile/fleet (a sibling, not a consumer)", () => {
    const pkg = JSON.parse(readFileSync(join(import.meta.dirname, "..", "package.json"), "utf8"));
    const allDeps = { ...(pkg.dependencies ?? {}), ...(pkg.devDependencies ?? {}) };
    expect(Object.keys(allDeps)).not.toContain("@concile/fleet");
  });
});
