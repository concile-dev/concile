...
describe("resolveShard — explicit envelope key (source #1)", () => {
  it("routes by the X-Concile-Shard header", async () => {
    const r = await resolveShard(get("/api/sync", { "x-concile-shard": "roomA" }));
  ...
  it("header wins over query param", async () => {
    const r = await resolveShard(get("/api/sync?shard=roomB", { "x-concile-shard": "roomA" }));
...
describe("resolveShard — derive from POST /api/run args (source #2)", () => {
  ...
  it("an explicit key overrides derivation (skips the body read)", async () => {
    const r = await resolveShard(
      run({ path: "messages:send", args: { roomId: "roomA" } }, { "x-concile-shard": "roomB" }),
      { loaded },
    );
...
describe("resolveShard — cross-shard is refused, never fanned out (M1 non-goal)", () => {
  it("rejects an X-Concile-Fanout header (mode 'key' has no enumerable shard set)", async () => {
    const r = await resolveShard(get("/api/run", { "x-concile-fanout": "true" }));
    expect(r.kind).toBe("error");
...
describe("resolveShard — geographic placement hint (locationHint threading)", () => {
  ...
  it("threads the X-Concile-Region header hint", async () => {
    const r = await resolveShard(get("/api/sync?shard=roomA", { "x-concile-region": "weur" }));
  ...
  it("places the DERIVE-from-args path too (explicit region + a POST /api/run mutation)", async () => {
    const r = await resolveShard(
      run({ path: "messages:send", args: { roomId: "roomA" } }, { "x-concile-region": "apac-se" }),
      { loaded },
    );
