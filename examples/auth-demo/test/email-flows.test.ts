 ...
 * `ConcileClient`, no real dev server) — proving `concile.config.ts`'s newly-composed
...
import { defineAuth, type EmailMessage, type EmailProvider, type MintResult } from "@concile/auth";
...
describe("auth-demo — email flows (magic-link + reset), embedded-runtime idiom", () => {
  it("magic-link round trip: request → redeem → whoami.get reactively sees the new userId", async () => {
    ...
    const c = new ConcileClient(loopbackTransport(runtime.connect("magic-c")));
  ...
  it("reset revocation fans out: connection A's live whoami subscription flips to null", async () => {
    const capture = captureProvider();
    const runtime = await makeRuntime(capture.provider);
    const a = new ConcileClient(loopbackTransport(runtime.connect("reset-a")));
    const b = new ConcileClient(loopbackTransport(runtime.connect("reset-b")));
