...
import { ConcileClient, loopbackTransport, anyApi } from "@concile/client";
...
import { systemModules } from "@concile/admin";
...
describe("auth-demo — reactive auth flow", () => {
  it("signUp returns a token and userId", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("t0")));
  ...
  it("after signUp + setAuth, whoami.get reactively returns the userId", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("t1")));
  ...
  it("signOut → whoami.get reactively becomes null", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("t2")));
  ...
  it("two clients: sign-up on one, other subscribing to whoami stays isolated", async () => {
    const a = new ConcileClient(loopbackTransport(runtime.connect("a")));
    const b = new ConcileClient(loopbackTransport(runtime.connect("b")));
  ...
  it("signIn after signUp returns a fresh token and the same userId", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("t5")));
  ...
  it("anonymous → signUp upgrade preserves the userId", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("anon1")));
  ...
  it("revokeSession on one connection flips another connection's whoami subscription to null", async () => {
    const c = new ConcileClient(loopbackTransport(runtime.connect("revoke1")));
    const admin = new ConcileClient(loopbackTransport(runtime.connect("revoke2")));
