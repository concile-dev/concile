 ...
 *    This proves concile.config.ts is read and the `auth` component is declared.
 *
 * 2. loadProject(handBuiltProject, [auth]) → moduleMap contains "auth:signIn" and "whoami:get",
 *    and componentNames contains "auth". This proves the composition that concile dev performs
...
import { loadConfig, loadProject } from "@concile/cli";
import { auth } from "@concile/auth";

const exampleRoot = resolve(fileURLToPath(import.meta.url), "../../");

describe("auth-demo project loads", () => {
  it("loadConfig reads concile.config.ts and reports the auth component", async () => {
  ...
  it("loadProject with [auth] + whoami module: moduleMap contains auth:signIn and whoami:get", () => {
    ...
    // one module file "whoami" exporting a query named "get" (mirrors concile/whoami.ts).
