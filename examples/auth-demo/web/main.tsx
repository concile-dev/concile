// This demo composes `@concile/auth` with an `email` block whose provider is `consoleEmail()`
// (see `../concile.config.ts`) — a ZERO-CONFIG dev provider that never actually delivers mail.
// Every verification/reset/magic-link code or link is printed to the `concile dev` SERVER
...
import { ConcileClient, webSocketTransport, createAuthClient, anyApi, type SessionInfo } from "@concile/client";
import { ConcileProvider, useQuery, useMutation, useAction } from "@concile/client/react";
  ...
  return (
    <fieldset>
      ...
      <p>
        ...
        <code>concile dev</code> server console (the zero-config console provider), never here.
      </p>
    ...
    <fieldset>
      ...
      {!sent ? (
        ...
        <form onSubmit={handleReset}>
          <p>
            A reset code for <strong>{email}</strong> printed to the <code>concile dev</code> console.
          </p>
      ...
      {!sent ? (
        ...
        <form onSubmit={redeem}>
          <p>
            ...
            to the <code>concile dev</code> console.
          </p>
 ...
 *  it must match `oauth.redirectAllowlist` in `concile.config.ts` or `/start` rejects with 400
 ...
 *
 *  Needs REAL credentials in `concile.config.ts` (`GOOGLE_CLIENT_ID`/`GITHUB_CLIENT_ID` etc.) to
...
function OAuthButtons() {
  ...
  return (
    <fieldset>
      ...
      <p className="hint">
        Needs real provider credentials in <code>concile.config.ts</code> (unset by default in
 ...
 *  id_token is verified once (jose live JWKS fetch) and traded DIRECTLY for a Concile session, no
 *  redirect/handoff needed (the client already holds the token and calls this action itself). Paste
 *  an id_token from whatever issuer `concile.config.ts`'s `jwt.issuers` names. */
