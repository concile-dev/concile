...
// `@concile/triggers` is opt-in like every other component (see `packages/cli/src/load-config.ts`
// — there is no CLI-level auto-install list). This is the reference pattern real projects copy from
// (mirrors `examples/auth-demo/concile.config.ts` for `@concile/scheduler`/`@concile/workflow`):
// a durable audit log on `messages`, driven by `defineTriggers` reading the MVCC log rather than any
// code inline in `concile/messages.ts`'s own mutations. See `docs/enduser/triggers.md` and
// `concile/audit.ts`'s `_onChange` handler.
