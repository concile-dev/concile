/**
 * `R2BlobStore` — a `BlobStore` (`@concile/blobstore`) backed by a Cloudflare R2 bucket binding,
 * for the Durable Object host. It is the DO-native analog of `@concile/blobstore-fs` (local disk,
 * broken on a DO — no filesystem) and `@concile/blobstore-s3` (points at S3 via the heavy AWS SDK +
...
import type {
  ...
  SignUrlOpts,
} from "@concile/blobstore";

/** The engine's own proxied-upload endpoint (the `ctx.storage` context provider appends the
 *  capability `id`/`exp`/`token` params — see `@concile/storage`'s `context.ts`). */
 ...
 *  `@concile/blobstore-s3`'s `toBuffer`). We buffer so `size`/`sha256` are known before the R2
...
export class R2BlobStore implements BlobStore {
  ...
  async read(key: string, range?: ByteRange): Promise<ReadableStream<Uint8Array> | null> {
    // The serve endpoint (`@concile/storage`'s `handleServe`) always clamps `end` to `size-1` and
