...
import { runBlobStoreConformance } from "@concile/blobstore/test-support/conformance";
import { S3BlobStore } from "../src/s3-blobstore";

const endpoint = process.env.CONCILE_TEST_S3_ENDPOINT;
const bucket = process.env.CONCILE_TEST_S3_BUCKET ?? "concile-test";
  ...
  runBlobStoreConformance(
    ...
    () =>
      new S3BlobStore({
        ...
        forcePathStyle: true,
        region: "us-east-1",
        accessKeyId: process.env.CONCILE_TEST_S3_KEY ?? "minioadmin",
        secretAccessKey: process.env.CONCILE_TEST_S3_SECRET ?? "minioadmin",
      }),
  );
});
