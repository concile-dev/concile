/**
 * The runtime entry a `concile build` binary calls. It is compiled `serve`: boot an already-loaded
...
export function resolveBinaryOptions(argv: string[], env: Record<string, string | undefined>): BinaryOptions {
  ...
  let ip = "0.0.0.0";
  let dataDir = "./data";
  let databaseUrl = env.CONCILE_DATABASE_URL;
...
export async function runBinaryServer(
  ...
  if (!opts.adminKey) {
    process.stderr.write("✗ CONCILE_ADMIN_KEY is required — set it to a strong secret.\n");
    process.exit(1);
  }
