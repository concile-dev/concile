...
import { S3BlobStore, type S3Config } from "@concile/blobstore-s3";
 ...
 * `--database-url` over `CONCILE_DATABASE_URL`). Unset bucket → the FS backend. Reads the S3
 * settings from `CONCILE_STORAGE_*` plus the standard AWS credential vars.
 */
export function resolveStorageConfig(
  ...
  return {
    bucket: flags?.bucket ?? env.CONCILE_STORAGE_BUCKET,
    endpoint: flags?.endpoint ?? env.CONCILE_STORAGE_ENDPOINT,
    region: flags?.region ?? env.CONCILE_STORAGE_REGION,
    publicBaseUrl: flags?.publicBaseUrl ?? env.CONCILE_STORAGE_PUBLIC_URL,
