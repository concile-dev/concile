/**
 * The shared boot core for `concile dev` and `concile serve`: load the project, compose
...
import { NodeSqliteAdapter, BunSqliteAdapter, SqliteDocStore } from "@concile/docstore-sqlite";
import { NodePgClient, BunSqlClient, PostgresDocStore, type PgClient } from "@concile/docstore-postgres";
import type { DocStore } from "@concile/docstore";
import {
  createEmbeddedRuntime,
  type EmbeddedRuntime,
  type WriteRouter,
  type EmbeddedWriteFanoutAdapter,
} from "@concile/runtime-embedded";
import { InMemoryLogSink } from "@concile/executor";
import { AdminApi, browseTableModule, systemModules, verifyAdminKey } from "@concile/admin";
import type { GeneratedBundle } from "@concile/codegen";
import type { ComponentDefinition, Driver, WakeHost } from "@concile/component";
import type { RegisteredFunction } from "@concile/executor";
import type { JSONValue } from "@concile/values";
import type { BlobStore } from "@concile/blobstore";
import type { ObjectStore } from "@concile/objectstore";
import { shardIdList, DEFAULT_SHARD } from "@concile/id-codec";
import {
...
} from "@concile/storage";
import { receiptsReaper } from "@concile/receipts";
...
/**
 * Build the `PgClient` for a postgres `--database-url`/`CONCILE_DATABASE_URL`. Under the Bun
...
// `concile serve --object-store <url>` boots a single-shard (shard "0") writer node whose store is
// the Tier 3 object-storage substrate (`@concile/objectstore-substrate`) instead of the usual
...
// `FleetModule` applies to `@concile/fleet`): the shapes below are hand-declared structural
...
/** Structural mirror of `@concile/objectstore-substrate`'s public surface this module needs. */
...
export const OBJECTSTORE_SUBSTRATE_ERR_NO_PACKAGE =
  "concile: --object-store requires @concile/objectstore-substrate — install it (bun add @concile/objectstore-substrate).";
 ...
 * missing credentials, unparseable URL) — all of which share the `"concile: --object-store"` /
 * `"concile: invalid --object-store"` message prefix. `serveCommand` uses this to print a clean
...
export function isObjectStoreBootFailFast(e: unknown): e is Error {
  return e instanceof Error && /^concile: (--object-store|invalid --object-store)\b/.test(e.message);
}

/** Dynamic-import gate for the ee substrate package (mirrors `serve.ts`'s `@concile/fleet` gate:
...
export async function loadObjectStoreSubstrateModule(): Promise<ObjectStoreSubstrateModule> {
  try {
    const specifier: string = "@concile/objectstore-substrate";
...
export async function acquireWithRetry(
  ...
  for (;;) {
    ...
    if (now() >= deadline) {
      throw new Error(
        `concile: --object-store shard "0" held by '${last.heldBy}' until ${new Date(last.expiresAt).toISOString()} — ` +
...
/** Mirrors `@concile/runtime-embedded`'s own private `DEPLOYMENT_ID_GLOBAL_KEY` (and
 *  `@concile/fleet`'s exported `FLEET_DEPLOYMENT_ID_KEY`) — the same well-known magic string, not
...
/** Default lease TTL (ms) — mirrors `@concile/fleet`'s own default (`CONCILE_FLEET_LEASE_TTL_MS`
 ...
 *  `storageReaper`'s own 60s default. Overridable via `CONCILE_OBJECTSTORE_GC_MS` (env, production
 ...
 * on: a CAS-unsupported object store, `@concile/objectstore-substrate` not installed, or a
...
async function buildObjectStoreWriterNode(opts: {
  ...
  if (resolved === null) {
    throw new Error(`concile: --object-store "${opts.objectStoreUrl}" did not resolve to a store (empty/unset value?).`);
  }
  ...
  if (opts.shards !== undefined && opts.shards !== numShards) {
    throw new Error(
      `concile: --shards ${opts.shards} disagrees with the bucket's persisted shard count ${numShards} — ` +
        `reshard the bucket (\`concile objectstore reshard --object-store <url> --dir <dir> --shards ${opts.shards}\`) ` +
...
// `concile serve --object-store <url> --replica` boots a read-scaled REPLICA instead of a writer:
...
export const REPLICA_WRITE_REJECTED_MESSAGE =
  "concile: this node is a read replica (--replica) — it holds no write lease and cannot commit " +
...
// Decided ONCE, at first boot, and immutable after: `CONCILE_FLEET_SHARDS` (or the fleet
...
/** Default shard count when neither `CONCILE_FLEET_SHARDS` nor a persisted value is present.
 *  Mirrors `@concile/fleet`'s own `DEFAULT_NUM_SHARDS` (kept as an independent literal here —
 *  core `packages/cli` has zero static dependency on the enterprise `@concile/fleet` package). */
...
/** Parse `CONCILE_FLEET_SHARDS` — a positive integer, else undefined (falls through to the
...
/**
 * Parse `CONCILE_GROUP_COMMIT` (Fleet B4) — a boolean env flag, same `1`/`true`/`yes`
 * (case-insensitive) shape `@concile/fleet`'s `fleetMultiWriterEnabled` uses for
 * `CONCILE_FLEET_MULTI_WRITER`. This is just the token PARSER (truthy = `1`/`true`/`yes`); the
 * single-node DEFAULT when the var is unset is store-conditional — see `resolveGroupCommit`. Unlike `CONCILE_FLEET_
...
/**
 * Resolve the single-node (non-fleet) group-commit default. An explicit `CONCILE_GROUP_COMMIT`
 ...
 * single-node path — the fleet path still threads its own `CONCILE_GROUP_COMMIT` read).
 */
...
/** Build the fail-fast message for a `CONCILE_FLEET_SHARDS` value that disagrees with what's
...
export function numShardsMismatchError(envValue: number, persisted: number): Error {
  return new Error(
    `concile: CONCILE_FLEET_SHARDS=${envValue} conflicts with the shard count already persisted ` +
      ...
      `Unset CONCILE_FLEET_SHARDS, or set it to ${persisted} to match the existing deployment.`,
  );
}
...
export interface BootResult {
  ...
  /** The boot-time component set (from concile.config.ts) — `applyDeploy` re-composes against it. */
  ...
  /**
   * Reserved engine routes contributed by composed components (e.g. `@concile/auth`'s
 ...
 * `runFunction` — both resolve `modules`. Since `setModules` (dev reload / `concile deploy`)
...
/**
 * Fail fast when S3-shaped settings (`CONCILE_STORAGE_ENDPOINT`/`REGION`/`PUBLIC_URL`, or their
 * `--storage-endpoint`/etc. flag equivalents) are present but no bucket was configured. Those
 * settings only make sense for the S3 backend, which is selected SOLELY by `CONCILE_STORAGE_BUCKET`
...
export function assertStorageConfigCoherent(storage: StorageConfig | undefined): void {
  ...
  if (!s3Shaped) return;
  throw new Error(
    "concile: S3 storage settings (CONCILE_STORAGE_ENDPOINT/REGION/PUBLIC_URL or --storage-endpoint/etc.) " +
      "are set, but no bucket was provided — the S3 backend is selected only by CONCILE_STORAGE_BUCKET " +
...
function ensureStorageDirWritable(dir: string): void {
  try {
    ...
    throw new Error(
      `concile: file-storage directory "${dir}" is not creatable/writable — ${e instanceof Error ? e.message : String(e)}. ` +
        `Point --data at a writable location, or configure S3 storage (set CONCILE_STORAGE_BUCKET).`,
    );
  }
}
...
export interface BootLoadedOptions {
  ...
  /**
   * Tier 2 fleet wiring (from `@concile/fleet`'s `prepareFleetNode`). When set, `store` is the
  ...
  fleet?: {
    ...
    /** Fleet B4: group commit — resolved by `@concile/fleet`'s `node.ts` from its OWN
     *  `CONCILE_GROUP_COMMIT` read (mirrors how `numShards` is resolved fleet-side, before
   ...
   *  (~60s). `serve.ts` threads `CONCILE_OBJECTSTORE_GC_MS` in here; tests can also set it directly
   ...
   *  WRITER node owns (`serve.ts` threads `--shards`/`CONCILE_FLEET_SHARDS` in here). Unset / `1`
  ...
  /**
   * Tier 3 Slice 8 follow-on: the writer node's URL, from `--writer-url`/`CONCILE_WRITER_URL`.
   ...
   * `--wake-url`/`CONCILE_WAKE_URL` (`httpWakeHost`); the runtime then multiplexes every driver
   ...
   * `--backstop-min-ms`/`CONCILE_BACKSTOP_MIN_MS`. Threaded straight into `createEmbeddedRuntime`.
...
export async function bootLoaded(opts: BootLoadedOptions): Promise<BootResult> {
  ...
  if (opts.objectStoreUrl !== undefined && opts.fleet) {
    throw new Error("concile: --object-store cannot be combined with --fleet (Tier 2) — pick one write-scaling story.");
  }
  if (opts.replica && opts.objectStoreUrl === undefined) {
    ...
    throw new Error(
      "concile: --replica requires --object-store — a replica materializes from an object-storage bucket; pass --object-store <url>.",
    );
  }
  ...
  if (objectStoreReplicaNode && opts.writerUrl !== undefined && objectStoreReplicaNode.numShards > 1) {
    throw new Error(
      `concile: --writer-url (replica write-forwarding) is not yet supported on a multi-shard bucket ` +
  ...
  if (opts.fleet) {
    ...
    await store.setupSchema();
    numShards = await resolveNumShards(store, parseNumShards(process.env.CONCILE_FLEET_SHARDS));
  }

  // Fleet B4 (T4): resolve GROUP_COMMIT the same shape as `numShards` above — a fleet caller has
  // already resolved its own `CONCILE_GROUP_COMMIT` read (mirrors `fleetMultiWriterEnabled`'s
  // pattern in `@concile/fleet`'s `node.ts`) and threads it in as `opts.fleet.groupCommit`; the
  ...
  const groupCommit = opts.fleet
    ...
    : resolveGroupCommit({ envRaw: process.env.CONCILE_GROUP_COMMIT, databaseUrl: opts.databaseUrl });
  ...
  const runtime = await createEmbeddedRuntime({
    ...
    // above (fleet: threaded in already-resolved; non-fleet: read from CONCILE_GROUP_COMMIT).
    groupCommit,
