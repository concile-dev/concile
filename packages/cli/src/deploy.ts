/**
 * `concile deploy` — package the local functions directory and push it to a target via the
 * `@concile/deploy` seam. Targets: `serve` (slice-6b live hot-swap, back-compat default),
...
export async function resolveDeployOptions(args: string[], env: NodeJS.ProcessEnv): Promise<DeployOptions | { error: string }> {
  let url = env.CONCILE_DEPLOY_URL?.trim() ?? "";
  let dirFlag: string | undefined;
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--url" && args[i + 1]) url = args[++i]!;
    else if (args[i] === "--dir" && args[i + 1]) dirFlag = args[++i]!;
  }
  const adminKey = env.CONCILE_ADMIN_KEY?.trim() ?? "";
  if (!url) return { error: "missing target URL — pass --url <url> or set CONCILE_DEPLOY_URL" };
  if (!adminKey) return { error: "CONCILE_ADMIN_KEY is required to deploy" };
...
export async function packageApp(functionsDir: string): Promise<Array<{ path: string; code: string }>> {
  ...
  for (const abs of absFiles) {
    ...
    // `transform` strips TS types and leaves import specifiers untouched — bare `@concile/*`
...
function parseDeployFlags(args: string[]): { target?: string; env?: string; url?: string; dirFlag?: string; dryRun: boolean; check: boolean } {
  ...
  let url: string | undefined = process.env.CONCILE_DEPLOY_URL?.trim() || undefined;
  let dirFlag: string | undefined;
  let dryRun = false;
  let check = false;
  for (let i = 0; i < args.length; i++) {
...
export async function deployCommand(args: string[], deps: DeployDeps = {}): Promise<number> {
  ...
  // `resolveFunctionsDir` handles the precedence (--dir > `functionsDir` in concile.config.ts >
  ...
  if (flags.check) {
    ...
    if (drift) {
      process.stderr.write(`✗ ${functionsDir}/_generated is out of date — run \`concile codegen\` and commit the result\n`);
      return 1;
    }
