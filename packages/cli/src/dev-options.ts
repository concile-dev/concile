/**
 * Resolve `concile dev` options from CLI flags + defaults.
 */
...
export function resolveDevOptions(options: DevOptions = {}): ResolvedDevOptions {
  return {
    ...
    dataPath: options.dataPath ?? ".concile/data.db",
    runtime: options.runtime ?? "auto",
    webDir: options.webDir,
    databaseUrl: options.databaseUrl ?? process.env.CONCILE_DATABASE_URL,
