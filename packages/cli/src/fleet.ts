/**
 * `concile fleet` — the CLI entrypoint for offline fleet maintenance operations that live in the
 * enterprise `@concile/fleet` package. Today: `fleet reshard` (B5 Part 1, Task 9.2), which changes
 * a STOPPED Postgres fleet's shard count. Core `packages/cli` keeps ZERO static dependency on
 * `@concile/fleet` — it's loaded only via dynamic `import()`, mirroring `serve --fleet`'s gate
 * (`serve.ts`'s `fleetSpecifier`/`FLEET_ERR_NO_PACKAGE`).
 */
import { isPostgresUrl, makePgClient } from "./boot";

/** The slice of `@concile/fleet`'s reshard surface `fleetCommand` consumes (via dynamic import).
...
export const FLEET_ERR_NO_PACKAGE = "fleet mode requires @concile/fleet — install it (bun add @concile/fleet).";
...
export function parseReshardArgs(args: string[]): { ok: true; args: ReshardArgs } | { ok: false; error: string } {
  ...
  let databaseUrl: string | undefined = process.env.CONCILE_DATABASE_URL;
  ...
  if (!isPostgresUrl(databaseUrl)) {
    return {
      ok: false,
      error: "✗ --database-url postgres://… is required (or CONCILE_DATABASE_URL) — fleet reshard is Postgres-only",
    };
  }
...
/** `fleet reshard --shards M --database-url <pg>`: dynamically import `@concile/fleet`, open a
...
async function reshardCommand(args: string[]): Promise<number> {
  ...
  try {
    ...
    // `@concile/fleet` — mirrors `serve.ts`'s `fleetSpecifier` gate.
  ...
  try {
    ...
    process.stdout.write(
      `✓ resharded ${result.previousShards} → ${result.newShards} shards ` +
        ...
        `update CONCILE_FLEET_SHARDS to ${result.newShards} (or unset) before restarting the fleet\n`,
    );
    return 0;
  } catch (e) {
...
/** `concile fleet <sub> [...]` — sub-dispatches on `args[0]`. Unknown/absent sub → usage error. */
    ...
    default:
      process.stderr.write(
        `✗ unknown fleet subcommand: ${sub ?? "(none)"}\n` +
          `Usage: concile fleet reshard --shards M --database-url <url>\n`,
      );
      return 1;
  }
}
