...
export interface ResolvedFunctionsDir {
  ...
  functionsDir: string;
  /** Absolute path to the project root (where concile.config.ts lives). */
  projectRoot: string;
}
 ...
 *   2. `functionsDir` in concile.config.ts
 ...
 * to read the config before the directory name is known: concile.config.ts always sits at
...
export function functionsDirNotFoundMessage(functionsDir: string): string {
  return (
    ...
    `If this is a Convex app, run \`concile migrate\` to convert it. It renames\n` +
    `convex/ to concile/ and rewrites your imports.\n\n` +
    `Otherwise create ${join(functionsDir, "schema.ts")}, or point at an existing folder:\n` +
    `  concile dev --dir <path>\n`
  );
}
