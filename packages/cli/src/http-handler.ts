...
import { getHttpStatus, toConcileError, NotShardOwnerError, CommitGuardRejection } from "@concile/errors";
import { matchRoute } from "@concile/executor";
import { DEFAULT_SHARD, type ShardId } from "@concile/id-codec";
import type { EmbeddedRuntime } from "@concile/runtime-embedded";
import { handleAdminRequest, verifyAdminKey, type AdminApi } from "@concile/admin";
...
/**
 * The fleet node handle the HTTP layer consumes (structural mirror of `@concile/fleet`'s
...
export interface FleetHandles {
   ...
   *  handler optional-chains it. The real `@concile/fleet` node always provides it. */
   ...
   * calls — see `@concile/fleet`'s `node.ts`). Structurally all-zero when `CONCILE_GROUP_COMMIT`
   ...
   * mirroring `frontierStats?`'s same optional-for-backward-compat shape. The real `@concile/fleet`
   ...
   * the structural mirror; the guard skips (fail open) when absent. The real `@concile/fleet` node
   ...
   * (byte-identical to before this feature existed). The real `@concile/fleet` node always
...
function dashboardHtml(info: ServerInfo): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Concile</title>
...
export async function handleHttpRequest(
  ...
  if (req.method === "GET" && req.path === "/api/health") {
    ...
    // `CONCILE_GROUP_COMMIT` is off — `groupCommitStats()` structurally returns zeros in that case
  ...
  if (req.method === "POST" && req.path === "/api/run") {
    try {
      ...
      if (parsed.forwarded && replicaWriterUrl !== undefined) {
        return json(409, {
          error:
            "concile: this node is a read replica configured to forward writes (--writer-url) and " +
