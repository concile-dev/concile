 ...
 * specifiers with ERR_MODULE_NOT_FOUND; Bun accepts them). Bare deps (`@concile/*`, user npm
 ...
 * `.ts`, a `concile deploy`-pushed tree is `.js` — both bundle+load the same way.
 */
...
import type { SchemaDefinition } from "@concile/values";
import type { LoadedProject } from "./project";

const CACHE_BUST = () => `?t=${Date.now()}`;

/** The module key `concile` uses to address a functions-directory function file — strips the extension. */
 ...
 *  functions directory sits under its project root, whose `node_modules` has `@concile/*`; that is what we
...
/** The bundled output goes under `<root>/node_modules/.cache/concile` so the bundle's external
 *  bare imports (`@concile/*`, user npm deps) resolve via Node's ancestor walk. Uses the app's own
 *  `node_modules` root when it has one; otherwise falls back to the CLI's own root (which always has
 *  `@concile/*`), so a `node_modules`-less project (e.g. a temp-dir test fixture) still loads. */
function resolveCacheDir(startDir: string): string {
  ...
  const cacheDir = join(dir, "node_modules", ".cache", "concile", ns);
  mkdirSync(cacheDir, { recursive: true });
  return cacheDir;
}

/** Extra bare specifiers to keep EXTERNAL during bundle-on-load, beyond `@concile/*` — the
 *  escape hatch for a dep that must NOT be bundled/inlined: e.g. a package relied on for singleton
 *  identity across function modules (the same reason `@concile/*` itself is external — an
 *  inlined copy would break `instanceof`/module-level state shared across files), or one with
 *  native bindings esbuild can't bundle cleanly. `CONCILE_BUNDLE_EXTERNAL` is a comma-separated
...
function extraBundleExternals(): string[] {
  const raw = process.env.CONCILE_BUNDLE_EXTERNAL;
  if (!raw) return [];
  return raw
    .split(",")
    .map((s) => s.trim())
 ...
 *  native Node ESM too. `@concile/*` stays EXTERNAL: those are the engine's own packages, and
 ...
 *  can widen the external set via `CONCILE_BUNDLE_EXTERNAL` (see `extraBundleExternals`) for a
...
async function bundleAndImport(file: string, key: string, cacheDir: string): Promise<Record<string, unknown>> {
  const result = await build({
    entryPoints: [file],
    bundle: true,
    external: ["@concile/*", ...extraBundleExternals()],
    format: "esm",
    platform: "node",
    write: false,
    sourcemap: "inline",
