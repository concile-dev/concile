/**
 * `concile migrate` — turn an existing Convex project into a Concile project: rewrite
...
export async function migrateCommand(args: string[]): Promise<number> {
  ...
  // codemod, which is unchanged) so `concile migrate` stays one coherent command.
  ...
  // Rename the functions directory to Concile's own convention. Runs AFTER detect (which looks
  ...
  // it as `cwd` makes `resolve(cwd)` idempotent, and it loads `concile.config.ts` from that same
  ...
  try {
    ...
    // guard below only checks for `server.ts` (Concile's own extension), so those stale Convex
    // artifacts would otherwise survive untouched and can shadow the regenerated Concile files
