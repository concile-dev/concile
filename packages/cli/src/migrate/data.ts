/**
 * `concile migrate export` / `concile migrate import` — move an app's DATA between the two
 ...
 * `CONCILE_ADMIN_KEY`. Both the container `serve` path and the Cloudflare DO host expose those
...
function resolveOptions(
  ...
  let url = "";
  let file = "";
  let adminKey = env.CONCILE_ADMIN_KEY?.trim() ?? "";
  ...
  if (!adminKey) return { error: "CONCILE_ADMIN_KEY is required (or pass --admin-key)" };
  return { url, file, adminKey };
}

/** `concile migrate export --url <src> --out dump.json` — pull the source's full state to a file. */
export async function migrateExportCommand(args: string[]): Promise<number> {
  ...
  if (res.status === 401) {
    process.stderr.write("✗ unauthorized — check CONCILE_ADMIN_KEY / --admin-key\n");
    return 1;
  }
  if (!res.ok) {
...
/** `concile migrate import --url <dst> --in dump.json` — push a dump into the target (fresh). */
