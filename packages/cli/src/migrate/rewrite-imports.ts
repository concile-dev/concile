...
/**
 * Rewrite Convex import specifiers to their Concile equivalents. Operates on the quoted module
...
export function rewriteImports(source: string, file: string): { output: string; entries: ReportEntry[] } {
  ...
  output = output.replace(serverRe, (full, names: string, q: string, offset: number) => {
    ...
    if (allSchema) {
      entries.push({ severity: "auto-fixed", file, line, what: `import "convex/server" (schema)`, fix: `rewritten to "@concile/values"` });
      return full.replace(/["']convex\/server["']/, `${q}@concile/values${q}`);
    }
    if (allServer) {
    ...
    const cronFix = `cronJobs → import from "@concile/scheduler" and compose defineScheduler() in concile.config.ts`;
    const genericFix = `defineSchema/defineTable → "@concile/values"; httpRouter/httpAction → "./_generated/server"`;
    const fix =
      hasCron && hasOther
  ...
  while ((m = residualRe.exec(output)) !== null) {
    ...
    entries.push({ severity: "action-needed", file, line: lineOf(output, m.index), what: `import "convex/server"`, fix: `map manually: defineSchema/defineTable → "@concile/values"; httpRouter/httpAction → "./_generated/server"` });
  }
