...
const RULES: Rule[] = [
  { test: /\.withIndex\s*\(/, severity: "action-needed", what: ".withIndex(...) query",
    fix: `Concile has no .withIndex — use ctx.db.query(table, "index").eq(f, v).gte(f, v).order("asc"|"desc").collect()` },
  { test: /ctx\.db\.patch\s*\(/, severity: "action-needed", what: "ctx.db.patch(...)",
    fix: `Concile has no patch — read the doc, spread-merge, ctx.db.replace(id, { ...doc, ...changes })` },
  { test: /\.paginate\s*\(/, severity: "action-needed", what: ".paginate(...)",
    fix: `Concile paginate({ cursor, pageSize, maxScan? }) returns { page, nextCursor, hasMore, scanCapped }` },
  { test: /ctx\.auth\b|getUserIdentity\s*\(/, severity: "action-needed", what: "ctx.auth / getUserIdentity()",
    fix: `Identity is a string token via a context provider (e.g. @concile/auth's ctx.auth), not a JWT-claims object` },
  { test: /@convex-dev\/auth|["']convex\/auth["']/, severity: "unsupported", what: "Convex Auth",
    fix: `Auth is not auto-translated — use @concile/auth or external JWT` },
  { test: /\bapp\.use\s*\(/, severity: "unsupported", what: "Convex Component (app.use)",
    fix: `Convex Components don't map 1:1 — recompose via concile.config.ts` },
  { test: /\.vectorIndex\s*\(|\.searchIndex\s*\(/, severity: "unsupported", what: "vector/search index",
    fix: `search/vector is not yet supported in Concile (see roadmap)` },
];

/** Line-based scan for Convex runtime-API divergences Concile does NOT auto-transform. */
export function scanDivergences(source: string, file: string): ReportEntry[] {
  ...
  if (base === "crons.ts" || /\bcronJobs\s*\(/.test(source)) {
    ...
    entries.push({ severity: "action-needed", file, line: idx >= 0 ? idx + 1 : 1, what: "Convex crons (cronJobs)",
      fix: `Compose defineScheduler() in concile.config.ts and use cronJobs() from "@concile/scheduler"` });
  }
  if (base === "convex.config.ts") {
    entries.push({ severity: "unsupported", file, line: 1, what: "Convex app config (convex.config.ts)",
      fix: `Recompose components via concile.config.ts` });
  }
