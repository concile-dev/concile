 ...
 *       (e.g. `file:///var/lib/concile/objects` -> dir `/var/lib/concile/objects`)
 ...
 *
 * Examples:
 *   `s3://minioadmin:minioadmin@localhost:9000/concile-objects?region=us-east-1`
 *   `s3+https://minioadmin:minioadmin@objects.example.com/concile-objects`
...
function parseBoolParam(v: string | null): boolean | undefined {
  ...
  throw new Error(`concile: invalid --object-store URL — forcePathStyle must be "true" or "false", got "${v}"`);
}
...
export function parseS3ObjectStoreUrl(
  ...
  try {
    ...
    throw new Error(`concile: invalid --object-store URL "${url}": ${(e as Error).message}`);
  }

  const bucket = decodeURIComponent(u.pathname.replace(/^\//, "").split("/")[0] ?? "");
  if (!bucket) {
    throw new Error(
      `concile: --object-store S3 URL "${url}" has no bucket — use s3://host/<bucket> ` +
  ...
  if (!accessKeyId || !secretAccessKey) {
    throw new Error(
      `concile: --object-store S3 URL "${url}" is missing credentials — supply them in the URL ` +
...
/**
 * Resolve `--object-store <url>` / `CONCILE_OBJECT_STORE` to a constructed `ObjectStore`
...
export function resolveObjectStore(
  ...
  switch (schemeMatch[1]) {
    ...
    default:
      throw new Error(
        `concile: --object-store URL "${trimmed}" uses an unsupported scheme "${schemeMatch[1]}://" — ` +
