/**
 * `concile objectstore <subcommand>` — object-storage maintenance tools.
 *
 ...
 * dynamic-imports + gates `@concile/objectstore-substrate`, and runs `reshardObjectStore` — which
  ...
  if (sub !== "reshard") {
    process.stderr.write(
      `✗ unknown \`objectstore\` subcommand '${sub ?? ""}' — usage: ` +
        `concile objectstore reshard --object-store <url> --dir <functionsDir> --shards M\n`,
    );
    return 1;
  }
...
async function reshardCommand(args: string[]): Promise<number> {
  ...
  if (!objectStoreUrl) {
    process.stderr.write("✗ objectstore reshard requires --object-store <url> (or CONCILE_OBJECT_STORE).\n");
    return 1;
  }
  ...
  try {
    ...
    process.stdout.write(
      `✓ resharded ${result.fromShards} → ${result.toShards} shard(s) ` +
        ...
        `(or CONCILE_FLEET_SHARDS), or drop it (the bucket's persisted count is authoritative).\n`,
    );
    return 0;
  } catch (e) {
