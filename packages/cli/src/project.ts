 ...
 */
import type { RegisteredFunction, ContextProvider } from "@concile/executor";
import type { SchemaDefinition, SchemaDefinitionJSON } from "@concile/values";
import type { AnalyzedFunction, AnalyzedFunctionManifest, ShardByDeclaration } from "@concile/codegen";
import { validatorToTsType, assertShardByDeclarations } from "@concile/codegen";
import { composeComponents, type ComponentDefinition, type BootContext, type Driver, type ResolvedComponentRoute } from "@concile/component";
import type { SimpleIndexCatalog } from "@concile/executor";
import { STORAGE_TABLE, STORAGE_TABLE_NUMBER, storageTableDefinition } from "@concile/storage";
...
export function loadProject(
  ...
  // File storage is an ALWAYS-ON core feature (not read from `concile.config.ts`): inject its
  ...
  // SHARED load path (not just boot) keeps `concile deploy`'s additive-schema diff consistent:
  // the live schema and every re-pushed schema both carry `_storage`, so a deploy never sees it as
  // a "dropped table". Codegen already emits `_storage` from `@concile/values`' canonical system
