/**
 * Tier 3 Slice 8 follow-on: replica write-forwarding. `concile serve --object-store <url>
 ...
 *
 * Implements the engine's CORE `WriteRouter` seam (`@concile/executor`, re-exported by
 * `@concile/runtime-embedded`) — the SAME seam `@concile/fleet`'s `WriteForwarder` (ee)
...
import type { ShardId } from "@concile/id-codec";
...
export class ReplicaWriteForwarder implements WriteRouter {
  ...
  async forward(
    ...
    try {
      ...
      throw new Error(
        `concile: replica write-forward to writer "${this.writerUrl}" failed (unreachable) for ${kind} "${path}" — ${
    ...
    if (!res.ok || parsed.error !== undefined) {
      throw new Error(
        parsed.error ??
          `concile: replica write-forward to writer "${this.writerUrl}" returned HTTP ${res.status} for ${kind} "${path}"`,
      );
    }
