/**
 * `concile serve` — the production server. Unlike `dev`: requires a persistent admin key,
...
import type { SchemaJsonLike } from "@concile/admin";
import type { DocStore } from "@concile/docstore";
import type { EmbeddedRuntime, WriteRouter, EmbeddedWriteFanoutAdapter } from "@concile/runtime-embedded";
import type { FleetHandles } from "./http-handler";

/**
 * Structural mirrors of `@concile/fleet`'s public surface. Declared locally (not imported) so
 * core `packages/cli` keeps ZERO static dependency on the enterprise `@concile/fleet` package —
...
export interface FleetRuntimeOptions {
  ...
  /** Fleet B4 (T4): group commit — resolved fleet-side from `CONCILE_GROUP_COMMIT` (mirrors how
   *  `@concile/fleet`'s `node.ts` resolves `CONCILE_FLEET_MULTI_WRITER`), threaded straight
...
export interface FleetPrep {
   ...
   *  after THIS node's own boot has run — see `@concile/fleet`'s `node.ts`). */
  replicaPath?: string;
  lease: unknown;
  forwarder: unknown;
  role: "sync" | "writer";
...
/** The slice of `@concile/fleet` serve consumes (via dynamic import). */
export interface FleetModule {
  prepareFleetNode(deps: {
    ...
    adminKey: string;
    dataDir: string;
    /** Lease TTL in ms — the failover-clock knob (see `@concile/fleet`'s `node.ts`). Threaded from
     *  `CONCILE_FLEET_LEASE_TTL_MS`; undefined → the fleet default (15000). Ops/test tuning. */
    leaseTtlMs?: number;
...
export interface ServeOptions {
  ...
  webDir?: string;
  /** Enable `POST /_admin/deploy` (`concile deploy`'s hot-swap target). Off by default — a running
   * `serve` only accepts live code changes when explicitly opted in. */
  allowDeploy: boolean;
  /** Postgres connection string (flag wins over `CONCILE_DATABASE_URL`); unset → SQLite. */
  databaseUrl?: string;
   ...
   *  `CONCILE_OBJECTSTORE_GC_MS`. Unset → `boot.ts`'s `DEFAULT_OBJECTSTORE_GC_MS` (~60s). Ignored
   ...
   *  WRITER owns (`--shards N`, or `CONCILE_FLEET_SHARDS`). Unset / `1` → single-shard (shard "0"),
   ...
   *  in `serveCommand`. Flag (`--shards`) wins over `CONCILE_FLEET_SHARDS` env. */
   ...
   *  sets it. Flag (`--replica`) wins over `CONCILE_REPLICA` env. */
  replica?: boolean;
   ...
   *  unless `replica` is also set. Flag (`--writer-url`) wins over `CONCILE_WRITER_URL` env. */
  writerUrl?: string;
  /** The wake seam's host endpoint (`--wake-url`, wins over `CONCILE_WAKE_URL`) — for a host that
   ...
   *  `CONCILE_BACKSTOP_MIN_MS`) — `backstopMs = (d) => Math.max(d, n)`. Unset → identity (the
...
export const FLEET_ERR_NO_DB =
  "fleet mode requires --database-url (Postgres) — set --database-url postgres://… or CONCILE_DATABASE_URL.";
export const FLEET_ERR_NO_ADVERTISE =
  "fleet mode requires --advertise-url (or CONCILE_ADVERTISE_URL) — the URL other fleet nodes reach this node at, e.g. --advertise-url http://10.0.0.2:3000";
export const FLEET_ERR_NO_PACKAGE =
  "fleet mode requires @concile/fleet — install it (bun add @concile/fleet).";
...
/** Parse `CONCILE_FLEET_LEASE_TTL_MS` — a positive finite integer of ms, else undefined (the fleet
 *  default applies). Kept here (not in `@concile/fleet`) so the env read stays at serve's config
 ...
 * `--database-url`/`CONCILE_DATABASE_URL` names, runs `setupSchema()` (idempotent DDL only —
...
export function resolveServeOptions(args: string[]): ServeOptions {
  ...
  // consults `functionsDir` in concile.config.ts). Resolving eagerly here would bake a literal
  ...
  let functionsDir = "";
  let dataPath = process.env.CONCILE_DATA_DIR ? join(process.env.CONCILE_DATA_DIR, "db.sqlite") : "./data/db.sqlite";
  let ip = "0.0.0.0";
  let port = process.env.PORT ? Number(process.env.PORT) : 3000;
  let dashboard = process.env.CONCILE_DASHBOARD?.trim().toLowerCase() !== "off";
  let allowDeploy = process.env.CONCILE_ALLOW_DEPLOY === "1";
  let webDir = process.env.CONCILE_WEB_DIR;
  let databaseUrl = process.env.CONCILE_DATABASE_URL;
  let storageBucket: string | undefined;
  let storageEndpoint: string | undefined;
  let fleet = process.env.CONCILE_FLEET === "1" || process.env.CONCILE_FLEET?.trim().toLowerCase() === "true";
  let advertiseUrl = process.env.CONCILE_ADVERTISE_URL;
  let objectStoreUrl = process.env.CONCILE_OBJECT_STORE;
  let replica = /^(1|true|yes)$/i.test(process.env.CONCILE_REPLICA ?? "");
  let writerUrl = process.env.CONCILE_WRITER_URL;
  ...
  let wakeUrl = process.env.CONCILE_WAKE_URL;
  let backstopMinMs = parseLeaseTtlMs(process.env.CONCILE_BACKSTOP_MIN_MS);
  // Tier 3 multi-shard single-node serve: object-storage writer lane count. Set ONLY by the
  // `--shards` flag here; the `CONCILE_FLEET_SHARDS` env fallback is applied AFTER the flag loop
  // and ONLY when `--object-store` is present — otherwise a plain `--fleet` boot (which legitimately
  // reads `CONCILE_FLEET_SHARDS` for its OWN shard count via `resolveFleetNumShards`) would trip
  ...
  // CONCILE_FLEET_LEASE_TTL_MS is a pure ops/test tuning knob with no flag equivalent.
  // `parseLeaseTtlMs`'s "positive finite number, else undefined" parse is generic, not lease-specific.
  const objectStoreGcMs = parseLeaseTtlMs(process.env.CONCILE_OBJECTSTORE_GC_MS);
  ...
  // `CONCILE_FLEET_SHARDS` env fallback — ONLY for an object-store boot (never a fleet boot; see
  ...
  if (objectStoreShards === undefined && objectStoreUrl !== undefined) {
    objectStoreShards = parseNumShards(process.env.CONCILE_FLEET_SHARDS);
  }
  return {
    functionsDir,
    dataPath,
 ...
 *  caller injects the dynamically-imported `@concile/fleet` module + resolved config so this core
          ...
          const result = await applyDeploy(
            {
              ...
              deployRoot: join(process.cwd(), ".concile-deploy"),
...
export async function serveCommand(args: string[]): Promise<number> {
  ...
  const adminKey = process.env.CONCILE_ADMIN_KEY?.trim();
  ...
  if (!adminKey) {
    process.stderr.write("✗ CONCILE_ADMIN_KEY is required for `serve` — set it to a strong secret.\n");
    return 1;
  }
  // Resolve the functions directory (flag > concile.config.ts `functionsDir` > DEFAULT_FUNCTIONS_DIR)
  ...
  if (!existsSync(join(opts.functionsDir, "_generated", "server.ts"))) {
    process.stderr.write(
      `✗ ${opts.functionsDir}/_generated not found — run \`concile codegen --dir ${opts.functionsDir}\` and commit _generated/ before deploying.\n`,
    );
    return 1;
  }
  ...
  if (opts.replica && opts.objectStoreUrl === undefined) {
    process.stderr.write(
      "✗ --replica requires --object-store — a replica materializes from an object-storage bucket; " +
        "set --object-store <url> (or CONCILE_OBJECT_STORE).\n",
    );
    return 1;
  }
  ...
  if (opts.writerUrl !== undefined && !opts.replica) {
    process.stderr.write(
      "✗ --writer-url only applies to --replica (it's the writer this replica forwards mutations/" +
        "actions to) — pass --replica too, or drop --writer-url (or CONCILE_WRITER_URL).\n",
    );
    return 1;
  }
  ...
  if (opts.fleet) {
    ...
    try {
      ...
      // `@concile/fleet` — core cli has no static/type dependency on the enterprise package; it's
      // resolved at runtime via the workspace link. See package.json (fleet is deliberately absent).
      const fleetSpecifier: string = "@concile/fleet";
      fleetModule = (await import(fleetSpecifier)) as unknown as FleetModule;
    } catch {
      process.stderr.write(`✗ ${FLEET_ERR_NO_PACKAGE}\n`);
      return 1;
    }
    // `CONCILE_FLEET_SHARDS` (Shards B2a, T5): NUM_SHARDS, persisted once at first boot and
    ...
    try {
      numShards = await resolveFleetNumShards(v.databaseUrl, parseNumShards(process.env.CONCILE_FLEET_SHARDS));
    } catch (e) {
      process.stderr.write(`✗ ${e instanceof Error ? e.message : String(e)}\n`);
      return 1;
    }
    // `CONCILE_FLEET_LEASE_TTL_MS` (ops/test tuning): the lease TTL in ms, the single knob the
    // whole failover clock scales from inside `@concile/fleet` (heartbeat + acquire cadences are
    ...
    fleetConfig = {
      ...
      leaseTtlMs: parseLeaseTtlMs(process.env.CONCILE_FLEET_LEASE_TTL_MS),
      numShards,
    };
  }
