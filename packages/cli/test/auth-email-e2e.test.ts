/**
 * Auth A2 (email flows) — E2E through the real `concile dev` server (e2e-through-shipped-entrypoint
  ...
  it("(3) console-provider smoke: requestOtp logs the code and returns { sent: true }", async () => {
    ...
    try {
      ...
      try {
        ...
        await waitFor(
          () => logSpy.mock.calls.some((args) => args.some((a) => typeof a === "string" && a.includes("concile auth"))),
          5000,
          "console log",
        );
        expect(
