/**
 * Auth A3 (external identity) — E2E through the real `concile dev` server
 * (e2e-through-shipped-entrypoint rule). A REAL `@concile/client` over a REAL WebSocket to a REAL
 * server with `@concile/auth` composed WITH `oauth`/`jwt` config blocks, mirroring
 ...
 *
 * Per Task 1, a composed component's reserved routes (`@concile/auth`'s `/api/auth/oauth/*`) are
...
import { defineAuth, oauthProvider, type MintResult } from "@concile/auth";
...
/** Boot the real dev server with `@concile/auth` composed WITH both `oauth` (a single "mock"
...
async function startServer(mock: MockProvider): Promise<{ server: DevServer; wsUrl: string }> {
  const project = loadProject({ schema: appSchema, modules: appModules }, [
    defineAuth({
      ...
      jwt: { issuers: [{ issuer: mock.url, audience: "concile", jwksUrl: mock.jwksUrl }] },
    }),
  ]);
...
describe("auth A3 external identity — E2E through the real dev server", () => {
  ...
  it("(3) verified-email autolink through the real server: OAuth sign-in with the SAME verified email links to the pre-existing password account and revokes its session reactively", async () => {
    ...
    try {
      const { server, wsUrl } = await startServer(mock);
      const a = new ConcileClient(webSocketTransport(wsUrl, { reconnect: false }));
      const b = new ConcileClient(webSocketTransport(wsUrl, { reconnect: false }));
      try {
