...
// `bun run --filter @concile/cli test` resolves `vitest` off PATH (node_modules/.bin/vitest), which
...
describe("concile build (real compiled binary)", () => {
  it("compiles a fixture app (with a component) and the binary serves a committing mutation", async () => {
    const rc = await buildCommand(["--dir", "test/fixtures/build-app/concile", "--outfile", OUT, "--no-dashboard"]);
    expect(rc).toBe(0);
    expect(existsSync(OUT)).toBe(true);

    const proc = spawn(OUT, ["--port", "3599", "--hostname", "127.0.0.1", "--data-dir", DATA], {
      env: { ...process.env, CONCILE_ADMIN_KEY: "e2e" },
  ...
  it("cross-compiles to linux-x64 (produces a non-empty file, not executed here)", async () => {
    const rc = await buildCommand(["--dir", "test/fixtures/build-app/concile", "--outfile", `${OUT}-linux`, "--target", "linux-x64", "--no-dashboard"]);
    expect(rc).toBe(0);
  ...
  it("embeds the dashboard by default (served) and omits it with --no-dashboard", async () => {
    const rc = await buildCommand(["--dir", "test/fixtures/build-app/concile", "--outfile", `${OUT}-dash`]); // dashboard ON
    expect(rc).toBe(0);
    ...
    try {
      ...
      expect(html.toLowerCase()).toContain("concile");
