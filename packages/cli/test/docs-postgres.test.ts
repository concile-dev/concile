 ...
 * `--database-url`/`CONCILE_DATABASE_URL` flag pair, a `postgres://` connection-string example,
...
describe("self-hosting docs document Postgres", () => {
  const doc = readFileSync(
    join(import.meta.dirname, "../../../docs/deploy/postgres.mdx"),
    "utf8",
  );

  it("covers the connection-string option and compose service", () => {
    expect(doc).toContain("CONCILE_DATABASE_URL");
