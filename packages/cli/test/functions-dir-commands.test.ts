 ...
 * containing both the missing path and the `concile migrate` hint.
 *
...
describe("commands fail loudly on a missing functions directory", () => {
  it("serve reports the missing directory and the migrate hint", async () => {
    ...
    const prevKey = process.env.CONCILE_ADMIN_KEY;
    process.env.CONCILE_ADMIN_KEY = "test-key";
    let result: { code: number; err: string };
    try {
      result = await captureStderr(() => serveCommand(["--dir", dir]));
    } finally {
      if (prevKey === undefined) delete process.env.CONCILE_ADMIN_KEY;
      else process.env.CONCILE_ADMIN_KEY = prevKey;
    }
    expect(result.code).toBe(1);
    expect(result.err).toContain(dir);
    expect(result.err).toContain("concile migrate");
  });
  ...
  it("codegen reports the missing directory and the migrate hint", async () => {
    ...
    expect(err).toContain("concile migrate");
  });
