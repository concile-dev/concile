...
describe("resolveFunctionsDir", () => {
  it("defaults to concile/ with no flag and no config", async () => {
    const root = scratch();
    const r = await resolveFunctionsDir(undefined, root);
    expect(r.functionsDir).toBe(join(root, "concile"));
    expect(r.projectRoot).toBe(root);
  });

  it("uses functionsDir from concile.config.ts when present", async () => {
    const root = scratch();
    writeFileSync(join(root, "concile.config.js"), `export default { components: [], functionsDir: "convex" };`);
    const r = await resolveFunctionsDir(undefined, root);
  ...
  it("derives the project root from an explicit --dir outside the cwd", async () => {
    ...
    mkdirSync(nested);
    const r = await resolveFunctionsDir(join(nested, "concile"), root);
    expect(r.functionsDir).toBe(join(nested, "concile"));
...
describe("functionsDirNotFoundMessage", () => {
  it("names the missing directory and points at migrate", () => {
    const msg = functionsDirNotFoundMessage("/tmp/app/concile");
    expect(msg).toContain("/tmp/app/concile");
    expect(msg).toContain("concile migrate");
...
describe("ensureFunctionsDirExists", () => {
  ...
  it("returns false and writes the friendly message when the directory is missing", () => {
    ...
    expect(all).toContain("concile migrate");
  });
});

describe("regression guard", () => {
  it("the default functions directory is concile, not convex", async () => {
