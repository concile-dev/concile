/**
 * Fleet B4, Task 4 — `CONCILE_GROUP_COMMIT` env parsing (`groupCommitEnabled`, boot.ts) and its
 * threading into `bootLoaded` (non-fleet path — the fleet path threads its own already-resolved
 * value via `opts.fleet.groupCommit`, covered at the `@concile/fleet` level). Default OFF at this
 * task; mirrors `@concile/fleet`'s `fleetMultiWriterEnabled` boolean-env shape.
 */
...
describe("bootLoaded — CONCILE_GROUP_COMMIT threads into the runtime (non-fleet)", () => {
  const DATA_DIR = "./.tmp-groupcommit-boot";
  const DATA = `${DATA_DIR}/db.sqlite`;
  const ENV_KEY = "CONCILE_GROUP_COMMIT";
  ...
  it("absent + SQLite store: runtime.groupCommitStats() stays all-zero after a committing mutation (store-conditional default → OFF for SQLite)", async () => {
    ...
    const loaded = await loadFunctionsDir("test/fixtures/shard-dev/concile");
  ...
  it("CONCILE_GROUP_COMMIT=1: a committing mutation flushes through the grouped path", async () => {
  ...
  it("CONCILE_GROUP_COMMIT=0: byte-identical to absent — all-zero counters", async () => {
