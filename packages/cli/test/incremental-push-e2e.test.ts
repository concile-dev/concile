/**
 * End-to-end test: incremental (delta) `concile deploy` through the REAL `startServe` + the
...
describe("concile deploy — incremental (delta) push end-to-end through the real serve server", () => {
  it("v1 full push populates /modules; v2 one-file delta is live + reactive; unrelated module untouched", async () => {
    ...
    const deployRoot = join(process.cwd(), ".concile-deploy");
    const OLD_ADMIN_KEY = process.env.CONCILE_ADMIN_KEY;
    ...
    try {
      ...
      process.env.CONCILE_ADMIN_KEY = "k";
      ...
      if (OLD_ADMIN_KEY === undefined) delete process.env.CONCILE_ADMIN_KEY;
      else process.env.CONCILE_ADMIN_KEY = OLD_ADMIN_KEY;
      if (round1) {
