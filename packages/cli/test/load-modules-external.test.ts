/**
 * `CONCILE_BUNDLE_EXTERNAL` escape hatch: a caller can widen bundle-on-load's `external` set
 * beyond `@concile/*`, for a dep that must NOT be bundled/inlined (the deferred follow-on from
 ...
 *   - With CONCILE_BUNDLE_EXTERNAL="singleton-lib": the import is left external, so both
 *     bundled modules resolve it through Node's own module cache at runtime and share ONE
 *     instance — bumping one is visible to the other, same as `@concile/*` singleton identity.
 */
...
describe("loadFunctionsDir — CONCILE_BUNDLE_EXTERNAL escape hatch", () => {
  afterEach(() => {
    delete process.env.CONCILE_BUNDLE_EXTERNAL;
  });
  ...
  it("with CONCILE_BUNDLE_EXTERNAL, both modules share the one real module instance", async () => {
    process.env.CONCILE_BUNDLE_EXTERNAL = "singleton-lib";
    const dir = makeFixture();
    const loaded = await loadFunctionsDir(dir);
    const bumpA = loaded.modules.a!.bumpA as () => number;
    const bumpB = loaded.modules.b!.bumpB as () => number;
