...
// The migrated fixture scaffolds a real `concile.config.ts` that composes
// `defineScheduler()` from the real `@concile/scheduler` package (which itself depends on
...
// `node_modules/` .gitignore rule), and `@concile/*`/`cron-parser` are symlinked from the CLI
// package's own install — the same pattern `load-config.test.ts`'s `makeTmpDir` uses for
// `@concile/*`, extended here since this fixture loads the REAL scheduler component (not an
// inline stand-in).
...
describe("migrateCommand", () => {
  it("rewrites imports, regenerates _generated, and writes a report", async () => {
    ...
    const schema = readFileSync(join(root, "concile", "schema.ts"), "utf8");
    expect(schema).toContain(`from "@concile/values"`);
  ...
  it("--dry-run leaves the working tree untouched and writes no content edits", async () => {
    ...
    // --dry-run would fail with "concile already exists" — see migrate.ts). It still writes
  ...
  it("deletes a pre-existing Convex _generated/ before regenerating, so stale .js files don't survive", async () => {
    ...
    // The pre-write guard only checks for server.ts (Concile's own extension), so these stale
    // Convex artifacts previously survived a migrate untouched and could shadow the regenerated
    // Concile files (a JS-first resolver picks the stale server.js, which imports the
    ...
    const generatedDir = join(root, "concile", "_generated");
