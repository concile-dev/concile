...
describe("concile migrate renames the functions directory", () => {
  it("moves convex/ to concile/ and leaves no convex/ behind", async () => {
    ...
    expect(existsSync(join(root, "concile", "notes.ts"))).toBe(true);
    expect(existsSync(join(root, "convex"))).toBe(false);
  });

  it("refuses when a concile/ directory already exists", async () => {
    const root = convexApp();
  ...
  it("--dry-run leaves convex/ untouched and does not create concile/", async () => {
    const root = convexApp();
    const prevCwd = process.cwd();
    try {
      process.chdir(root);
  ...
  it("preview-then-commit: --dry-run followed by a real run completes the rename", async () => {
    ...
    // dry run had already renamed convex/ -> concile/, this second call would fail with
    // "concile already exists" (functionsDir still resolves to convex, which the dry run left
