/**
 * Notifications N1 — E2E through the real `concile dev` server (e2e-through-shipped-entrypoint
 * rule). A REAL `@concile/client` over a REAL WebSocket to a REAL server with
 * `@concile/notifications` composed. The reactive-inbox proof is the headline:
...
import { defineNotifications, type EmailMessage, type EmailProvider } from "@concile/notifications";
