/**
 * Shards B2a, Task 5 — NUM_SHARDS first-boot config: `CONCILE_FLEET_SHARDS` env parsing,
...
describe("bootLoaded — NUM_SHARDS persists across boots (non-fleet)", () => {
  ...
  const ENV_KEY = "CONCILE_FLEET_SHARDS";
  ...
  it("a later boot with a disagreeing CONCILE_FLEET_SHARDS fails fast, naming both values", async () => {
    delete process.env[ENV_KEY];
    const loaded = await loadFunctionsDir("test/fixtures/shard-dev/concile");
    const first = await bootLoaded({ loaded, components: [], dataPath: DATA, adminKey: "k" });
    first.store.close();
  ...
  it("a later boot with an AGREEING CONCILE_FLEET_SHARDS boots normally", async () => {
    delete process.env[ENV_KEY];
    const loaded = await loadFunctionsDir("test/fixtures/shard-dev/concile");
    const first = await bootLoaded({ loaded, components: [], dataPath: DATA, adminKey: "k" });
    first.store.close();
