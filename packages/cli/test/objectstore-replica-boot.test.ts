...
// E2E through two real `concile serve` processes over one bucket (fs + MinIO, WebSocket
...
describe("bootLoaded — Tier 3 Slice 8 object-store replica node", () => {
  ...
  it("serveCommand rejects --replica without --object-store with a clear, fast (no boot work) error", async () => {
    const originalAdminKey = process.env.CONCILE_ADMIN_KEY;
    const originalWrite = process.stderr.write.bind(process.stderr);
    let captured = "";
    process.env.CONCILE_ADMIN_KEY = "test-admin-key";
    process.stderr.write = (chunk: string) => {
      captured += chunk;
      return true;
    };
    try {
      const code = await serveCommand(["--replica", "--dir", "test/fixtures/deploy-v2/concile"]);
      expect(code).toBe(1);
      expect(captured).toMatch(/--replica requires --object-store/);
    } finally {
      process.stderr.write = originalWrite;
      if (originalAdminKey === undefined) delete process.env.CONCILE_ADMIN_KEY;
      else process.env.CONCILE_ADMIN_KEY = originalAdminKey;
    }
  });
});
