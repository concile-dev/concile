 ...
 * object-store bucket, each a real `concile serve` boot through the shipped `startServe` entrypoint
 ...
 *      reactive propagation through TWO real `concile serve` processes, over object storage alone
...
import { readConsumerWatermarks } from "@concile/objectstore-substrate";
...
const FUNCTIONS_DIR = "test/fixtures/deploy-v2/concile";
...
describe("concile serve --object-store --replica — end-to-end (fs, real server)", () => {
  it(
...
const RUN_MINIO = dockerAvailable() && process.env.CONCILE_OBJECTSTORE_S3 === "1";
...
const BUCKET = "concile-objstore-replica-e2e";
...
/** Resolve `@aws-sdk/client-s3` from `@concile/objectstore-s3`'s own node_modules (used only for
...
function loadS3Sdk(): { S3Client: any; CreateBucketCommand: any } {
  ...
  const reqS3 = createRequire(reqRoot.resolve("@concile/objectstore-s3"));
...
maybeDescribe("concile serve --object-store --replica — end-to-end (real MinIO, gated)", () => {
  afterAll(() => stopMinio());
