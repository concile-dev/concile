 ...
 * over ONE object-store bucket, each a real `concile serve` boot through `startServe` (real HTTP +
...
const FUNCTIONS_DIR = "test/fixtures/shard-dev/concile";
...
describe("concile serve --object-store --shards 3 --replica — end-to-end (fs, real server)", () => {
  it(
...
const BUCKET = "concile-objstore-replica-ms-e2e";
...
maybeDescribe("concile serve --object-store --shards 3 --replica — end-to-end (real MinIO, gated)", () => {
  afterAll(() => stopMinio());
