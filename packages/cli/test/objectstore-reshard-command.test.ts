/**
 * `concile objectstore reshard` (object-storage reshard, Task R2/R3): arg parsing/gate (R2a, no
...
describe("objectstoreCommand — R2a arg parsing / gate (no bucket needed)", () => {
  const savedEnv = process.env.CONCILE_OBJECT_STORE;
  afterEach(() => {
    if (savedEnv === undefined) delete process.env.CONCILE_OBJECT_STORE;
    else process.env.CONCILE_OBJECT_STORE = savedEnv;
  });
  ...
  it("reshard without --object-store (and no env fallback) → exit 1", async () => {
    delete process.env.CONCILE_OBJECT_STORE;
