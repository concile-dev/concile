 ...
 * `concile serve` running under BUN (the primary runtime; `PostgresDocStore` was only proven
 ...
 * spawns the real `concile` CLI entrypoint (`src/bin.ts`) as a child process via `bun`, mirroring
 ...
 *
 * Proves, through the real `concile serve` process + a real Postgres container:
...
function fixtureFunctionsDir(): string {
  return resolve(new URL(".", import.meta.url).pathname, "fixtures", "deploy-v2", "concile");
}
...
function spawnServe(databaseUrl: string): ServeProcess {
  return spawn(
    ...
    { env: { ...process.env, CONCILE_ADMIN_KEY: ADMIN_KEY }, stdio: ["ignore", "pipe", "pipe"] },
  );
}
...
maybeDescribe("concile serve — Postgres ship gate (real container, real bun process)", () => {
  afterAll(() => {
