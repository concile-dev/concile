...
describe("push with components passes contextType into codegen", () => {
  it("emits declare module augmentation in server.ts when component has contextType", () => {
    ...
    expect(generated.server.content).toContain('declare module "@concile/executor"');
    expect(generated.server.content).toContain('auth: import("@concile/auth").AuthContext');
  });
  ...
  it("does NOT emit augmentation when no components have contextType", () => {
    ...
    expect(generated.server.content).not.toContain('declare module "@concile/executor"');
  });
