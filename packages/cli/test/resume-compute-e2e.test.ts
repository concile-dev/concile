 ...
 *
 * Two required scenarios, driven through a REAL `concile dev` server + a REAL `@concile/client`
...
describe("resume compute E2E (1) — unchanged reconnect skips the RERUN re-execution", () => {
  it(
    ...
    async () => {
      ...
      const reader = new ConcileClient(webSocketTransport(directUrl, { reconnect: false }));
      try {
