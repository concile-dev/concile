 ...
 * unit-test harness: `loadProject` composes `@concile/scheduler` exactly the way a project's
 * `concile.config.ts` would (see `examples/auth-demo/concile.config.ts`), `createEmbeddedRuntime`
...
describe("scheduler — Convex parity end-to-end through the real dev server", () => {
  it("ctx.scheduler.runAfter(0, ...) dispatches through the shipped dev server via reactive wake", async () => {
    // Compose the app + the scheduler component exactly as a real `concile.config.ts` listing
    // `defineScheduler()` would (see `../src/load-config.ts` / `examples/auth-demo/concile.config.ts`).
