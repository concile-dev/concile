...
describe("resolveServeOptions — --allow-deploy / CONCILE_ALLOW_DEPLOY", () => {
  const OLD = process.env.CONCILE_ALLOW_DEPLOY;
  afterEach(() => {
    if (OLD === undefined) delete process.env.CONCILE_ALLOW_DEPLOY;
    else process.env.CONCILE_ALLOW_DEPLOY = OLD;
  });

  it("defaults to false", () => {
    delete process.env.CONCILE_ALLOW_DEPLOY;
  ...
  it("CONCILE_ALLOW_DEPLOY=1 sets it true", () => {
    process.env.CONCILE_ALLOW_DEPLOY = "1";
    expect(resolveServeOptions([]).allowDeploy).toBe(true);
  });

  it("CONCILE_ALLOW_DEPLOY unset or any other value stays false", () => {
    process.env.CONCILE_ALLOW_DEPLOY = "true";
    expect(resolveServeOptions([]).allowDeploy).toBe(false);
  });
});
