/**
 * `concile serve` — production server. Fail-fast checks (no admin key, no `_generated/`) return
...
describe("serveCommand fail-fast", () => {
  const OLD = process.env.CONCILE_ADMIN_KEY;
  afterEach(() => {
    if (OLD === undefined) delete process.env.CONCILE_ADMIN_KEY;
    else process.env.CONCILE_ADMIN_KEY = OLD;
  });

  it("returns 1 with a clear message when CONCILE_ADMIN_KEY is unset", async () => {
    delete process.env.CONCILE_ADMIN_KEY;
