...
import { mutation, query, action, type RegisteredFunction } from "@concile/executor";
import { createStorageToken } from "@concile/storage";
...
describe("storage wiring (boot smoke)", () => {
  ...
  // Fix 1 regression: `concile serve`/`dev` on Node reads the storage-route body via
