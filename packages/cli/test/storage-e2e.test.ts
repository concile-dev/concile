/**
 * Slice 4 (file storage) SHIP GATE — proves the whole feature works through the REAL `concile
 ...
 * on-disk fixture `concile/` dir with a `files` table holding `image: v.id("_storage")`, and both
 ...
 * per run so a fresh `node_modules/@concile` symlink can resolve the bare `@concile/*` imports
...
function makeFixtureFunctionsDir(): string {
  const src = resolve(new URL(".", import.meta.url).pathname, "fixtures", "storage-app", "concile");
  const root = mkdtempSync(join(tmpdir(), "sb-storage-e2e-"));
  const dir = join(root, "concile");
...
/** Resolve `@aws-sdk/client-s3` from `@concile/blobstore-s3`'s own node_modules (it isn't a
 * direct `@concile/cli` dependency, but the S3 adapter carries it). Used only for bucket
...
function loadS3Sdk(): { S3Client: any; CreateBucketCommand: any } {
  ...
  const reqS3 = createRequire(reqRoot.resolve("@concile/blobstore-s3"));
...
maybeDescribe("file storage — MinIO container ship gate (real serve, presigned direct path)", () => {
  ...
  it(
    ...
    async () => {
      ...
      process.env.CONCILE_STORAGE_REGION = "us-east-1";
