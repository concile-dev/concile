 ...
 * `ConcileClient`, no UI) draining a durable outbox against a REAL `concile dev` server.
 *
 * Mirrors the reload-fidelity model `outbox-e2e.test.ts:13-22` documents: a single `IDBFactory`
 * shared across a seeding `ConcileClient` and the headless drain is the faithful analog of a real
 * browser reload / a Service Worker `sync` event firing after the tab that queued the work is gone.
 * Session 1 (a normal `ConcileClient`) seeds mutations OFFLINE — pointed at a dead port that never
 ...
 *   (3) a poison entry (a coded, non-retryable `ConcileError` throw) is counted in `failed`; the
...
import { v, defineSchema, defineTable, type Value } from "@concile/values";
import { query, mutation } from "@concile/executor";
import { DocumentValidationError } from "@concile/errors";
...
const notesModule = {
  ...
  // A deliberate poison unit (scenario 3): a coded, NON-retryable `ConcileError` throw so the wire
  ...
  // non-`ConcileError` throw never gets `.code` threaded onto a FRESH response, per
...
/** Seed `mutations` offline via a normal `ConcileClient` pointed at a dead port, over the given
...
async function seedOffline(idb: IDBFactory, mutations: Array<{ udfPath: string; args: Record<string, Value> }>): Promise<void> {
  ...
  const client = new ConcileClient(nodeWsTransport(wsUrlFor(deadPort)), {
    outbox,
    outboxLocks: null,
...
async function listNotes(port: number, box: string): Promise<Array<{ text: string }>> {
  const reader = new ConcileClient(nodeWsTransport(wsUrlFor(port)), {
    outbox: memoryOutbox(),
    outboxLocks: null,
    outboxDrainIntervalMs: 0,
  });
