...
const ENV_KEYS = ["CONCILE_WAKE_URL", "CONCILE_BACKSTOP_MIN_MS"] as const;
...
describe("resolveServeOptions — wake seam flags", () => {
  ...
  it("reads both from env", () => {
    stash("CONCILE_WAKE_URL", "http://wake.do/arm");
    stash("CONCILE_BACKSTOP_MIN_MS", "900000");
    const opts = resolveServeOptions([]);
  ...
  it("the flag wins over env (mirroring --object-store)", () => {
    stash("CONCILE_WAKE_URL", "http://env.example/arm");
    stash("CONCILE_BACKSTOP_MIN_MS", "1000");
