/**
 * `createAuthClient` — a thin token-lifecycle manager over a `ConcileClient` (auth slice A1).
...
/** The minimal `ConcileClient` surface `createAuthClient` needs (kept structural for testability).
 *  `mutation` deliberately returns `Promise<unknown>` (not a generic) so `ConcileClient`'s
...
export interface AuthManagedClient {
  ...
  /** `opts.transient` is threaded straight to `ConcileClient.mutation`'s own escape hatch — the
...
function defaultLock(): RefreshLock {
  ...
  if (locks) {
    return { run: <T>(fn: () => Promise<T>) => locks.request("concile:auth:refresh", fn as () => Promise<unknown>) as Promise<T> };
  }
...
function defaultBroadcast(): PairBroadcast {
  ...
  const ch = new BC("concile:auth:pair");
  return {
