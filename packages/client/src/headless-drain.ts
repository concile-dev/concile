 ...
 * `ConcileClient`, no queries, no optimistic layers. Chromium's one-shot Background Sync then
 ...
 *  - The SAME `Connect`-handshake helpers `ConcileClient` uses (`./connect-handshake`), fed from
 ...
 *    Locks name `client.ts` uses (`concile:outbox:<origin>:<deployment>`) — a live tab already
 ...
 *
 * Unlike `ConcileClient`, this function does NOT implement the `known: true` per-seq classifying
 ...
 */
import type { ServerMessage } from "@concile/sync";
...
export interface HeadlessDrainOptions {
   ...
   *  reconnecting `ConcileClient` replays its last-set token. */
   ...
   *  `SESSION_STORAGE_KEY` (`"concile.session"`, exported from `auth-client.ts`) in `localStorage`,
 ...
 *  compute the SAME value a live tab's `ConcileClient` would, or the two never contend for the
 *  same lock. */
 ...
 * handler inside `event.waitUntil(...)`, or from any other script with no live `ConcileClient`.
 */
export async function drainOutboxOnce(opts: HeadlessDrainOptions): Promise<{ drained: number; failed: number; remaining: number }> {
  ...
  const lockName = `concile:outbox:${originTag()}:${deployment}`;
  const timeoutMs = opts.timeoutMs ?? 30_000;

  const initial = await outbox.loadAll();
  if (countActive(initial.entries) === 0) {
    // Nothing to do — mirrors `ConcileClient`'s own "an EMPTY outbox sends NO first-connect
      ...
      if (msg.known) {
        ...
        void handleClientReset()
          ...
          .catch((err) => {
            ...
            console.error("[concile] outbox: handleClientReset failed", err);
            checkDone();
          });
      }
    }
