/**
 * `@concile/client` — the framework-agnostic reactive client: the `api` proxy, transports
 * (loopback + WebSocket), and `ConcileClient`. React bindings are at `@concile/client/react`.
 */
...
export { ConcileClient } from "./client";
...
// The Connect-handshake helpers shared between `ConcileClient` and the headless drain below.
...
// Service Worker (or any UI-less context) can drain the durable queue with no `ConcileClient`.
export type { HeadlessDrainOptions } from "./headless-drain";
export { drainOutboxOnce } from "./headless-drain";

// Auth slice A1 — the token-lifecycle manager over a `ConcileClient` (rotation, refresh
...
export { mintEncodedDocumentId as mintDocumentId } from "@concile/id-codec";
