 ...
 *
 * Shape (verdict §(d)):
 *  - **Web Locks leader** (`concile:outbox:<origin>:<deployment>`) — probe `navigator.locks`,
 ...
 */
import { jsonToConvex, type JSONValue, type Value } from "@concile/values";
import type { MutationBatchEntry, ServerMessage } from "@concile/sync";
 ...
 *
 * The client is a browser SDK; it must NOT depend on the server-side `@concile/scheduler`
 * component (which pulls in `@concile/executor` and the whole engine). So the formula is
...
export function dropIfNonReplayable(outbox: OutboxStorage, entry: OutboxEntry): boolean {
  ...
  console.error(
    `[concile] outbox: dropping a queued "${entry.udfPath}" entry at hydrate — replaying an auth ` +
...
export interface OutboxDrainOptions {
  /** The Web Locks lock name — `concile:outbox:<origin>:<deployment>`. */
  lockName: string;
  ...
  private flushChunk(): void {
    ...
    for (const entry of all) {
      ...
      if (entry.identityFingerprint !== undefined && entry.identityFingerprint !== currentFingerprint) {
        console.error(
          `[concile] outbox: dropping mutation "${entry.udfPath}" — it was queued under a different ` +
  ...
  onResponse(msg: MutationResponse): void {
    ...
    if (msg.code !== undefined) {
      ...
      if (this.poisonPolicy === "pause") {
        this.paused = true;
        console.error(
          `[concile] outbox drain PAUSED on a coded failure of "${entry.udfPath}" (${msg.code}); ` +
