 ...
 * origin). Durability is opt-in constructor config (`new ConcileClient(transport, { outbox:
 ...
 * construction, never reused across a reload.** A fresh `ConcileClient` instance always mints a
 ...
 *  (default 1000)". Overridable via `new ConcileClient(transport, { outboxMaxQueueSize })`. */
 ...
 *  survives past this `ConcileClient` instance's lifetime, which is exactly today's (pre-outbox)
