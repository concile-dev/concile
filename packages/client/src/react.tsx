...
import { convexToJson, type Value } from "@concile/values";
import { ConcileClient, type PendingMutationEntry } from "./client";
...
const ClientContext = createContext<ConcileClient | null>(null);

export function ConcileProvider(props: { client: ConcileClient; children: ReactNode }) {
  return <ClientContext.Provider value={props.client}>{props.children}</ClientContext.Provider>;
}

export function useConcileClient(): ConcileClient {
  const client = useContext(ClientContext);
  if (!client) throw new Error("useConcileClient must be used within <ConcileProvider>");
  return client;
}
 ...
 * without an `outbox` configured — `ConcileClient.pendingMutations()`'s own no-outbox behavior).
...
// Reactive in-app inbox helper (@concile/notifications) — see ./notifications.
