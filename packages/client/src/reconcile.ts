 ...
 */
import { jsonToConvex } from "@concile/values";
import type { StateModification } from "@concile/sync";
...
export class Reconciler {
  ...
  private rebuild(forceNotify?: ReadonlySet<string>): void {
    ...
    for (const d of dropped) {
      ...
      console.warn(`[concile] optimistic update for "${path}" threw during replay; dropping its pending layer`, d.error);
    }
  }
    ...
    for (const mod of modifications) {
      if (mod.type === "QueryUpdated") {
        ...
        if (sub) {
          ...
          console.error(`[concile] query "${sub.path}" failed: ${mod.error}`);
        ...
        if (sub) {
          ...
          if (mod.reset === undefined && sub.renderMode === undefined && sub.answered) {
            console.error(`[concile] query "${sub.path}" incremental diff arrived with no established render mode; resyncing`);
            ...
            if (drift) {
              console.error(`[concile] query "${sub.path}" diff checksum mismatch; resyncing`);
              this.onDrift?.(mod.queryId);
            }
          }
        }
  ...
  onMutationSuccess(requestId: string, ts: number | undefined): void {
    ...
    if (ts === undefined || ts <= 0) {
      console.warn(`[concile] mutation "${entry.udfPath}" acked with no usable commitTs (ts=${ts}); dropping its layer now`);
      this.log.delete(requestId);
      this.rebuild();
      return;
    }
  ...
  private armGateTimer(requestId: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      console.warn(`[concile] mutation "${entry.udfPath}" layer not confirmed within ${this.gateTimeoutMs}ms; dropping it`);
      this.log.delete(requestId);
      this.rebuild();
    }, this.gateTimeoutMs);
