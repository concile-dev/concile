...
import { ConcileClient, loopbackTransport, anyApi, memoryOutbox, type ClientTransport } from "../src/index";
...
function newClient(session: string): ConcileClient {
  return new ConcileClient(loopbackTransport(runtime.connect(session)));
}
...
describe("ConcileClient — reactive subscriptions", () => {
...
describe("ConcileClient — one-shot query and mutation", () => {
...
describe("ConcileClient — protocol safety", () => {
...
describe("ConcileClient — query failure surfacing (no silent QueryFailed drop)", () => {
...
describe("ConcileClient — the OutboxStorage seam (Task 1: identity only)", () => {
  ...
  it("mints a durable clientId when constructed with an `outbox`", async () => {
    const client = new ConcileClient(loopbackTransport(runtime.connect("sOutbox1")), { outbox: memoryOutbox() });
  ...
  it("two client instances sharing the same outbox mint DIFFERENT clientIds — never reused across a reload", async () => {
    ...
    const a = new ConcileClient(loopbackTransport(runtime.connect("sOutboxA")), { outbox });
    const b = new ConcileClient(loopbackTransport(runtime.connect("sOutboxB")), { outbox });
  ...
  it("the minted clientId's meta row is durable in the outbox itself", async () => {
    const outbox = memoryOutbox();
    const client = new ConcileClient(loopbackTransport(runtime.connect("sOutboxMeta")), { outbox });
    const identity = await client.getOutboxIdentity();
    expect(identity).toBeDefined();
