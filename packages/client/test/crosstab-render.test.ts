 ...
 *
 * Two `ConcileClient`s share one `IDBFactory` (fake-indexeddb) — a faithful two-tab model — and
...
const CHANNEL_NAME = "concile:outbox:app:default:pending";
...
function baseSubscribe(t: MockTransport, client: ConcileClient): unknown[][] {
...
describe("cross-tab live optimistic rendering (T-crosstab)", () => {
  it("1. tab A enqueues (durable) -> tab B renders the pending row, with EQUAL placeholder ids (deterministic seed replay)", async () => {
    ...
    const clientA = new ConcileClient(tA, { outbox: indexedDBOutbox({ indexedDB: idb }), outboxLocks: null, outboxDrainIntervalMs: 0 });
    const framesA = baseSubscribe(tA, clientA);

    const tB = new MockTransport();
    const clientB = new ConcileClient(tB, {
  ...
  it("2. registry miss in B -> no layer, accessor status only, exactly one console.warn", async () => {
    ...
    try {
      ...
      const tB = new MockTransport();
      const clientB = new ConcileClient(tB, { outbox: indexedDBOutbox({ indexedDB: idb }), outboxLocks: null, outboxDrainIntervalMs: 0 }); // no registry
      const framesB = baseSubscribe(tB, clientB);
  ...
  it("6. missed-settle backstop: an entry deleted from the store directly drops on the next enqueued re-read", async () => {
    ...
    const tA = new MockTransport();
    const clientA = new ConcileClient(tA, { outbox: outboxA, outboxLocks: null, outboxDrainIntervalMs: 0 });
    baseSubscribe(tA, clientA);
  ...
  it("7. legacy compat: a bare `1` payload still nudges accessor listeners, mirrors nothing, throws nothing", async () => {
    ...
    try {
      const tB = new MockTransport();
      const clientB = new ConcileClient(tB, { outbox: indexedDBOutbox({ indexedDB: idb }), outboxLocks: null, outboxDrainIntervalMs: 0 });
      const framesB = baseSubscribe(tB, clientB);
  ...
  it("8. memory-outbox client: no shared store, no mirroring, unaffected by another tab's broadcasts", async () => {
    ...
    const clientC = new ConcileClient(tC, {
      outbox: memoryOutbox(),
      outboxLocks: null,
      outboxDrainIntervalMs: 0,
      optimisticUpdates: { "messages:send": registryFn },
  ...
  it("10. own-tab regression: a foreign `enqueued` broadcast must not backstop-drop THIS tab's own post-ack gated layer", async () => {
    ...
    const tA = new MockTransport();
    const clientA = new ConcileClient(tA, {
      outbox: indexedDBOutbox({ indexedDB: idb }),
      outboxLocks: null,
      outboxDrainIntervalMs: 0,
      optimisticUpdates: { "messages:send": registryFn },
  ...
  it("12. a mirrorFromStore rejection (fail-stopped outbox) routes to the console floor, never an unhandled promise rejection", async () => {
    ...
    try {
      ...
      const tB = new MockTransport();
      const clientB = new ConcileClient(tB, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0 });
  ...
  it("13. [I-1] a tab constructed AFTER another tab's durable enqueue renders the pending row at construction, with no broadcast required", async () => {
    ...
    const tA = new MockTransport();
    const clientA = new ConcileClient(tA, { outbox: indexedDBOutbox({ indexedDB: idb }), outboxLocks: locks, outboxDrainIntervalMs: 0 });
    baseSubscribe(tA, clientA);
