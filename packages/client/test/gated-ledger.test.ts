 ...
 * `ConcileClient`. Two harnesses (verdict §(h)):
...
import { ConcileClient, loopbackTransport, anyApi, MutationUndeliveredError, type ClientTransport, type OptimisticStoreView } from "../src/index";
...
describe("Gated Ledger — controlled frames (MockTransport)", () => {
  ...
  function baseSubscribe(t: MockTransport, client: ConcileClient, seed: string[]): string[][] {
    const frames: string[][] = [];
    client.subscribe(api.messages.list, { conversationId: "c1" }, (v) => frames.push(bodies(v)));
    t.emit({
      type: "Transition",
