/**
 * Task 4 — the drain, through the REAL `ConcileClient` (verdict §(d) "Drain"). A `MockTransport`
...
import type { ClientMessage, MutationBatchEntry, ServerMessage } from "@concile/sync";
...
describe("OutboxDrain — hydrate + FIFO by persisted order (T4)", () => {
  it("hydrates a prior session's durable entries into the log and drains them FIFO by `order`, not seq", async () => {
    ...
    const client = new ConcileClient(t, { ...armedClientOpts, outbox });
...
describe("OutboxDrain — the transient-stop chunk contract (T4)", () => {
  it("a codeless failure + a silent remainder re-send on the next chunk, from the failed unit", async () => {
    ...
    const t = new MockTransport();
    const client = new ConcileClient(t, { ...armedClientOpts, outbox, outboxBackoffMs: () => 0 });
    client.setOutboxArmed(true);
...
describe("OutboxDrain — the applied-drop routes by `replayed`: fresh vs replay (T4, review-caught)", () => {
  it("a FRESH (non-replayed) drain apply holds its layer until the covering Transition — no flicker", async () => {
    const outbox = memoryOutbox();
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0 });
    client.setOutboxArmed(true);
...
describe("OutboxDrain — leader handoff across two tabs sharing one queue (T4, hazard 7)", () => {
  it("a successor leader drains the entries the dead leader never finished, under recorded ids", async () => {
    ...
    const clientA = new ConcileClient(tA, { outbox, outboxLocks: locks, outboxDrainIntervalMs: 0 });
    ...
    const clientB = new ConcileClient(tB, { outbox, outboxLocks: locks, outboxDrainIntervalMs: 0 });
...
describe("OutboxDrain — first-connect (reload) fires the handshake + drain (T4 / T3 handoff #1)", () => {
  ...
  it("an EMPTY outbox sends NO first-connect Connect (byte-identical to the pre-drain client)", async () => {
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox: memoryOutbox(), outboxLocks: null, outboxDrainIntervalMs: 0 });
    await tick();
