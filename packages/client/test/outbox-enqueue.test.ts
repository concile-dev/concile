/**
 * Task 2 — durable enqueue + the ConnectAck-armed park swap, through the REAL `ConcileClient`
...
import { ConcileClient } from "../src/client";
...
describe("ConcileClient.mutation — no-outbox-config byte-identity", () => {
...
describe("ConcileClient.mutation — durable enqueue (Task 2)", () => {
  ...
  it("write-behind: the append is fired but never awaited by the send — the Mutation frame is on the wire before append() even resolves", () => {
    ...
    const client = new ConcileClient(t, { outbox: storage });
    void client.mutation("messages:send", { body: "hi" });
...
describe("ConcileClient.setAuth — the identityFingerprint cache (Task 2)", () => {
...
describe("ConcileClient — the S4 park swap (Task 2)", () => {
...
describe("ConcileClient.mutation — the `{ transient: true }` escape hatch", () => {
  ...
  it("is a no-op for a client constructed with no outbox at all — byte-identical wire shape either way", () => {
    const t = new MockTransport();
    const client = new ConcileClient(t); // no `outbox` option
