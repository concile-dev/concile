 ...
 * and `onClientReset`, through the REAL `ConcileClient`. Two harnesses (the verdict §(h) split):
 *
...
import type { ClientMessage, ServerMessage } from "@concile/sync";
...
function enqueueUnsent(client: ConcileClient, body: string): Promise<unknown> {
...
describe("ConcileClient — the Connect handshake on reopen (T3)", () => {
  ...
  it("sends Connect{clientId, held, ackedThrough} on reopen for a configured outbox — held = every unsettled durable entry, ackedThrough omitted when the lowest held seq is 0", async () => {
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox: memoryOutbox() });
...
describe("ConcileClient — ConnectAck verdict settlement (T3)", () => {
  function armedClient(outbox: OutboxStorage = memoryOutbox()): { t: MockTransport; client: ConcileClient } {
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox });
    return { t, client };
  }
...
describe("ConcileClient — the baseline await (T3 / spec decision 5)", () => {
...
describe("ConcileClient — first-connect handshake vs. a subscription delivered before hydrate finishes (T4 bug fix)", () => {
  ...
  it("a subscription created (and answered) BEFORE hydrate completes does not starve whenBaselineAdopted() forever", async () => {
    ...
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox: delayed, outboxLocks: null, outboxDrainIntervalMs: 0 });
...
describe("ConcileClient — the applied-drop is flicker-free (T3)", () => {
...
describe("ConcileClient — onClientReset on ConnectAck{known:false} (T3)", () => {
  it("re-mints the clientId, re-enqueues unsent under NEW seqs, rejects parked LOUDLY, and fires the callback", async () => {
    ...
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox, onClientReset: (i) => resets.push(i) });
    client.setOutboxArmed(true);
...
describe("ConcileClient — Connect/ConnectAck against the REAL SyncProtocolHandler (T3)", () => {
  ...
  it("the real handler answers a client's Connect with a ConnectAck: the client arms + surfaces a non-empty deploymentId", async () => {
    ...
    const client = new ConcileClient(wrap.transport, { outbox: memoryOutbox() });
  ...
  it("an unsent seq the server never saw classifies as a foreign timeline → known:false → onClientReset, end-to-end", async () => {
    ...
    const client = new ConcileClient(wrap.transport, { outbox: memoryOutbox(), onClientReset: (i) => resets.push(i) });
    const oldCid = (await client.getOutboxIdentity())!.clientId;
