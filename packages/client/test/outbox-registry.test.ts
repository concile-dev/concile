...
describe("optimisticUpdates registry — precedence (T5)", () => {
  it("a live call-site closure wins; the registry is NEVER consulted for a live mutation()", () => {
    ...
    const client = new ConcileClient(t, { optimisticUpdates: { "messages:send": registryFn } });
...
describe("optimisticUpdates registry — hydrate-only consultation (T5)", () => {
  ...
  it("the SAME persisted seed mints IDENTICAL placeholders across two client instances (a simulated reload)", async () => {
    ...
    const client1 = new ConcileClient(t1, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0, optimisticUpdates: { "messages:send": registryFn } });
    const seen1: unknown[] = [];
    ...
    const client2 = new ConcileClient(t2, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0, optimisticUpdates: { "messages:send": registryFn } });
    const seen2: unknown[] = [];
  ...
  it("a hydrate-time registry MISS warns exactly ONCE per udfPath, not once per entry — the entry still drains fine", async () => {
    ...
    const t = new MockTransport();
    const client = new ConcileClient(t, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0 }); // no registry at all
    await tick();
  ...
  it("a registry miss on udfPath A does not suppress the warning for a DIFFERENT unregistered udfPath B", async () => {
    ...
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const t = new MockTransport();
    new ConcileClient(t, { outbox, outboxLocks: null, outboxDrainIntervalMs: 0 });
    await tick();
