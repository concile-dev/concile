 ...
 *  - `ConcileClient`'s reopen sequence: a `MockTransport` that can synthesize `onClose`/`onReopen`
...
import { ConcileClient, webSocketTransport, reconnectDelayMs, type ClientTransport } from "../src/index";
...
describe("webSocketTransport — reconnect state machine", () => {
  ...
  it("a FAILED first-connect attempt (socket dies before ever opening) makes the NEXT open fire onReopen too — the 8ff4dda fix, isolated", () => {
    ...
    // `hadFailedConnect` directly at the transport, without going through `ConcileClient` — this
...
// ================================================================================================
// `ConcileClient`'s reopen sequence — via a MockTransport that can synthesize reopen on demand.
// ================================================================================================
...
describe("ConcileClient — reconnect reopen sequence (S4 flush path)", () => {
  ...
  it("a transport with no onReopen (e.g. loopbackTransport-shaped) behaves exactly as before — no crash, no flush", () => {
    ...
    expect(() => new ConcileClient(bare)).not.toThrow();
  });
