...
import { SYSTEM_TABLE_DEFINITIONS } from "@concile/values";
...
export interface ComponentTypeInfo {
   ...
   * from `contextType.import` — e.g. `@concile/scheduler`'s `["cronJobs"]`, so an app's
...
const DEFAULT_HEADER = "// Generated by Concile. Do not edit.\n";
...
export function generateDataModel(schema: SchemaDefinitionJSON, options: CodegenOptions = {}): GeneratedFile {
  ...
  // definitions in `@concile/values`, so `Id<"_storage">`/`Doc<"_storage">`/`v.id("_storage")`
  ...
  const content = `${options.header ?? DEFAULT_HEADER}import type { GenericId } from "@concile/values";
 ...
 * `api.messages.send.__path` format exactly — see `@concile/client`'s `api.ts#getFunctionPath`)
 * reachable from a generated `Api`-shaped type. Spec §(k) decision 6: the durable outbox's
 * `optimisticUpdates` registry (`ConcileClient`'s constructor option, `@concile/client`) is
 ...
 * (no per-key Args inference), so `@concile/client` itself only needs a plain string-keyed
 ...
 * `@concile/scheduler`'s design spec §5.2.
 */
...
export function generateServer(_schema: SchemaDefinitionJSON, options: CodegenOptions = {}): GeneratedFile {
  ...
  // Components can re-export extra named values through server.ts (e.g. `@concile/scheduler`'s
  ...
  // `api`/`internal` — the runtime path-building proxy (`@concile/client`'s `anyApi`, typed at
  ...
  const content = `${options.header ?? DEFAULT_HEADER}export { query, mutation, action, httpAction, httpRouter } from "@concile/executor";
...
export function generateIds(
  ...
  const content = `${options.header ?? DEFAULT_HEADER}import { mintEncodedDocumentId } from "@concile/id-codec";
 ...
 *  \`concile dev\`/\`concile codegen\` against the live deployment lineage — the server
