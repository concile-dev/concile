 ...
 *
 * Runs once per `push()` (dev boot + every hot reload, `serve` boot, `concile codegen`) — see
 ...
 */
import type { SchemaDefinitionJSON, ValidatorJSON } from "@concile/values";
...
export function assertShardByDeclarations(
  ...
  if (errors.length > 0) {
    throw new Error(`concile: invalid shardBy declaration(s):\n  - ${errors.join("\n  - ")}`);
  }
}
