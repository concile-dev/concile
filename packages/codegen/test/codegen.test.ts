...
describe("generateServer", () => {
  it("re-exports the runtime function builders and the generated types", () => {
    ...
    expect(server.content).toContain('export { query, mutation, action, httpAction, httpRouter } from "@concile/executor";');
  ...
  it("emits runtime `api`/`internal` proxy values typed against the generated Api/Internal", () => {
    const server = generateServer(schema);
    expect(server.content).toContain('import { anyApi } from "@concile/client";');
  ...
  it("re-exports a component's serverExports from its contextType.import", () => {
    const server = generateServer(schema, {
      components: [{ name: "scheduler", contextType: { import: "@concile/scheduler", type: "SchedulerContext" }, serverExports: ["cronJobs"] }],
    });
    expect(server.content).toContain('export { cronJobs } from "@concile/scheduler";');
  });
