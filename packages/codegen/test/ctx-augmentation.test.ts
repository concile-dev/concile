...
describe("generateServer — ctx augmentation", () => {
  it("emits a typed ctx.<component> augmentation for components with a contextType", () => {
    const out = generateServer(emptySchema, {
      components: [{ name: "auth", contextType: { import: "@concile/auth", type: "AuthContext" } }],
    });
    expect(out.content).toContain('declare module "@concile/executor"');
    expect(out.content).toContain('auth: import("@concile/auth").AuthContext');
  ...
  it("emits augmentation for multiple components", () => {
    const out = generateServer(emptySchema, {
      components: [
        { name: "auth", contextType: { import: "@concile/auth", type: "AuthContext" } },
        { name: "storage", contextType: { import: "@concile/storage", type: "StorageContext" } },
        { name: "noCtx" },
      ],
    });
    expect(out.content).toContain('auth: import("@concile/auth").AuthContext');
    expect(out.content).toContain('storage: import("@concile/storage").StorageContext');
