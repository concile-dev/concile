import type { SchemaDefinition, Validator, JSONValue } from "@concile/values";
import type { RegisteredFunction, TablePolicy, PolicyContextProvider, GuestDatabaseWriter, ActionApi } from "@concile/executor";
import type { ComponentContext } from "@concile/executor";
 ...
 * `@concile/triggers` consumes). `op` is derived from the log entry: `value === null` → `"delete"`,
...
export interface DriverContext {
   ...
   * change feed (`@concile/triggers`). `limit` bounds the number of SCANNED revisions (not matched
   ...
   * `load_documents` scan at all). This is how `@concile/triggers` seeds a NEW (non-`fromStart`)
   * trigger's cursor at the log's current tip cheaply, instead of paying for a scan just to discover
   * where "now" is — see `@concile/triggers`' `src/boot.ts`.
   */
   ...
   * startup can validate a config-supplied function path (e.g. `@concile/triggers`' `handler`
   ...
   * doesn't need path validation (e.g. `@concile/scheduler`'s, which dispatches whatever `fnPath`
   ...
   * `@concile/runtime-cloudflare`'s `GlobalReactivityPoller`). Delegates to
 ...
 * itself, as this repo's routes carry no named params (see `@concile/executor`'s `matchRoute`, and
...
export interface ComponentDefinition {
   ...
   * (typically `_`-prefixed) modules. See `ContextProvider.buildAction` in `@concile/executor`.
   */
  buildAction?: (api: ActionApi) => object;
  /**
   * Extra named values this component wants codegen to re-export from `_generated/server.ts`,
   * sourced from `contextType.import` — e.g. `@concile/scheduler` sets `["cronJobs"]` so an
