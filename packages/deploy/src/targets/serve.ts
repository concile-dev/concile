...
function creds(ctx: DeployContext): { url: string; adminKey: string } {
  const url = String(ctx.target.settings.url ?? process.env.CONCILE_DEPLOY_URL ?? "").trim();
  const adminKey = String(ctx.target.settings.adminKey ?? process.env.CONCILE_ADMIN_KEY ?? "").trim();
  return { url, adminKey };
}
...
export const serveTarget: DeployTarget = {
  ...
  async preflight(ctx) {
    ...
    if (!url) throw new DeployError("serve target needs a url — pass --url or set deploy.targets.serve settings / CONCILE_DEPLOY_URL");
    if (!adminKey) throw new DeployError("CONCILE_ADMIN_KEY is required to deploy to a serve target");
  },
