...
const DO_BINDING = "CONCILE_DO";
const DO_CLASS = "ConcileDO";
...
/** Additively ensure the Concile DO bindings/migration/compat flag (+ optional R2) exist. Never
...
export function reconcileWrangler(input: Record<string, unknown>, opts: ReconcileOpts): ReconcileResult {
  ...
  if (opts.needsR2) {
    ...
    if (!buckets.some((b) => b.binding === R2_BINDING)) {
      buckets.push({ binding: R2_BINDING, bucket_name: opts.r2BucketName ?? "concile-storage" });
