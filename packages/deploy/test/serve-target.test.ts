...
function ctxWith(settings: Record<string, unknown>): DeployContext {
  return {
    cwd: "/tmp", functionsDir: "/tmp/concile", env: "production",
...
describe("serveTarget", () => {
  ...
  afterEach(() => { server?.close(); server = undefined; delete process.env.CONCILE_ADMIN_KEY; delete process.env.CONCILE_DEPLOY_URL; });
