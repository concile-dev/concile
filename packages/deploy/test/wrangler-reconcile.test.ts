...
describe("reconcileWrangler", () => {
  it("adds the DO binding + sqlite migration + nodejs_compat to a bare config, preserving user fields", () => {
    ...
    expect(r.config).toMatchObject({
      ...
      durable_objects: { bindings: [{ name: "CONCILE_DO", class_name: "ConcileDO" }] },
      migrations: [{ tag: "v1", new_sqlite_classes: ["ConcileDO"] }],
      compatibility_flags: ["nodejs_compat"],
    });
    expect(r.added).toContain("durable_objects.CONCILE_DO");
  });
  ...
  it("does not collide with a non-sequential existing migration tag", () => {
    ...
    const sqliteMig = (r.config.migrations as Array<{ tag: string; new_sqlite_classes?: string[] }>).find((m) => m.new_sqlite_classes?.includes("ConcileDO"));
