...
[`@concile/docstore-sqlite`](../docstore-sqlite) (`SqliteDocStore`). This package adds only the one
...
Concile's only INTEGER columns are logical timestamps (a per-store monotonic `MAX(ts)+1` counter
...
(`@concile/docstore/test-support/conformance` — the identical contract SQLite/Postgres/PGlite run),
