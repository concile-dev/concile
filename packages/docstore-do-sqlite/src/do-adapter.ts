 ...
 * Concile's only INTEGER columns are logical timestamps (`ts`/`prev_ts`/`commit_ts` — a per-store
 ...
 */
import type { DatabaseAdapter, PreparedStatement, RunResult, SqlRow, SqlValue } from "@concile/docstore-sqlite";
 ...
 *  (unreachable for Concile's monotonic-counter timestamps; see the module note). */
