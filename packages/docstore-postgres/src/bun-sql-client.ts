 ...
 * reservation and any commit-pool shard reservations to always get a slot. A `CONCILE_PG_STREAM=0`/
 ...
 * `resolveStreamingEnabled` exactly (same env var, same semantics): `CONCILE_PG_STREAM=0` (or
...
function resolveStreamingEnabled(): boolean {
  const v = process.env.CONCILE_PG_STREAM;
  return v !== "0" && v !== "false";
}
...
export class BunSqlClient implements PgClient {
   ...
   *  ON (the default) — absent (`undefined`) when `CONCILE_PG_STREAM=0`/`"false"` disables it, so
  ...
  constructor(opts: {
    ...
    // Kill switch (CONCILE_PG_STREAM=0/"false"): leave `queryStream` unassigned so it stays
  ...
  async acquireWriterLock(): Promise<void> {
    ...
    if (rows[0]?.ok !== true) {
      throw new Error("another Concile engine is already connected to this database (advisory lock held)");
    }
  }
  ...
  private async *queryStreamImpl(sql: string, params?: readonly PgValue[]): AsyncIterable<PgRow> {
    ...
    const cursorName = `concile_stream_${this.nextStreamCursorId++}`;
