 ...
 * `CONCILE_PG_STREAM=0` (or `"false"`) to make `NodePgClient` NOT advertise `queryStream`, so
...
export class NodePgClient implements PgClient {
  ...
  private buildClient(applicationName?: string): pg.Client {
    return new Client({
      ...
      // per-node `concile-fleet-<port>` so an operator — or the fleet self-exit E2E — can identify
