 ...
 *
 * Run: `CONCILE_TEST_DATABASE_URL=postgres://sb:pw@localhost:5433/bunsql_smoke bun test/bun-sql-smoke.ts`
...
import type { DocumentLogEntry, IndexWrite, InternalDocumentId, Interval } from "@concile/docstore";

const DEFAULT_URL = "postgres://sb:pw@localhost:5433/bunsql_smoke";
const URL = process.env.CONCILE_TEST_DATABASE_URL ?? DEFAULT_URL;
...
async function resetDatabase(): Promise<void> {
  ...
  await admin.query(`DROP SEQUENCE IF EXISTS concile_ts`);
  await admin.close();
}
...
async function testStreamErrorDiscardsConn(): Promise<void> {
  ...
  try {
    ...
    for await (const _row of client.queryStream!(`SELECT * FROM concile_stream_error_probe_missing_table`)) {
      // never reached
    }
  } catch {
    threw = true;
...
// ── kill switch: CONCILE_PG_STREAM=0/"false" disables queryStream on the Bun path, mirroring
//    NodePgClient's identical env var ────────────────────────────────────────────────────────────
async function testKillSwitch(): Promise<void> {
  const original = process.env.CONCILE_PG_STREAM;
  try {
    process.env.CONCILE_PG_STREAM = "0";
    const disabled = new BunSqlClient({ connectionString: URL });
    assert.equal(disabled.queryStream, undefined, "CONCILE_PG_STREAM=0 must leave queryStream unassigned");
    await disabled.close();

    process.env.CONCILE_PG_STREAM = "false";
    const disabled2 = new BunSqlClient({ connectionString: URL });
    assert.equal(disabled2.queryStream, undefined, 'CONCILE_PG_STREAM="false" must leave queryStream unassigned');
    await disabled2.close();

    delete process.env.CONCILE_PG_STREAM;
    const enabledUnset = new BunSqlClient({ connectionString: URL });
    assert.equal(typeof enabledUnset.queryStream, "function", "unset CONCILE_PG_STREAM must default queryStream ON");
    await enabledUnset.close();

    process.env.CONCILE_PG_STREAM = "1";
    const enabledOne = new BunSqlClient({ connectionString: URL });
    assert.equal(typeof enabledOne.queryStream, "function", 'CONCILE_PG_STREAM="1" (any non-"0"/"false" value) must leave queryStream ON');
    await enabledOne.close();
  } finally {
    if (original === undefined) delete process.env.CONCILE_PG_STREAM;
    else process.env.CONCILE_PG_STREAM = original;
  }
  ok("kill switch: CONCILE_PG_STREAM=0/\"false\" disables queryStream; unset/\"1\" leaves it enabled");
}
