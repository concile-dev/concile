...
describe("NodePgClient — CONCILE_PG_STREAM kill switch", () => {
  const ENV_KEY = "CONCILE_PG_STREAM";
  ...
  it('does NOT advertise queryStream when CONCILE_PG_STREAM="0"', () => {
    process.env[ENV_KEY] = "0";
    const client = new NodePgClient({ connectionString: "postgres://fake" });
    expect(client.queryStream).toBeFalsy();
  });

  it('does NOT advertise queryStream when CONCILE_PG_STREAM="false"', () => {
    process.env[ENV_KEY] = "false";
    const client = new NodePgClient({ connectionString: "postgres://fake" });
    expect(client.queryStream).toBeFalsy();
  });

  it("advertises a working queryStream when CONCILE_PG_STREAM is unset (default ON)", async () => {
    delete process.env[ENV_KEY];
    const client = new NodePgClient({ connectionString: "postgres://fake" });
  ...
  it('advertises a working queryStream when CONCILE_PG_STREAM="1"', async () => {
