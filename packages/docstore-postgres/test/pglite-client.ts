...
export class PgliteClient implements PgClient {
   ...
   *  `@concile/test`'s embedded runtime) never collide on a cursor name. See `queryStream` below. */
  private cursorSeq = 0;
   ...
   * independent pool/pinned connections (see its own doc comment), `@concile/test` defaults to
  ...
  // `CONCILE_TEST_DATABASE_URL`-gated test + the T6 fleet E2E against real Postgres. The two
