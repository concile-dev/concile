 ...
 *   - a document's field `Value`s         → `@concile/values`' `convexToJson`/`jsonToConvex`
 *
 ...
 */
import { convexToJson, jsonToConvex, type JSONValue } from "@concile/values";
import type {
  ConflictStrategy,
  DatabaseIndexUpdate,
  DatabaseIndexValue,
...
/* --- base64 (portable: global btoa/atob, no Node Buffer — matches @concile/values' json.ts) --- */
...
export const MIGRATION_DUMP_FORMAT = "concile-migration-dump";
