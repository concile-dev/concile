 ...
 */
import type { InternalDocumentId, ShardId } from "@concile/id-codec";
import { documentIdKey } from "@concile/id-codec";
import type { JSONValue, Value } from "@concile/values";
...
export interface DocStore {
   ...
   * `nextval('concile_ts')` inside the commit transaction, so the ts is visible atomically with
   ...
   * (`@concile/docstore-postgres`) or a synchronous `SqliteGuardQuerier`
   * (`@concile/docstore-sqlite`) — so this one interface member is deliberately typed loosely
