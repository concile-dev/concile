...
import type { QueryRuntime } from "@concile/query-engine";
...
export const COLLECT_BRAND = Symbol.for("concile.executor.collectBrand");
 ...
 * `asc` and `desc` (see `PaginatedResult.pageBounds`'s doc comment in `@concile/query-engine`
...
// Always-on at every tier (Tier-0 SQLite, `concile dev`'s 8 virtual shards, fleet). The one
