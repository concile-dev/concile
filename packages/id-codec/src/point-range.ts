 ...
 * a dependency on `@concile/fleet` (an EE package objectstore-substrate must not import — the
 * object-store substrate and the Postgres fleet are siblings, not a hierarchy).
 *
 * Homed in `@concile/id-codec`, NOT `@concile/index-key-codec`, despite `SerializedKeyRange`
 ...
 * (`indexKeyspaceId`/`tableKeyspaceId` — `@concile/index-key-codec`'s `keyspace.ts`), so they
 * inherently need both id spaces. `@concile/id-codec` already depends on
 * `@concile/index-key-codec` (see `jump-hash.ts`'s `encodeIndexKey` import), so this direction adds
