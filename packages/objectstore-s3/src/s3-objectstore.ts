...
import { CasConflict, isCasConflict, type ObjectStore } from "@concile/objectstore";
...
export interface S3ObjectStoreOpts {
   ...
   *  to true whenever `endpoint` is set (mirrors `@concile/blobstore-s3`'s `makeS3Client`). */
...
export class S3ObjectStore implements ObjectStore {
  ...
  /** Write-once/keep-first (matches fs/memory — see `@concile/objectstore`'s `ObjectStore` doc).
