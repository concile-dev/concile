 ...
 *
 * GATED: only runs when docker is available AND `CONCILE_OBJECTSTORE_S3=1` is set, so the default
 * `bun run --filter @concile/objectstore-s3 test` stays green (skipped) with no docker/env present.
...
const BUCKET = "concile-objectstore-conformance";
