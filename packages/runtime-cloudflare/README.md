...
The single-shard **Cloudflare Durable Object host** for Concile — Slice 3 of the DO-native program
...
`ConcileDurableObject` is a **unified** Durable Object: one object owns the OCC writer, the DO-SQLite
...
explicitly with the **`CONCILE_DO_LOCATION_HINT`** env var (e.g. `enam`): the Worker reads it per
...
[`@concile/runtime-cloudflare-shard`](../../ee/packages/runtime-cloudflare-shard) router (Slice 6, M1).
...
- **App code is statically bundled** (decision 4): `generateWorkerEntrySource` emits static imports of
  every module/schema/config (no dir-scan in a DO), the twin of `concile build`'s entrypoint codegen.
