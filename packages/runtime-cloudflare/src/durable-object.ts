/**
 * `ConcileDurableObject` — the UNIFIED single-shard Durable Object host (Slice 3, decision 1).
 *
...
import type { D1Client } from "@concile/docstore-d1";
...
export interface DurableObjectAppConfig {
  ...
  adminKey: string;
  /** File-storage byte backend (`env.R2` → `@concile/blobstore-r2`), supplied by the concrete DO
   ...
   *  subclass (wrapping its raw `env.DB` with `@concile/docstore-d1`'s `bindingD1Client`). Injected,
...
export abstract class ConcileDurableObject {
   ...
   */
  private ensureSession(ws: DoWebSocketLike): ConcileSocketAttachment | null {
  ...
  async fetch(request: Request): Promise<Response> {
    await this.bootDone;
    if (this.bootError) {
      return jsonResponse(500, { error: "concile: Durable Object failed to boot", detail: String(this.bootError) });
    }
   ...
   */
  private updateAttachmentFromMessage(ws: DoWebSocketLike, att: ConcileSocketAttachment, text: string): string {
    let msg: {
      type?: string;
      token?: unknown;
  ...
  private persistAttachment(ws: DoWebSocketLike, att: ConcileSocketAttachment): void {
    try {
