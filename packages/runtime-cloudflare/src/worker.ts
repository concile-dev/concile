 ...
 * in `wrangler.jsonc` (e.g. `"CONCILE_DO"`). Every request — HTTP and WS upgrade alike — forwards
 * to the single DO instance.
 *
 * Placement: the deployment can pin its ONE DO's home region with `CONCILE_DO_LOCATION_HINT` (e.g.
  ...
  return {
    async fetch(request: Request, env: Record<string, unknown>): Promise<Response> {
      ...
      if (!ns || typeof ns.idFromName !== "function") {
        return jsonError(500, `concile: Durable Object binding "${bindingName}" is not configured in wrangler.jsonc`);
      }
      ...
      if (typeof rawHint === "string" && rawHint.length > 0) {
        if (!isValidLocationHint(rawHint)) {
          return jsonError(
            500,
            `concile: ${DEPLOYMENT_LOCATION_HINT_ENV}="${rawHint}" is not a valid Durable Object location hint. ` +
