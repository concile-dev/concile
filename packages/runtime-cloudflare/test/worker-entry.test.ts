/**
 * The Worker-entry codegen (Task 7). `generateWorkerEntrySource` is the DO twin of `concile build`'s
...
describe("generateWorkerEntrySource", () => {
  const src = generateWorkerEntrySource({
    ...
    configAbsPath: "/app/convex/concile.config.js",
  ...
  it("statically imports every module + schema + config (no dir scan, no top-level await)", () => {
    ...
    expect(src).toContain(`import * as __config from "/app/convex/concile.config.js";`);
  ...
  it("exports a concrete DO class (named in wrangler) + the default Worker handler", () => {
    ...
    expect(src).toContain(`export default createWorkerHandler("CONCILE_DO");`);
    // Admin key is read from the env at boot (fail-fast on empty happens in serve; here it's threaded).
    expect(src).toContain(`env["CONCILE_ADMIN_KEY"]`);
  });
  ...
  it("omits the config import + empties components when there is no concile.config", () => {
