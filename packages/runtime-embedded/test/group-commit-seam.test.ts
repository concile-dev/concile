 ...
 * same aggregate the fleet health seam (`@concile/fleet`'s `node.ts`) reads for `/api/health`.
 */
...
class HookedSqliteStore extends SqliteDocStore {
  ...
  override async commitWriteBatch(units: readonly CommitUnit[], shardId?: import("@concile/id-codec").ShardId): Promise<bigint[]> {
