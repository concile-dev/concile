 ...
 *   1. every import in `host.ts` is either `@concile/*` or type-only, and none names a forbidden
...
describe("host.ts is import-neutral (RuntimeHost seam)", () => {
  ...
  it("imports only @concile/* or type-only specifiers", () => {
    ...
    for (const { typeOnly, spec } of imports) {
      const neutral = typeOnly || spec.startsWith("@concile/");
...
const SKIP_DIRS = new Set(["node_modules", "dist", ".turbo", ".git", "coverage", ".concile-deploy"]);
