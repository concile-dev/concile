 ...
 * (`ProcessRuntimeHost`) lives in `@concile/cli` and is asserted `satisfies RuntimeHost` there
 * (Task 2). Here we only prove the neutral seam is IMPLEMENTABLE with `@concile`-only types — a
...
describe("RuntimeHost seam contract", () => {
  it("is implementable with neutral (@concile-only) types", () => {
