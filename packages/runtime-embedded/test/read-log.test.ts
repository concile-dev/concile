...
import { composeComponents, type DriverContext } from "@concile/component";
...
import { newDocumentId, encodeInternalDocumentId, type InternalDocumentId } from "@concile/id-codec";
...
describe("DriverContext.readLog", () => {
  ...
  it("limit:0 peeks the current bound at O(1) cost — no scan, no crash — on a non-empty log", async () => {
    ...
    // 0` special-case in `runtime.ts`'s `readLog`. `@concile/triggers` relies on this exact idiom
