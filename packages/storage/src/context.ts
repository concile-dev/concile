 ...
 * `@concile/executor`). Mirrors `components/scheduler/src/facade.ts`: a `build(cctx)` in-txn
 ...
 */
import type { ComponentContext, ActionApi, ContextProvider } from "@concile/executor";
import { GuestDatabaseWriter } from "@concile/executor";
import type { BlobStore, UploadTarget, BlobMetadata } from "@concile/blobstore";
 ...
 * `build` throws (see `ContextProvider.write` in `@concile/executor`). With it, and only during a
