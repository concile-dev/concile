...
/**
 * Internal `_storage` metadata mutations/queries for `@concile/storage` — the low-level ops
 ...
 * `ctx.runMutation`-style trusted callers, mirroring `@concile/admin`'s `_system:*` built-ins
 * (`packages/admin/src/system-functions.ts`) — NOT `@concile/component`'s namespaced-component
 ...
 * name (`"_storage"`), exactly like `@concile/admin`'s built-ins use bare table names such as
