 ...
 * shape is defined ONCE in `@concile/values` (`storageDocumentFields`) and consumed by both this
 * module (the live `TableDefinition` the engine registers) and codegen (the emitted `DataModel`),
 * so the two can never drift.
 *
 * The table NUMBER is reserved and stable in `@concile/id-codec` (`STORAGE_TABLE_NUMBER`): a
 * persisted `Id<"_storage">` encodes it, so it must decode to the same table forever.
 */
import { defineTable, storageDocumentFields } from "@concile/values";
import { STORAGE_TABLE_NUMBER } from "@concile/id-codec";

/** The reserved name of the file-storage document table. */
export const STORAGE_TABLE = "_storage";

/** The reserved, stable system-table number for `_storage` (see `@concile/id-codec`). */
export { STORAGE_TABLE_NUMBER };

/**
 * The live `_storage` `TableDefinition`, built from the canonical field validators in
 * `@concile/values`. `_id`/`_creationTime` are auto-added by the engine/codegen, as for every
