 ...
 * `MemoryBlobStore` in `@concile/blobstore`'s test-support isn't a published export). `delete`
...
describe("storageReaper — orphan sweep driver", () => {
  ...
  it("stop() while a tick is in flight does not resurrect the driver once that tick settles", async () => {
    ...
    // `stop()` raced in while a sweep's `runFunction` was still awaiting (e.g. a `concile dev`
