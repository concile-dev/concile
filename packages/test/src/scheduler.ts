...
/** Fully-qualified name of `@concile/scheduler`'s jobs table, as it lands in the composed
...
/**
 * Privileged raw scan of `@concile/scheduler`'s `jobs` table, reusing the SAME `_test:_run`
 ...
 *
 * A clean no-op if `@concile/scheduler` wasn't composed (no `defineScheduler()` in
 ...
 * `@concile/scheduler` is composed (a no-op tick otherwise — there's no driver to drive).
