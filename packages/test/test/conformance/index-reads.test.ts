...
// D1: Concile has no `.withIndex(cb)` — range predicates chain directly on the
...
// A second, INLINE `createTestConcile` over a single-field index (`by_n`, not the shared
...
describe("conformance — index reads (inline single-field index, gt/lte/desc/take)", () => {
  ...
  beforeEach(async () => {
    t2 = await createTestConcile({ modules: { "mod.ts": idxMod, "schema.ts": { default: idxSchema } } });
