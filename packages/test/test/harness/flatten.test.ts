...
describe("flattenModules", () => {
  ...
  it("normalizes import.meta.glob-style keys (./concile/ prefix, the DEFAULT_FUNCTIONS_ROOT) to the same function-path root as explicit keys", async () => {
    const send = mutation(async () => "ok");
    const ping = httpAction(async () => new Response("ok"));
    const schema = { __isSchema: true };
    const router = { __isRouter: true, routes: [] };
    const out = await flattenModules({
      "./concile/messages.ts": { send },
      "./concile/schema.ts": { default: schema },
      "./concile/http.ts": { default: router, ping },
    });
  ...
  it("does NOT implicitly strip a legacy convex/ prefix when functionsRoot defaults to concile", async () => {
    const send = mutation(async () => "ok");
