...
/**
 * Harness self-tests — these exercise `@concile/test` itself (not an app), proving the
 * properties the rest of the conformance suite silently depends on: every `createTestConcile()`
...
describe("harness — isolation", () => {
  it("two instances do not share data (each is its own SQLite :memory:)", async () => {
    const a = await createTestConcile({ modules: { "messages.ts": messages, "schema.ts": { default: schema } } });
    const b = await createTestConcile({ modules: { "messages.ts": messages, "schema.ts": { default: schema } } });
    try {
  ...
  it("a string path and the equivalent anyApi-proxy ref resolve to the SAME function", async () => {
    const t = await createTestConcile({ modules: { "messages.ts": messages, "schema.ts": { default: schema } } });
    try {
  ...
  describe("schema: 'auto'", () => {
    ...
    it("rejects a wrong-typed write at runtime — schema validation is enforced", async () => {
      ...
      const { mutation } = await import("@concile/executor");
      const { defineSchema, defineTable, v } = await import("@concile/values");
      const badSchema = defineSchema({ nums: defineTable({ n: v.number() }) });
      const mod = {
        insert: mutation(async (ctx: any, a: any) => ctx.db.insert("nums", a)),
      };
      const t = await createTestConcile({ modules: { "mod.ts": mod, "schema.ts": { default: badSchema } } });
      try {
