...
export async function startBackend(opts: StartBackendOptions, deps: { spawn: SpawnFn; probe: ProbeFn }): Promise<Backend> {
  ...
  while (Date.now() < deadline) {
    if (exited) { stop(); throw new Error(`concile dev exited before becoming ready (code ${exitCode})`); }
    if (await deps.probe(opts.port)) return { stop };
    await new Promise((r) => setTimeout(r, interval));
  }
  stop();
  throw new Error(`concile dev did not become ready on port ${opts.port} within ${timeout}ms`);
}
