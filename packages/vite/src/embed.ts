/**
 * Phase 2 — in-process embed. Boot the Concile engine INSIDE Vite's own dev-server process (no
 * child `concile dev`, no proxy hop) and serve it as connect-middleware + a `/api/sync` WebSocket
 * on Vite's own origin. Opt-in via `concile({ mode: "embed" })`; the default proxy mode
 * (`index.ts`) is untouched.
 *
 * The engine boot, HTTP dispatch, codegen, and sync wiring are all REUSED from `@concile/cli`
...
import type { ConcileVitePluginOptions } from "./index";
 ...
 */
export function embedPlugin(options: ConcileVitePluginOptions): Plugin {
  return {
    name: "concile:embed",
    async configureServer(server) {
      ...
      // (also where DEFAULT_FUNCTIONS_DIR comes from: a static top-level import of `@concile/cli`
      ...
      let cli: typeof import("@concile/cli");
      try {
        cli = await import("@concile/cli");
      } catch (e) {
        throw new Error(
          `@concile/vite embed mode requires @concile/cli to be installed — ${e instanceof Error ? e.message : String(e)}`,
        );
      }
      const {
        bootProject,
      ...
      const dataPath = options.dataPath ? resolve(root, options.dataPath) : join(root, ".concile", "dev.db");
      ...
      // storage/component routes — is fixed at boot, exactly as `concile dev`/`serve` behave).
      ...
      if (!server.httpServer) {
        server.config.logger.warn(
          "[concile] embed mode needs Vite's own HTTP server — under `server.middlewareMode` the reactive /api/sync WebSocket and engine cleanup do NOT wire up. Use proxy mode, or run Vite without middlewareMode.",
        );
      }
      ...
      const reload = async () => {
        try {
          ...
          log.info(`[concile] ↻ pushed (${Object.keys(next.project.moduleMap).length} functions)`);
        } catch (e) {
          log.error(`[concile] ✗ reload failed: ${e instanceof Error ? e.message : String(e)}`);
        }
      };
      ...
      log.info(`[concile] embed → engine in-process on Vite's origin (admin key: ${adminKey})`);
    },
  };
}
