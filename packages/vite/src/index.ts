...
// Mirrors `DEFAULT_FUNCTIONS_DIR` from `@concile/cli` (`packages/cli/src/functions-dir.ts`).
// NOT imported: `@concile/cli` is an optional peer dependency (see package.json) so that
// proxy-mode-only consumers — who spawn `concile dev` as a child process and never touch the
...
// a test can assert this literal hasn't drifted from `@concile/cli`'s own constant — see
// `test/plugin.test.ts`'s "DEFAULT_FUNCTIONS_DIR guard" — without a static top-level import of
// `@concile/cli` here in the shipped proxy path itself.
export const DEFAULT_FUNCTIONS_DIR = "concile";

export interface ConcileVitePluginOptions {
  /**
   * How the backend is run alongside Vite:
   *   - `"proxy"` (default): spawn `concile dev` as a child and proxy the engine-owned prefixes to
   *     it (Phase 1 — unchanged). Node builtins only; no `@concile/*` runtime dependency.
   *   - `"embed"`: boot the engine INSIDE Vite's own process as connect-middleware + a `/api/sync`
   *     WebSocket — no child, no proxy hop (Phase 2). Reaches `@concile/cli` via a dynamic import,
   *     so proxy-mode users never pull it. See `docs/superpowers/specs/2026-05-15-vite-phase2-embed-design.md`.
   */
  mode?: "proxy" | "embed";
  /** App functions dir → `--dir` (default "concile"). Shared by both modes. */
  functionsDir?: string;

  // ── proxy mode ──────────────────────────────────────────────────────────────────────────────
  /** Backend port to proxy to (default: an OS-assigned free port). */
  port?: number;
  /** How to invoke the CLI (default: local node_modules/.bin/concile, else `npx concile`). */
  command?: string;
  /** Extra flags forwarded to `concile dev` (e.g. ["--database-url", "postgres://…"]). */
  args?: string[];

  // ── embed mode ──────────────────────────────────────────────────────────────────────────────
  /** SQLite file for the in-process engine (default `<root>/.concile/dev.db`). Ignored in proxy mode. */
  dataPath?: string;
...
/**
 * Single-origin dev: `vite` alone serves the frontend AND the Concile backend on one browser origin
 * (no manual proxy, no CORS). Two modes behind one surface (default `"proxy"` — Phase 1, unchanged):
 *   - `"proxy"`: spawn `concile dev` and proxy the engine-owned prefixes to it.
 *   - `"embed"`: boot the engine in Vite's own process (no child, no proxy hop).
 */
export function concile(options: ConcileVitePluginOptions = {}): Plugin {
  if ((options.mode ?? "proxy") === "embed") return embedPlugin(options);
  return proxyPlugin(options);
}

/** Phase 1 (default): spawn `concile dev` as a child and inject Vite's proxy. Byte-for-byte the
 *  behavior that shipped in Phase 1 — the only change is that it's now reached via the mode switch. */
function proxyPlugin(options: ConcileVitePluginOptions): Plugin {
  let port: number;
  ...
  return {
    ...
    async configureServer(server) {
      ...
      backend = await startBackend(
        { command: cli.command, args, cwd: root, port, onLog: (l) => server.config.logger.info(`[concile] ${l}`) },
