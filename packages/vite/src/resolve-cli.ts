...
/** How to invoke the concile CLI: an explicit override (split on whitespace), else the app's local
 *  `node_modules/.bin/concile`, else `npx concile`. */
export function resolveCli(cwd: string, override?: string): ResolvedCli {
  ...
  const localBin = join(cwd, "node_modules", ".bin", "concile");
  if (existsSync(localBin)) return { command: localBin, baseArgs: [] };
  return { command: "npx", baseArgs: ["concile"] };
}
