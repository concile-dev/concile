...
describe("@concile/vite — embed mode (engine IN Vite's process, no child, no proxy)", () => {
  ...
  it("serves /api/health (200, in-process) and /api/sync ws, coexisting with Vite's HMR ws", async () => {
    ...
    vite = await createViteServer({
      ...
      plugins: [concile({ mode: "embed", functionsDir: "concile", dataPath: join(dataDir, "dev.db") })],
    });
    await vite.listen();
