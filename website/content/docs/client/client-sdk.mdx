...
`@concile/client` is the framework-agnostic client that runs the reactive sync protocol underneath
your app: a transport, a `ConcileClient` built on top of it, and (via `@concile/client/react`)
hooks that subscribe a component to a query and re-render it on every push.

This page covers the whole client surface: construction, transports, every `ConcileClient` method,
