...
maybe with a sleep or a wait for an external signal in between. `@concile/workflow` runs that
whole sequence durably.

A workflow survives a process restart because it isn't held in memory. Each step's outcome is
journaled to a durable table, and concile **replays the whole handler from the top** every time
...
It's built on [`@concile/scheduler`](/docs/components/scheduling) (`requires: ["scheduler"]`),
