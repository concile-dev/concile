...
This page is the process guide: how an idea becomes a shipped change in concile. If you're
...
The short version: concile treats "how we build" as seriously as "what we build." Every
...
Concile isn't built feature-by-feature in a random order. It's built in **slices**, each one a
...
the project README's "What works today" status list before proposing something
...
`concile dev` starts up aren't polish on top of the "real" product. They are the product. A
...
*uses* concile. `docs/dev/` and `docs/superpowers/` are internal engineering material:
...
Even though concile's runtime is Convex-shaped and a `concile migrate` on-ramp exists for
Convex users, `@concile/*` is the one documented, canonical import surface. `convex/*`
...
real `concile dev` or `concile serve` server**, not just an isolated mechanism test. These
...
Beyond a normal code review, concile uses staged reviews at each pipeline step: a plan review,
...
Concile's core is licensed under **FSL-1.1-Apache-2.0** (free to use, modify, and self-host,
including commercially, but you may not offer concile itself as a competing hosted service;
