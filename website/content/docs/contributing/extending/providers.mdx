...
`@concile/notifications` groups delivery into **channels**: `email`, `sms`, `push`, and
...
`@concile/auth` needs external systems too: it sends OTP/magic-link/verification emails, and it
...
`@concile/notifications` in your project, auth's verification/reset/magic-link/OTP emails
...
own separate auth service) rather than doing an OAuth redirect through concile, that's the `jwt`
...
token is then exchanged for a concile session the same way an OAuth login is, through the shared
...
Which adapter is active is decided purely by configuration: set `CONCILE_STORAGE_BUCKET` (or
...
Once you've internalized that shape, adding a new vendor anywhere in concile is always "write a
