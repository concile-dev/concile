...
Every backend Concile ships today, SQLite and Postgres, talks to the engine through the exact same doorway. Concile's engine (the transactor, the query engine, reactivity) never talks to a database directly. It talks to one narrow TypeScript interface called `DocStore`. Whatever sits behind that interface (SQLite, Postgres, a Durable Object's embedded SQLite, or something you write yourself) is invisible to everything above it.

That's the whole pitch of this page: **to back Concile with a different database, you implement one interface, and nothing else in the engine has to change.**
...
The commit timestamp is allocated as `MAX(ts) + 1`, computed inside the same transaction as the writes. Since Concile enforces single-writer, no other writer can race in a higher timestamp between the `MAX(ts)` read and the insert. The whole thing is race-free by construction, not by locking.
