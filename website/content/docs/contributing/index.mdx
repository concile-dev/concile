...
This section is for a different reader than the rest of the docs. It's for people who want to work *on* concile itself: reading the engine source, fixing a bug, adding a storage adapter, or writing a new component. If you just want to use concile in your app, go to [Get Started](/docs/get-started/what-is-concile) or [Core Concepts](/docs/core-concepts/schema-and-tables) instead.

## concile in one breath

concile is an open-source, self-hostable, reactive backend. You write ordinary TypeScript functions: queries, mutations, and actions. They run on the server inside a transaction.
...
- The familiar *authoring style* (how you write a query or a schema) is not a promise to be a drop-in replacement for anything. concile's canonical imports are its own (`@concile/*`), and it has already grown capabilities its prior art doesn't have: durable workflows with saga/compensation, a Postgres adapter, a single-binary build. Compatibility with an existing project shows up as a migration on-ramp (`concile migrate`), not the product's identity.
...
- **[Extending](/docs/contributing/extending/custom-component)**: the seams you can build against without touching the engine core.
  - [A custom component](/docs/contributing/extending/custom-component): like `@concile/auth` or `@concile/scheduler`.
...
- The reactive core.
- Single-node/single-binary production tooling: `concile serve`, Docker self-host, live hot-deploy, a compiled standalone binary.
