...
Think of a file the way concile thinks of any other row: something you can reference, query, and
react to. The bytes themselves just happen to live somewhere else.

File storage is always on. There's nothing to opt into in `concile.config.ts`. The moment you run
`concile dev`, every project has a reserved `_storage` system table and a `ctx.storage` facade
...
this resolves to a CDN or bucket public URL (when `CONCILE_STORAGE_PUBLIC_URL` is configured). On
...
URL (public files with `CONCILE_STORAGE_PUBLIC_URL` set).
...
`concile dev`/`serve`'s env and flag selection below.

</Tab>

</Tabs>

For `concile dev`/`serve`, the byte backend is picked automatically at boot based on whether an
...
`--database-url`/`CONCILE_DATABASE_URL` (see [Self-hosting](/docs/deploy/self-hosting)).

The bucket is the *only* switch between the two backends. If `CONCILE_STORAGE_ENDPOINT`,
`_REGION`, or `_PUBLIC_URL` are set but `CONCILE_STORAGE_BUCKET` isn't, the server refuses to
...
On the filesystem backend, `concile dev`/`serve` also fails fast at boot if the storage
