...
Think of `concile dev` as your app's dev server, except the backend behind it is the real one, not
...
From your app's root (where `concile/` lives):
...
1. **Loads your project.** It resolves `concile.config.ts` (component composition), then reads
   `concile/schema.ts` and every function file under `concile/`.
2. **Generates `concile/_generated/`**: the typed `api`/`Doc`/`Id` helpers your functions and client
   import.
...
5. **Starts watching `concile/`** for changes and hot-reloads your functions on every save (see
...
Here's every flag `concile dev` accepts:
...
| `--dir <path>` | `concile` | Where your schema and functions live. |
| `--data <path>` | `.concile/data.db` | SQLite database file path. Ignored when `--database-url` is set. |
...
| `--database-url <url>` | unset | A `postgres://` or `postgresql://` connection string. When set, concile uses Postgres instead of SQLite; see [Postgres](/docs/deploy/postgres). Also readable from `CONCILE_DATABASE_URL`, though the flag wins. |
| `--storage-bucket <name>` | unset | Select the S3-compatible file storage backend (bucket name). Unset means local filesystem storage under `<data dir>/storage`. Also readable from `CONCILE_STORAGE_BUCKET`. |
| `--storage-endpoint <url>` | unset | S3-compatible endpoint (MinIO, R2, and so on), only meaningful alongside `--storage-bucket`. Also readable from `CONCILE_STORAGE_ENDPOINT`. |
...
Setting `CONCILE_STORAGE_ENDPOINT`/`REGION`/`PUBLIC_URL` without a bucket is a misconfiguration,
and `concile dev` refuses to boot from it. Otherwise it would silently write uploads to local disk
...
- If `CONCILE_ADMIN_KEY` is set in your environment, `concile dev` uses it as is.
- If it's unset, `concile dev` generates a fresh ephemeral key on every run and prints it to
  stdout. Nothing persists it. Restart the server and you get a new one.
- If `CONCILE_ADMIN_KEY` is set but blank or whitespace, `concile dev` treats it as unset (a
...
A persistent `CONCILE_ADMIN_KEY` is never embedded in the HTML, loopback or not.

</Callout>

## Hot reload

`concile dev` watches `concile/` (recursively) via a filesystem watch, ignoring anything under
`concile/_generated/` so codegen's own writes don't re-trigger themselves. On every change, after a
...
2. Re-writes `concile/_generated/` with the freshly generated types.
3. Calls `runtime.setModules(...)` with the new function map. This replaces the module map
   wholesale, so `concile dev` re-applies the always-on `_storage:*` built-in functions on every
   reload. They aren't part of your `concile/` source, so they'd otherwise vanish on the first save.
...
This regenerates `concile/_generated/` and exits: no server, no watch loop. `concile dev` does this
automatically on every change. You need the explicit command before
[`concile serve`](/docs/deploy/self-hosting), which never runs codegen itself (it fails fast at
boot if `_generated/` is missing).

## Call a function without the dashboard

There's no `concile run` command. Either use the dashboard's [function runner](#function-runner),
...
Open the URL `concile dev` prints: `http://localhost:<port>/_dashboard`. In dev mode, on a loopback
...
That's the right tradeoff for local development, and it's why `concile dev` hands you the key with
zero friction on a loopback bind. But never expose an unprotected admin key over the network.

[`concile serve`](/docs/deploy/self-hosting) serves the same dashboard build in production, but
key-less by default (no key embedded in the HTML at all: it prompts you), and requires you to set a
real `CONCILE_ADMIN_KEY` before it will even start.
...
- [Self-hosting with Docker](/docs/deploy/self-hosting): `concile serve`, the production
  counterpart of `concile dev` (no codegen, required admin key, binds `0.0.0.0`, dashboard served
  key-less).
- [Deploying without a restart](/docs/deploy/deploy-and-build): `concile deploy`, for pushing a
  schema and function change to an already-running `serve` without hot-reload's dev-only watch loop.
- [Vite plugin](/docs/deploy/vite-plugin): a Vite app can run `concile dev` for you as part of
  its own dev server.
