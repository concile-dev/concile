...
SQLite is the zero-config default for `concile serve`. It's a single file, and it needs nothing
...
`CONCILE_DATABASE_URL` environment variable instead. The flag wins if both are set. Leave both
...
including unset, falls back to SQLite. The same flag and env var work for `concile dev` and for a
compiled `concile build` binary too: the storage backend is a boot-time choice, not something
...
`docker compose up` now persists to the `postgres-data` named volume instead of `concile-data`.
...
Exactly one concile engine may be connected to a given Postgres database at a time. On boot,
...
`concile serve --fleet`, the multi-node story built on top of this same adapter.
...
<Callout type="warn" title="Known limitations">
  **Single pinned connection, no automatic reconnect.** The engine holds exactly one Postgres
  ...
  does not transparently reconnect. Restart the concile process to re-establish it.
...
`@concile/docstore-postgres` implements `PostgresDocStore`, the Postgres analogue of the SQLite
...
This is on by default and needs no configuration. Set `CONCILE_PG_STREAM=0` (or `false`) to
...
Group commit defaults on for single-node Postgres and off for SQLite. `CONCILE_GROUP_COMMIT`
...
single-writer store at this tier. Postgres doesn't make concile's transactor multi-writer by
...
drives the real `concile serve` production entrypoint (spawned as the real CLI binary, not an
...
- [Scaling](/docs/deploy/scaling): `concile serve --fleet`, multiple nodes sharing this same
...
- [Configuration reference](/docs/reference/configuration): the full flag/env table, including
  `CONCILE_DATABASE_URL` and `CONCILE_GROUP_COMMIT` alongside every other setting.
