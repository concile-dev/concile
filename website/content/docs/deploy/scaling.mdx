...
concile runs as a set of tiers, not one fixed shape. Think of it as a building with three floors.
You write your schema, queries, mutations, and subscriptions once, on the ground floor, and the
same code runs unchanged whether you're on a laptop running `concile dev`, one `concile serve`
...
`concile serve` process is one writer: every mutation goes through it, fully serializable, no
...
`concile serve --fleet` runs a small, symmetric fleet of `concile serve` processes that share
...
- **`--database-url` / `CONCILE_DATABASE_URL`** must point at Postgres. SQLite has no concept of
  a shared lease across processes, so `--fleet` against SQLite (or no database URL at all) fails
  fast at boot.
- **`--advertise-url` / `CONCILE_ADVERTISE_URL`** is the URL *other* fleet nodes use to reach this
...
- **The same `CONCILE_ADMIN_KEY` on every node.** Nodes authenticate to each other's internal
  forwarding endpoint with this key.
- **A unique data directory per node.** A node's local replica lives at `<dir>/fleet-replica.db`
  alongside whatever `--data`/`CONCILE_DATA_DIR` you gave it. This isn't validated: two nodes
  sharing a directory silently stomp each other's replica file instead of failing fast.
- **`@concile/fleet` installed.** `serve` loads it via a dynamic `import()` (core concile has no
...
missing advertise URL, `@concile/fleet` not installed, or `--fleet` combined with
...
variable, are in [the CLI reference](/docs/core-concepts/cli#concile-serve).
...
- **A writer that's alive but stuck** (a GC pause, a frozen process, a hung transaction) is a
  ...
  writer's lease carries a time limit, **`CONCILE_FLEET_LEASE_TTL_MS`** (default `15000`), that it
...
- Lowering `CONCILE_FLEET_LEASE_TTL_MS` makes a wedged-writer takeover faster at the cost of a
...
concile recognizes the marker and hands back the original result instead of running the write a
...
running `concile dev`, a single `concile serve`, or a fleet. `concile dev` runs the full shard
...
and if a wedged writer can never corrupt or fork a shard's history. Concile's design for this,
...
- **Default: 8**, whether on `concile dev`, a single `serve`, or a fleet.
- **Set explicitly with `CONCILE_FLEET_SHARDS`** (a positive integer) before the deployment's very
  first boot. It's read once, persisted, and every later boot reads the persisted value back.
- **Immutable after first boot.** A `CONCILE_FLEET_SHARDS` value that disagrees with what's
...
or `CONCILE_DATABASE_URL`, must resolve to a real Postgres URL) and refuses to run against a fleet
...
`CONCILE_FLEET_SHARDS` (or unset it) on every node before restarting the fleet. The command's own
...
`CONCILE_GROUP_COMMIT=1` batches concurrent writes on a single shard (or an unsharded fleet, which
...
(`CONCILE_FLEET_MULTI_WRITER=1`) over one shared Postgres, 4 shards per node, driver routing every
...
- **`--object-store` / `CONCILE_OBJECT_STORE`** selects the backend by URL scheme: `s3://` /
...
- **`--shards N` / `CONCILE_FLEET_SHARDS`** (object-store boots only) sizes a **multi-shard
...
- **`--replica` / `CONCILE_REPLICA`** boots this node as a **read-only replica** of an
  ...
  `--object-store`.
- **`--writer-url` / `CONCILE_WRITER_URL`** (replica boots only) makes a replica **forward**
...
- **`CONCILE_OBJECTSTORE_GC_MS`** sets the writer's background garbage-collection sweep cadence
  (default ~60 seconds), reclaiming segments and snapshots already superseded by a later snapshot.
- **`--wake-url` / `CONCILE_WAKE_URL`** and **`--backstop-min-ms` / `CONCILE_BACKSTOP_MIN_MS`**
...
[the CLI reference](/docs/core-concepts/cli#concile-serve).
...
Both `@concile/fleet` (Tier 2) and `@concile/objectstore-substrate` (Tier 3) are `ee/`-licensed
packages: source-available under concile's separate, non-converting commercial license, not the
...
Core concile has no hard dependency on either `ee/` package: `serve --fleet` and
