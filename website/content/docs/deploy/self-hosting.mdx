...
This is the baseline way to self-host concile. A generic `concile:latest` image runs
`concile serve`, your app's `concile/` folder is mounted in, and its SQLite database sits on a
...
`concile serve` is `concile dev` minus the file watcher and codegen-write, plus production
hardening.
...
1. **`CONCILE_ADMIN_KEY` must be set and non-blank.** There is no default and no fallback. A
...
routes, any component-contributed routes (for example `@concile/auth`'s OAuth callbacks), and,
...
`CONCILE_ADMIN_KEY` to, and it's used from then on to authenticate `/_admin` calls.
...
`CONCILE_ADMIN_KEY` is required and has no flag equivalent. That's deliberate: a secret belongs in
...
You need a `concile/` directory with **committed `_generated/`**. `serve` fails fast without it, as
...
(`CONCILE_ADMIN_KEY: ${CONCILE_ADMIN_KEY:?set CONCILE_ADMIN_KEY in a .env file}`). Compose
...
bind-mounts `./concile` **read-only** into `/app/concile`, and persists SQLite on the named
`concile-data` volume at `/data/db.sqlite`. The container's command is exactly
...
The SQLite database lives on the `concile-data` Docker volume, not inside the container's
...
`@concile/*` package links **nested per-package**, for example
`/app/packages/cli/node_modules/@concile/executor`, never at `/app/node_modules`.

That's fine for code living inside a workspace package. But your bind-mounted `/app/concile` is not
under any package. Its bare `import "@concile/values"` (every `schema.ts`) and the
`_generated/server` module's re-export of `@concile/executor` resolve by walking **up** to
...
`@concile/*` name straight into `/app/node_modules/`, so a mounted (or baked-in) `concile/`
...
(The same `RUN` also creates and chowns `/app/.concile-deploy`, the scratch directory
`concile deploy`'s push target writes to. It's the same class of bug: `/app`'s directory node
...
The default compose file bind-mounts `concile/` at run time. That's convenient for local
...
separate `concile/` directory around, build a small wrapper `Dockerfile` on top of
`concile:latest` that copies your app in instead of mounting it:
...
Because the root-`node_modules` symlink fixup above is part of the base `concile:latest` image
(baked into the `runner` stage, not something the bind-mount does at run time), a `COPY`ed
`concile/` resolves its `@concile/*` imports exactly the same way a mounted one does. No extra
...
`serve` also accepts `--fleet`/`CONCILE_FLEET`, `--advertise-url`, `--object-store`,
...
`CONCILE_DATABASE_URL`. No other change to the compose file's shape is required beyond swapping
...
host `concile serve`; Docker is the baseline, not a requirement. The checklist is always the
same: set `CONCILE_ADMIN_KEY` as a platform secret (never in a config file), mount a persistent
...
Set `CONCILE_ADMIN_KEY` in the service's Railway variables. Railway terminates TLS for you.
...
Set the secret with `fly secrets set CONCILE_ADMIN_KEY=...`, and start the process as
`concile serve --dir ./concile --data /app/data/db.sqlite`. Fly terminates TLS at its edge.

</Tab>

</Tabs>

A managed Postgres on either platform works through the same `CONCILE_DATABASE_URL` as anywhere
else. Both platforms also have an automated push path: `concile deploy --target railway` and
...
concile serves plain HTTP. `serve` has no TLS termination built in. Put a reverse proxy (nginx,
Caddy, or Traefik) in front of the container to terminate TLS and forward to `concile:3000`.
...
- [Postgres](/docs/deploy/postgres): swap the SQLite volume for a Postgres database, still
  single-node.
- [Deploy and build](/docs/deploy/deploy-and-build): `concile deploy` for live hot-swap onto a
  running `serve` (no restart, `--allow-deploy` opt-in), and `concile build` for a single
