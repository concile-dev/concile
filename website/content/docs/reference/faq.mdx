---
title: FAQ
description: Honest answers about what concile is, what it isn't, and what isn't built yet.
---
...
invalidation (not table-level); optimistic updates; a durable offline outbox; `concile
...
Multi-node scale is newer, and it lives under a separate license. `concile serve --fleet` (Tier
...
If you're choosing concile today, start with single-node self-hosting (SQLite or Postgres). It's
...
See [What is concile?](/docs/get-started/what-is-concile) for the full pitch and
...
`concile migrate` flags a Convex app's use of either as unsupported in its migration report
...
concile serves plain HTTP. Put a reverse proxy (nginx, Caddy, Traefik) in front for HTTPS. This
...
Within `@concile/workflow`: a workflow can't start and await another workflow as one of its own
...
MinIO, and its tail items have shipped since: a multi-shard writer (`concile serve
...
`CONCILE_OBJECTSTORE_GC_MS`), and an offline reshard tool (`concile objectstore reshard`).
...
the right choice for `concile dev`, and for self-hosted single-node production too. It's
...
(or `CONCILE_DATABASE_URL`), no code changes. It's also the required shared substrate if you're
...
- A single-binary compile (`concile build`) embeds the engine, your app, and the dashboard in one
  executable, useful for Electron/Tauri-style distribution.
- Native `@concile/*` imports are canonical, not `convex/*`. concile is its own product, not a
  Convex account or drop-in replacement. `concile migrate --from convex` is the on-ramp (see
  [Migrate from Convex](/docs/reference/migrate-from-convex)), not the identity.
- concile has already grown past Convex's shipped feature set in some areas: durable workflows
...
`ctx.db.replace(id, { ...doc, ...changes })`. `concile migrate` flags every `.patch(...)` call it
...
scorecard and its caveats. concile is a clean-room build studied against Convex's open, published
architecture documentation, not a fork or a decompilation of Convex's code.

## How does concile compare to Firebase/Supabase?
...
- concile's reactivity comes from one mechanism, in one process: deterministic TypeScript
  functions with recorded read/write sets, intersected at commit time. There's no rules DSL to gate
  access (authorization is ordinary function code, optionally composed via `@concile/authz`'s row
  policies), and no fleet of services to run. One process is the whole backend.

There's also the deploy axis. concile deploys anywhere you can run a container, a binary, or a
...
Yes. concile is licensed FSL-1.1-Apache-2.0 (the Functional Source License, the same license
Convex itself uses). That means free to use, modify, and self-host, including at scale, on your own
infrastructure. The license forbids exactly one thing: offering concile itself as a competing
...
- **Deploy anywhere.** Your box, your VPS, your cloud, Docker, an air-gapped server. No phone-home,
  ever.
- **Data and code portability.** Plain HTTP, open formats, `concile migrate` in and `concile
  migrate export`/`import` your data out. You are never trapped.

What's gated, and when: multi-node write scale-out, meaning `concile serve --fleet`
(`@concile/fleet`, Postgres-backed) and the Cloudflare-native multi-shard router
(`@concile/runtime-cloudflare-shard`), lives in a separate `ee/` area under a different commercial
...
documented as a feature anywhere else in these docs, and `concile migrate` flags a Convex app's
...
Every number concile quotes is hand-transcribed from a runnable benchmark harness under
...
Bun is primary: `concile dev`/`serve`, and the single-binary compile (`bun build --compile`). But
Node is fully supported for running the engine (npm packages, a Node SQLite adapter). The engine
itself is runtime-agnostic behind its storage/runtime seams. Neither `concile dev` nor `serve`
cares which one you're running, as long as the seam is satisfied.

## Can I run it on Cloudflare?

Yes, via `@concile/runtime-cloudflare`, in one of two genuinely different architectures. The
DO-native path is a first-class deploy target: `concile deploy --target cloudflare` reconciles
...
`wrangler deploy` of the portable `concile serve` image.
...
that's a separate paid-tier package, `@concile/runtime-cloudflare-shard`).
...
It runs the exact same portable `concile serve` image you'd run under Docker anywhere else,
...
- **The same app code runs everywhere.** Your `schema.ts` and `concile/` functions run unchanged on
  `concile dev`, `concile serve` (SQLite or Postgres), a compiled single binary, a multi-node
...
- **Your data moves too, explicitly.** `concile migrate export`/`concile migrate import` pulls
  ...
  two hosts or topologies, because every concile store shares the same logical MVCC-log shape.
...
project by listing it in `concile.config.ts` (for example, `defineAuth()`, `defineScheduler()`).
...
- New to concile: [What is concile?](/docs/get-started/what-is-concile) →
