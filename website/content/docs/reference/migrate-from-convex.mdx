...
A Convex app already looks like a concile app. Same query and mutation handlers, the same
...
`concile migrate` automates the mechanical part: import specifiers, `package.json`,
`_generated/`. For everything it can't do for you, it gives you a concrete, file-and-line list.

Canonical imports in concile are `@concile/*`, never `convex/*` (see
[What is concile?](/docs/get-started/what-is-concile)). `concile migrate` also renames your
app's function directory from `convex/` to concile's own default, `concile/`, as part of the
same run. Migrating moves your app onto the native import surface; it does not make concile
execute `convex/*` imports unchanged. There's no compatibility shim to fall back on. The codemod is
the whole migration.

This page covers two independent things under one command: `concile migrate` rewrites a Convex
project's **code**, and `concile migrate export`/`import` moves an app's **data** between
...
- `query`/`mutation`/`action` still come from `./_generated/server`. Your function bodies don't
  move.
- `schema.ts` keeps its shape (it just relocates from `convex/` to `concile/` along with the rest
...
- `useQuery`/`useMutation` keep the same signatures, imported from `@concile/client/react`
...
Convex app directory), the same directory `package.json` and `concile.config.ts` live in.
...
directory (`--dir`, default `convex`) differs from where concile itself would put it
(`concile/` by default, or your project's own `functionsDir` in `concile.config.ts`), the whole
...
`"latest"`, whichever of `@concile/values`/`@concile/client` the rewrite actually introduced
...
source, and only if `concile.config.ts` doesn't already exist. An existing config file is never
overwritten.
...
`loadFunctionsDir → push → writeGenerated` pipeline `concile codegen` uses.
...
routing, and crons, which map to three different places in concile:
...
Convex app already imports those from the same relative path concile uses.
...
runtime patterns that concile's engine doesn't accept, even after the import moves. Every finding
names the exact file and line.

**Action-needed.** A real concile equivalent exists, but you adapt the call site by hand:
...
| `ctx.auth` / `getUserIdentity()` | Identity is a string token via a context provider (e.g. `@concile/auth`'s `ctx.auth`), not a JWT-claims object |
| `crons.ts` file, or any `cronJobs(...)` call | Compose `defineScheduler()` in `concile.config.ts`; use `cronJobs()` |

**Unsupported.** No automatic path; you decide how, or whether, to proceed:

| Convex pattern | Why |
|---|---|
| `@convex-dev/auth`, or an import from `"convex/auth"` | Not auto-translated, use [`@concile/auth`](/docs/components/auth) or an external JWT/OIDC provider |
| `app.use(...)` (Convex Components), or a `convex.config.ts` file | Convex Components don't map 1:1. Recompose the equivalent concile component(s) via `concile.config.ts` |
| `.vectorIndex(...)` / `.searchIndex(...)` | Full-text and vector search are not shipped in concile |
...
starting `concile.config.ts` (skipped if one already exists), but it doesn't wire your actual
...
Add `@concile/scheduler` and `@concile/component` to `package.json` yourself. Unlike
`@concile/values`/`@concile/client`, the scaffold's own dependencies aren't auto-added: the
...
`cronJobs()` in concile is imported from `./_generated/server` (not `@concile/scheduler`
...
`concile migrate --from <source>` is not hardcoded to Convex internally. It dispatches through a
...
Convex is a near-free first source specifically because concile's runtime already executes
...
`concile migrate export` and `concile migrate import` move an app's actual rows between two
...
Both are HTTP clients, modeled on `concile deploy`, that call a **running** deployment's
bearer-gated admin endpoints (`GET /_admin/export`, `POST /_admin/import`), authenticated with the
same `CONCILE_ADMIN_KEY` the dashboard and `concile deploy --allow-deploy` use. Both the
...
point a throwaway `concile serve`/`concile dev` at its data directory first, export from that,
then tear it down.
...
| `--admin-key <key>` | both | `$CONCILE_ADMIN_KEY` | Overrides the env var |

`CONCILE_ADMIN_KEY` (env or `--admin-key`) is required for both verbs. Missing it fails
...
`concile migrate` rewrites only your app's own function-directory source (renamed from `convex/`
to `concile/` as part of the same run). It does not touch `convex-test` call sites. Switch those
to [`@concile/test`](/docs/reference/testing) yourself.
The fixture shape (`createTestConcile`) is different enough that it isn't auto-rewritten.
...
| `ctx.scheduler.runAfter`/`runAt` and crons | Yes | Shipped via the `@concile/scheduler` component, composed explicitly |
| Durable workflows | `@convex-dev/workflow` | Shipped via `@concile/workflow`, including saga/compensation |
| File storage (`ctx.storage`) | Yes | Shipped (two-phase uploads; `store`/`get` are action-only) |
| Auth | Convex Auth, or third-party JWT | `@concile/auth` (sessions, OAuth, JWT/OIDC, MFA, passkeys); Convex Auth itself doesn't port |
| Convex Components (`app.use`) | Yes | Different model: compose `@concile/*` components in `concile.config.ts` |
| Full-text search / vector search | Yes | Not built; the migrator flags usage as `unsupported` |

## Honest compatibility notes

**Near-free, by design, not a coincidence.** concile's runtime already executes Convex-shaped
...
- **No `ctx.db.patch`.** concile's writer is `insert`/`replace`/`delete` only. A Convex `patch`
  call becomes a read, spread-merge, and `replace`. See [Mutations](/docs/core-concepts/mutations).
- **No `.withIndex(...)`.** concile's query builder is `ctx.db.query(table, index)` with chained
...
- **No full-text or vector search.** `.searchIndex(...)`/`.vectorIndex(...)` have no concile
...
- **Crons need an explicit component.** `cronJobs()` only takes effect once composed into
  `concile.config.ts` via `defineScheduler({ crons })`. Convex's implicit registration doesn't
  carry over.
- **`ctx.auth` is a string token, not a JWT-claims object**, resolved via a context provider (e.g.
  [`@concile/auth`](/docs/components/auth)) rather than baked into the runtime.
- **Convex Components and Convex Auth don't map 1:1.** Recompose the equivalent concile
  component(s): [`@concile/auth`](/docs/components/auth),
  [`@concile/scheduler`](/docs/components/scheduling),
  [`@concile/workflow`](/docs/components/workflows),
  [`@concile/triggers`](/docs/components/triggers),
  [`@concile/notifications`](/docs/components/notifications), via `concile.config.ts`.

`@concile/*` is the canonical surface, permanently. This isn't a temporary migration artifact.
concile's own default function directory is `concile/`, not `convex/`; `convex/` was never the
target, only where a Convex app starts from, and `concile migrate` renames it as part of the same
run. There's no alias path back to `convex/*` imports to maintain: once migrated, you're simply
writing a native concile app.

## Related

- [What is concile?](/docs/get-started/what-is-concile): why the canonical surface is
  `@concile/*`, not `convex/*`.
- [CLI reference](/docs/core-concepts/cli#migrate): the terse flag reference for every `concile`
  subcommand, including `migrate`/`migrate export`/`migrate import`.
- [Testing](/docs/reference/testing): `@concile/test` vs. `convex-test`.
